...
- (NSString *)formatNotification:(WFCCMessage *)message {
    ...
    if ([[WFCCNetworkService sharedInstance].userId isEqualToString:self.operateUser]) {
        formatMsg = (isChinese?@"你更新了群头像":@"Bạn đã cập nhật ảnh đại diện nhóm");
    } else {
        ...
        if (userInfo.friendAlias.length > 0) {
            formatMsg = [NSString stringWithFormat:@"%@%@", userInfo.friendAlias, (isChinese?@"更新了群头像":@" đã cập nhật ảnh đại diện nhóm")];
        } else if(userInfo.groupAlias.length > 0) {
            formatMsg = [NSString stringWithFormat:@"%@%@", userInfo.groupAlias, (isChinese?@"更新了群头像":@" đã cập nhật ảnh đại diện nhóm")];
        } else if (userInfo.displayName.length > 0) {
            formatMsg = [NSString stringWithFormat:@"%@%@", userInfo.displayName, (isChinese?@"更新了群头像":@" đã cập nhật ảnh đại diện nhóm")];
        } else {
            formatMsg = [NSString stringWithFormat:@"%@<%@>%@",(isChinese?@"用户":@"Người dùng"), self.operateUser, (isChinese?@"更新了群头像":@" đã cập nhật ảnh đại diện nhóm")];
        }
    }
