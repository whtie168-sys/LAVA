...
- (NSString *)formatNotification:(WFCCMessage *)message {
    ...
    if ([[WFCCNetworkService sharedInstance].userId isEqualToString:self.creator]) {
        return [self.type isEqualToString:@"1"] ? (isChinese?@"你开启了全员禁言":@"Cấm trò chuyện toàn bộ thành viên") : (isChinese?@"你取消了全员禁言":@"Hủy bỏ lệnh cấm toàn bộ trò chuyện");
    } else {
        ...
        if (isChinese) {
            ...
            if (userInfo.friendAlias.length > 0) {
                return [NSString stringWithFormat:[self.type isEqualToString:@"1"] ? @"%@ cấm trò chuyện toàn bộ thành viên" : @"%@ hủy bỏ lệnh cấm toàn bộ trò chuyện", userInfo.friendAlias];
            } else if(userInfo.groupAlias.length > 0) {
                return [NSString stringWithFormat:[self.type isEqualToString:@"1"] ? @"%@ cấm trò chuyện toàn bộ thành viên" : @"%@ hủy bỏ lệnh cấm toàn bộ trò chuyện", userInfo.groupAlias];
            } else if (userInfo.displayName.length > 0) {
                return [NSString stringWithFormat:[self.type isEqualToString:@"1"] ? @"%@ cấm trò chuyện toàn bộ thành viên" : @"%@ hủy bỏ lệnh cấm toàn bộ trò chuyện", userInfo.displayName];
            } else {
                return [NSString stringWithFormat:[self.type isEqualToString:@"1"] ? @"User<%@> cấm trò chuyện toàn bộ thành viên" : @"<%@> hủy bỏ lệnh cấm toàn bộ trò chuyện", self.creator];
            }
        }
    }
}
