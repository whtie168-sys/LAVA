...
- (NSString *)formatNotification:(WFCCMessage *)message {
    ...
    if ([[WFCCNetworkService sharedInstance].userId isEqualToString:self.operateUser]) {
        formatMsg = (isChinese?@"你解散了群聊":@"Bạn giải tán nhóm chat");
    } else {
        ...
        if (userInfo.friendAlias.length > 0) {
            formatMsg = [NSString stringWithFormat:@"%@%@", userInfo.friendAlias, (isChinese?@"解散了群聊":@" giải tán nhóm chat")];
        } else if(userInfo.groupAlias.length > 0) {
            formatMsg = [NSString stringWithFormat:@"%@%@", userInfo.groupAlias, (isChinese?@"解散了群聊":@" giải tán nhóm chat")];
        } else if (userInfo.displayName.length > 0) {
            formatMsg = [NSString stringWithFormat:@"%@%@", userInfo.displayName, (isChinese?@"解散了群聊":@" giải tán nhóm chat")];
        } else {
            if (isChinese) {
                formatMsg = [NSString stringWithFormat:@"用户<%@>解散了群聊", self.operateUser];
            }else {
                formatMsg = [NSString stringWithFormat:@"<%@> giải tán nhóm chat", self.operateUser];
            }
        }
    }
