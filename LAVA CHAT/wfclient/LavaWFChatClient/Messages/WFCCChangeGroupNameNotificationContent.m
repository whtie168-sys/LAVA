...
- (NSString *)formatNotification:(WFCCMessage *)message {
    ...
    if ([[WFCCNetworkService sharedInstance].userId isEqualToString:self.operateUser]) {
        formatMsg = [NSString stringWithFormat:@"%@%@",(isChinese?@"你修改群名称为：":@"Bạn thay đổi tên nhóm là: "), self.name];
    } else {
        ...
        if (userInfo.friendAlias.length > 0) {
            formatMsg = [NSString stringWithFormat:@"%@%@", userInfo.friendAlias, (isChinese?@"修改群名称为：":@" thay đổi tên nhóm là ")];
        } else if(userInfo.groupAlias.length > 0) {
            formatMsg = [NSString stringWithFormat:@"%@%@", userInfo.groupAlias, (isChinese?@"修改群名称为：":@" thay đổi tên nhóm là ")];
        } else if (userInfo.displayName.length > 0) {
            formatMsg = [NSString stringWithFormat:@"%@%@", userInfo.displayName, (isChinese?@"修改群名称为：":@" thay đổi tên nhóm là ")];
        } else {
            formatMsg = [NSString stringWithFormat:@"%@%@", self.operateUser, (isChinese?@"修改群名称为：":@" thay đổi tên nhóm là ")];
        }
