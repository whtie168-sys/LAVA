...
//        return WFCCString(@"YouRecallAMessage");
        return (isChinese ? @"你撤回了一条消息" : @"Bạn đã thu hồi một tin nhắn.");
    } else {
        ...
        if (userInfo.friendAlias.length) {
            if (isChinese) {
                ...
                return [NSString stringWithFormat:@"%@ thu hồi tin nhắn.", userInfo.friendAlias];
            }
        }
        if (userInfo.displayName != nil) {
            if (isChinese) {
                ...
                return [NSString stringWithFormat:@"%@ thu hồi tin nhắn.", userInfo.displayName];
            }
        }
        if (isChinese) {
            return [NSString stringWithFormat:@"%@撤回了一条消息", self.operatorId];
        }else {
            return [NSString stringWithFormat:@"%@ thu hồi tin nhắn.", self.operatorId];
        }
    }
}
@end
