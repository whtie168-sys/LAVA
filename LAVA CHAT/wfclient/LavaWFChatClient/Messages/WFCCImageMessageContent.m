...
- (WFCCMessagePayload *)encode {
    ...
    payload.searchableContent = self.name;//([WFCCIMService.main isChinese]?@"[图片]":@"[Hình ảnh]");
...
- (void)decode:(WFCCMessagePayload *)payload {
    ...
    if ([payload isKindOfClass:[WFCCMediaMessagePayload class]]) {
        ...
        if([self.name rangeOfString:([WFCCIMService.main isChinese]?@"[图片]":@"[Hình ảnh]")].location == 0) {
            self.name = [self.name substringFromIndex:([WFCCIMService.main isChinese]?@"[图片]":@"[Hình ảnh]").length];
        }
...
- (NSString *)digest:(WFCCMessage *)message {
    return [NSString stringWithFormat:@"%@%@",([WFCCIMService.main isChinese]?@"[图片]":@"[Hình ảnh]"), self.name];
//    return ([WFCCIMService.main isChinese]?@"[图片]":@"[Hình ảnh]");
}
@end
