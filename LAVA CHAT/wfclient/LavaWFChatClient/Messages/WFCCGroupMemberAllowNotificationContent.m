...
- (NSString *)formatNotification:(WFCCMessage *)message {
    ...
    if ([self.type isEqualToString:@"1"]) {
        formatMsg = [NSString stringWithFormat:@"%@ %@", formatMsg, (isChinese?@"允许了":@"cho phép ")];
    } else {
        formatMsg = [NSString stringWithFormat:@"%@ %@", formatMsg, (isChinese?@"取消了":@"hủy ")];
    }
    ...
    if(self.targetIds.count > count) {
        if (isChinese) {
            ...
            formatMsg = [formatMsg stringByAppendingFormat:@" %ld thành viên", self.targetIds.count];
        }
    }

    formatMsg = [formatMsg stringByAppendingString:(isChinese?@"群组禁言时的发言权限":@"Quyền phát biểu khi nhóm cấm")];
