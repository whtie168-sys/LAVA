...
- (NSString *)formatNotification:(WFCCMessage *)message {
    ...
    if ([[WFCCNetworkService sharedInstance].userId isEqualToString:self.operateUser]) {
        ...
        if (userInfo.friendAlias.length > 0) {
            formatMsg = [NSString stringWithFormat:@"%@%@",(isChinese?@"你把群主转让给了":@"Bạn chuyển giao nhóm cho "), userInfo.friendAlias];
        } else if(userInfo.groupAlias.length > 0) {
            formatMsg = [NSString stringWithFormat:@"%@%@",(isChinese?@"你把群主转让给了":@"Bạn chuyển giao nhóm cho "), userInfo.groupAlias];
        } else if (userInfo.displayName.length > 0) {
            formatMsg = [NSString stringWithFormat:@"%@%@",(isChinese?@"你把群主转让给了":@"Bạn chuyển giao nhóm cho "), userInfo.displayName];
        } else {
            formatMsg = [NSString stringWithFormat:@"%@%@",(isChinese?@"你把群主转让给了":@"Bạn chuyển giao nhóm cho "), self.owner];
        }
    } else {
        WFCCUserInfo *userInfo = [[WFCCIMService sharedWFCIMService] getUserInfo:self.operateUser inGroup:self.groupId refresh:NO];
        if (userInfo.friendAlias.length > 0) {
            formatMsg = [NSString stringWithFormat:@"%@%@", userInfo.friendAlias, (isChinese?@"把群主转让给了":@" chuyển giao nhóm cho ")];
        } else if(userInfo.groupAlias.length > 0) {
            formatMsg = [NSString stringWithFormat:@"%@%@", userInfo.groupAlias, (isChinese?@"把群主转让给了":@" chuyển giao nhóm cho ")];
        } else if (userInfo.displayName.length > 0) {
            formatMsg = [NSString stringWithFormat:@"%@%@", userInfo.displayName, (isChinese?@"把群主转让给了":@" chuyển giao nhóm cho ")];
        } else {
            formatMsg = [NSString stringWithFormat:@"%@%@", self.operateUser, (isChinese?@"把群主转让给了":@" chuyển giao nhóm cho ")];
        }
