...
- (void)decode:(WFCCMessagePayload *)payload {
    ...
    if ([payload isKindOfClass:[WFCCMediaMessagePayload class]]) {
        ...
        if([self.name rangeOfString:([WFCCIMService.main isChinese]?@"[文件] ":@"[Tài liệu] ")].location == 0) {
            self.name = [self.name substringFromIndex:([WFCCIMService.main isChinese]?@"[文件] ":@"[Tài liệu] ").length];
        }
...
- (NSString *)digest:(WFCCMessage *)message {
    return [NSString stringWithFormat:@"%@%@",([WFCCIMService.main isChinese]?@"[文件]":@"[Tài liệu]"), self.name];
}
@end
