...
- (NSString *)formatNotification:(WFCCMessage *)message {
    ...
    if ([[WFCCNetworkService sharedInstance].userId isEqualToString:self.creator]) {
        return [NSString stringWithFormat:@"%@\"%@\"", (isChinese?@"你创建了群":@"Bạn tạo group "), self.groupName];
    } else {
        ...
        if (userInfo.friendAlias.length > 0) {
            return [NSString stringWithFormat:@"%@%@\"%@\"", userInfo.friendAlias, (isChinese?@"创建了群":@" tạo group "), self.groupName];
        } else if(userInfo.groupAlias.length > 0) {
            return [NSString stringWithFormat:@"%@%@\"%@\"", userInfo.groupAlias, (isChinese?@"创建了群":@" tạo group "), self.groupName];
        } else if (userInfo.displayName.length > 0) {
            return [NSString stringWithFormat:@"%@%@\"%@\"", userInfo.displayName, (isChinese?@"创建了群":@" tạo group "), self.groupName];
        } else {
            if (isChinese) {
                return [NSString stringWithFormat:@"用户<%@>创建了群\"%@\"", self.creator, self.groupName];
            }else {
                return [NSString stringWithFormat:@"<%@> tạo group \"%@\"", self.creator, self.groupName];
            }
        }
    }
}
