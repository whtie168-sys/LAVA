...
#import "Chat86AVEngineKit_Import.h"
...
WFCCUserInfo* convertUserInfo(const mars::stn::TUserInfo &tui) {
    ...
    if (tui.deleted) {
        userInfo.displayName = @"Người dùng đã xóa";
    } else {
...
- (void)suspend {
  if(_bgTaskId != UIBackgroundTaskInvalid) {
      ...
      BOOL inCall = NO;
      Class cls = NSClassFromString(@"Chat86AVEngineKit");
...
- (BOOL)checkSDKHost:(NSString *)host {
    if(NSClassFromString(@"Chat86AVEngineKit")) {
        Chat86AVEngineKit *avEngineKit = [NSClassFromString(@"Chat86AVEngineKit") performSelector:@selector(sharedEngineKit)];
