...
//    }
    ...
    //当群主在群内撤回消息时，无需提示XX撤回了消息
    void onSuccess() {
        // 取群信息（message.conversation.target 是群 ID）
        WFCCGroupInfo *groupInfo = [[WFCCIMService sharedWFCIMService] getGroupInfo:message.conversation.target refresh:NO];
        
        // 当前撤回操作者 ID
        NSString *operatorId = [WFCCNetworkService sharedInstance].userId;
        
        BOOL isGroupOwner = groupInfo && [groupInfo.owner isEqualToString:operatorId];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (isGroupOwner) {
                ...
                recallCnt.operatorId = operatorId;
                recallCnt.messageUid = message.messageUid;
                recallCnt.originalSender = message.fromUser;
                recallCnt.originalContent = orignalPayload.content;
                recallCnt.originalSearchableContent = orignalPayload.searchableContent;
...
- (NSArray<WFCCConversationInfo *> *)getConversationInfos:(NSArray<NSNumber *> *)conversationTypes lines:(NSArray<NSNumber *> *)lines{
    ...
    for (std::list<mars::stn::TConversation>::iterator it = convers.begin(); it != convers.end(); it++) {
        ...
        if ([info.conversation.target isEqualToString:@"admin"]) {
            continue;
        }
//        if ([info.conversation.target isEqualToString:@"FireRobot"]) {
//            continue;
//        }
        [ret addObject:info];
    }
...
WFCCGroupInfo *convertProtoGroupInfo(const mars::stn::TGroupInfo &tgi) {
    ...
    groupInfo.memberCount = tgi.memberCount; // 0113 注释 群成员数量
    ...
    if(!groupInfo.portrait.length && [WFCCNetworkService sharedInstance].defaultPortraitProvider && [[WFCCNetworkService sharedInstance].defaultPortraitProvider respondsToSelector:@selector(groupDefaultPortrait:memberInfos:)]) {
        ...
        [[[WFCCIMService sharedWFCIMService] getGroupMembers:groupInfo.target count:9] enumerateObjectsUsingBlock:^(WFCCGroupMember * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([obj.memberId isEqualToString:@"FireRobot"] || [obj.memberId isEqualToString:@"wfc_file_transfer"]) {
                // 该判断0113新增
            }else {
        ...
        groupInfo.memberCount = memberUserInfos.count;
        if(!pendding) {
