...
static QWERMediaMessageDownloader *sharedSingleton = nil;

@interface QWERMediaMessageDownloader()
@property(nonatomic, strong)NSMutableDictionary<NSString*, NSNumber*> *downloadingMessages;
@end



@implementation QWERMediaMessageDownloader
+ (instancetype)sharedDownloader {
    if (sharedSingleton == nil) {
        @synchronized (self) {
            if (sharedSingleton == nil) {
                sharedSingleton = [[QWERMediaMessageDownloader alloc] init];
...
- (BOOL)tryDownload:(WFCCMessage *)msg
    ...
    if (mediaContent.localPath.length && [QWERUtilities isFileExist:mediaContent.localPath]) {
    ...
    NSString *cacheDir = [[QWERConfigManager globalManager] cachePathOf:msg.conversation mediaType:payload.mediaType];
    ...
    savedPath = [QWERUtilities getUnduplicatedPath:savedPath];
    ...
    [self downloadFileWithURL:mediaContent.remoteUrl parameters:nil savedPath:savedPath downloadSuccess:^(NSURLResponse *response, NSURL *filePath) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [[QWERMediaMessageDownloader sharedDownloader].downloadingMessages removeObjectForKey:mediaContent.remoteUrl];
...
- (BOOL)tryDownload:(NSString *)mediaPath
    ...
    if(msg) {
        WFCCMediaMessagePayload *payload = (WFCCMediaMessagePayload *)[msg.content encode];
        cacheDir = [[QWERConfigManager globalManager] cachePathOf:msg.conversation mediaType:payload.mediaType];
    ...
    [self downloadFileWithURL:mediaPath parameters:nil savedPath:savedPath downloadSuccess:^(NSURLResponse *response, NSURL *filePath) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [[QWERMediaMessageDownloader sharedDownloader].downloadingMessages removeObjectForKey:mediaPath];
