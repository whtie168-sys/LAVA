...
@implementation QWERUtilities
+ (instancetype)main { // 0308新增
    static dispatch_once_t once;
    static QWERUtilities *instance;
    dispatch_once(&once, ^{
        instance = [[QWERUtilities alloc] init];
    });
    return instance;
}
- (BOOL)isChinese {
...
+ (NSString *)formatTimeLabel:(int64_t)timestamp {
    ...
    BOOL isChinese = [QWERUtilities.main isChinese];
    ...
    if ([calendar isDateInToday:date]) {
        ...
        if (years == curYears) {
            ...
            if (weeks == curWeeks) {
                switch (weekDays) {
                    case 1:
                        return (isChinese?@"周日":@"Chủ nhật");
                        break;
                    case 2:
                        return (isChinese?@"周一":@"Thứ 2");
                        break;
                    case 3:
                        return (isChinese?@"周二":@"Thứ 3");
                        break;
                    case 4:
                        return (isChinese?@"周三":@"Thứ 4");
                        break;
                    case 5:
                        return (isChinese?@"周四":@"Thứ 5");
                        break;
                    case 6:
                        return (isChinese?@"周五":@"Thứ 6");
                        break;
                    case 7:
                        return (isChinese?@"周六":@"Thứ 7");
                        break;
                        
                    default:
                        break;
                }
                return [NSString stringWithFormat:@"%@%ld",(isChinese?@"周":@"Tuần "), weekDays];
            } else {
...
+ (NSString *)formatTimeDetailLabel:(int64_t)timestamp {
    ...
    if ([calendar isDateInToday:date]) {
        ...
        return [NSString stringWithFormat:@"%@ %@",(isChinese?@"昨天":@"Hôm qua"), hourTimeStr];
        ...
        if(weeks == curWeeks) {
            return [NSString stringWithFormat:@"%@ %@", [QWERUtilities formatWeek:weekDays], hourTimeStr];
        }
        if (isChinese) {
...
+ (UIImage *)imageForExt:(NSString *)extName {
    ...
    return [QWERImage imageNamed:fileImage];
}
...
+ (CGFloat)wf_navigationFullHeight {
    return [QWERUtilities wf_statusBarHeight] + 44;
}
