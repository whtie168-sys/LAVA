...
@interface AUETAChannelProfileVC () <UIActionSheetDelegate>
...
@implementation AUETAChannelProfileVC
...
- (void)viewDidLoad {
    ...
    CGFloat top = [QWERUtilities wf_navigationFullHeight] + 40;
    ...
    if(![[WFCCNetworkService sharedInstance].userId isEqualToString:self.channelInfo.owner]) {
        ...
        if ([[WFCCIMService sharedWFCIMService] isListenedChannel:self.channelInfo.channelId]) {
            btn = [[UIButton alloc] initWithFrame:CGRectMake(20, self.view.frame.size.height - [QWERUtilities wf_safeDistanceBottom] - 40 - 16, screenWidth - 40, 40)];
