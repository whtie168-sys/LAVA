...
#import "FRSDAUserSectionKeySupport.h"
#import "UIFont+YH.h"
#import "UIColor+YH.h"
#import "UIImage+ERCategory.h"
#import "QWERConfigManager.h"
#import "FRSDASeletedUserSearchResultVC.h"
...
@interface FRSDASeletedUserVC () <UITableViewDataSource, UITableViewDelegate,
UICollectionViewDataSource, UICollectionViewDelegate,
UISearchBarDelegate, FRSDASelectedUserTVCellDelegate>
...
@property(nonatomic, strong)NSMutableArray<ESZQSCOrganization *> *organizations;
@property(nonatomic, strong)NSMutableArray<ESZQSCEmployee *> *employees;

@property(nonatomic, strong)NSMutableArray<ESZQSCOrganizationEx *> *paths;
...
@implementation FRSDASeletedUserVC
...
- (void)viewDidLoad {
    ...
    if(!self.disabledUserNotSelected) {
        for (NSString *defaultUserId in self.disableUserIds) {
            FRSDASelectModel *defaultUser = [[FRSDASelectModel alloc] init];
...
- (void)updateNavi {
    if(self.organizationIds.count) {
        UIBarButtonItem *back = [[UIBarButtonItem alloc] initWithImage:[QWERImage imageNamed:@"back"] style:UIBarButtonItemStylePlain target:self action:@selector(onBackBtn:)];
        UIBarButtonItem *close = [[UIBarButtonItem alloc] initWithImage:[QWERImage imageNamed:@"close"] style:UIBarButtonItemStylePlain target:self action:@selector(cancel)];
...
#pragma mark - FRSDASelectedUserTVCellDelegate
- (void)didTapNextLevel:(FRSDASelectModel *)model {
    if(!self.organizationIds.count) {
    ...
    ESZQSCOrganizationEx *ex = [[ESZQSCOrganizationEx alloc] init];
...
- (BOOL)searchBarShouldBeginEditing:(UISearchBar *)searchBar {
    FRSDASeletedUserSearchResultVC *resultVC = [[FRSDASeletedUserSearchResultVC alloc] init];
    ...
    resultVC.selectedUserBlock = ^(FRSDASelectModel * _Nonnull user) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    FRSDASelectedUserTVCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell"];
    cell.delegate = self;
    ...
    if (self.type == Vertical) {
        ...
        cell.asoucNameLabel.textColor = [UIColor whiteColor];
        cell.asoucNameLabel.textColor = [UIColor whiteColor];
    } else {
        cell.separatorInset = UIEdgeInsetsMake(0, 16, 0, 16);
        cell.backgroundColor = [UIColor whiteColor];
        cell.asoucNameLabel.textColor = [UIColor colorWithHexString:@"0x1d1d1d"];
    }
    return cell;
}
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    FRSDASelectedUserCVCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"selectedUserC" forIndexPath:indexPath];
...
- (void)loadData {
    ...
    if(self.organizationIds.count) {
        ...
        [[ESZQSCOrganizationCache sharedCache] getOrganizationEx:orgId refresh:NO success:^(NSInteger organizationId, ESZQSCOrganizationEx * _Nonnull ex) {
            [hud hideAnimated:NO];
            if(organizationId == [ws.organizationIds.lastObject integerValue]) {
                ESZQSCOrganizationEx *p = [ws.paths lastObject];
        ...
        for (WFCCUserInfo *userInfo in userDataSource) {
            __block FRSDASelectModel *info = [[FRSDASelectModel alloc] init];
            info.userInfo = userInfo;
            if ([self.disableUserIds containsObject:info.userInfo.userId]) {
                info.selectedStatus = Disable_Checked;
            }
            [self.selectedUsers enumerateObjectsUsingBlock:^(FRSDASelectModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        NSMutableArray<NSNumber *> *ids = [[[ESZQSCOrganizationCache sharedCache] rootOrganizationIds] mutableCopy];
        [ids addObjectsFromArray:[ESZQSCOrganizationCache sharedCache].bottomOrganizationIds];
        if(ids.count) {
            ...
            [ids enumerateObjectsUsingBlock:^(NSNumber * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                ESZQSCOrganization *org = [[ESZQSCOrganizationCache sharedCache] getOrganization:[obj integerValue] refresh:NO];
                if(org) {
                    [orgs addObject:org];
                }
            }];
...
- (void)setUpUI {
    ...
    if (self.type == Vertical) {
        ...
        self.tableView.backgroundColor = [UIColor colorWithHexString:@"0x1f2026"]; // 1212
...
//        }
        ...
        self.doneButton.frame = CGRectMake(0, 0, 60, 30);
        [self setDoneButtonStyleAndContent:NO];
        self.doneButton.backgroundColor = [UIColor colorWithHexString:@"0x17B1FE"];
        [self.doneButton setTitle:WFCString(@"Done") forState:UIControlStateNormal];
        self.doneButton.titleLabel.font = [UIFont pingFangSCWithWeight:FontWeightStyleMedium size:14];
...
- (void)setDoneButtonStyleAndContent:(BOOL)enable {
    if (enable) {
        ...
        if (self.type == Horizontal) {
            [self.doneButton setTitle:[NSString stringWithFormat:@"Hoàn tất(%lu)", (unsigned long)self.selectedUsers.count] forState:UIControlStateNormal];
            [self.doneButton sizeToFit];
            self.doneButton.frame = CGRectMake(0, 0, self.doneButton.frame.size.width + 8 * 2, self.doneButton.frame.size.height);
        } else {
            [self.doneButton setTitle:[NSString stringWithFormat:@"Hoàn tất(%lu/%d)", (unsigned long)self.selectedUsers.count, self.maxSelectCount] forState:UIControlStateNormal];
                    [self.doneButton sizeToFit];
                    self.doneButton.frame = CGRectMake(0, 0, self.doneButton.frame.size.width + 8 * 2, self.doneButton.frame.size.height);
        }
...
//    [[QWERConfigManager globalManager] setupNavBar];
    ...
    for (FRSDASelectModel *user in self.selectedUsers) {
    ...
    if(orgIds.count) {
        ...
        [[QWERConfigManager globalManager].orgServiceProvider getBatchOrgEmployees:orgIds success:^(NSArray<NSString *> * _Nonnull employeeIds) {
            [hud hideAnimated:NO];
...
- (void)mergeOrgAndEmps {
    ...
    ESZQSCOrganizationEx *ex = [self.paths lastObject];
    if(ex.employees.count) {
        NSMutableArray *emps = [[NSMutableArray alloc] init];
        [ex.employees enumerateObjectsUsingBlock:^(ESZQSCEmployee * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            FRSDASelectModel *model = [[FRSDASelectModel alloc] init];
            model.employee = obj;
            model.selectedStatus = Unchecked;
            [self.selectedUsers enumerateObjectsUsingBlock:^(FRSDASelectModel * _Nonnull obj1, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    if(ex.subOrganizations.count) {
        NSMutableArray *orgs = [[NSMutableArray alloc] init];
        [ex.subOrganizations enumerateObjectsUsingBlock:^(ESZQSCOrganization * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            FRSDASelectModel *model = [[FRSDASelectModel alloc] init];
...
- (void)sortAndRefreshWithList:(NSArray *)friendList {
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        NSMutableDictionary *resultDic = [FRSDAUserSectionKeySupport userSectionKeys:friendList];
...
- (BOOL)toggelSeletedUser:(FRSDASelectModel *)user {
...
- (void)reloadCellForUser:(FRSDASelectModel *)user {
    for (NSString *key in self.sectionKeys) {
        NSArray *users = self.sectionDictionary[key];
        for (FRSDASelectModel *u in users) {
            if ([u isEqual:user]) {
...
- (UICollectionView *)selectedUserCollectionView {
    if (!_selectedUserCollectionView) {
        ...
        [_selectedUserCollectionView registerClass:[FRSDASelectedUserCVCell class] forCellWithReuseIdentifier:@"selectedUserC"];
...
//        _tableView.sectionIndexColor = [UIColor colorWithHexString:@"0x4e4e4e"];
        ...
        [_tableView registerClass:[FRSDASelectedUserTVCell class] forCellReuseIdentifier:@"cell"];
...
- (UIView *)topView {
    if (!_topView) {
        _topView = [UIView new];
        if (self.type == Horizontal) {
            _topView.backgroundColor = [QWERConfigManager globalManager].naviBackgroudColor;
