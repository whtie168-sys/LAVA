//
//  FRSDASeletedUserSearchResultVC.m
//  WFChatUIKit
//
...
@interface FRSDASeletedUserSearchResultVC ()<UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate>
@property (nonatomic, strong)UISearchBar *searchBar;
@property (nonatomic, strong)NSArray *results;
@end

@implementation FRSDASeletedUserSearchResultVC
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchString {
    if(self.organizationId) {
        if(searchString.length) {
            ...
            [[QWERConfigManager globalManager].orgServiceProvider searchEmployee:self.organizationId keyword:searchString success:^(NSArray<ESZQSCEmployee *> * _Nonnull employees) {
                NSMutableArray *arr = [[NSMutableArray alloc] init];
                [employees enumerateObjectsUsingBlock:^(ESZQSCEmployee * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    __block FRSDASelectModel *model = [[FRSDASelectModel alloc] init];
                    model.selectedStatus = Unchecked;
                    model.employee = obj;
                    [ws.selectedUsers enumerateObjectsUsingBlock:^(FRSDASelectModel * _Nonnull obj2, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        NSMutableArray <FRSDASelectModel *>*searchList = [NSMutableArray new];
        QOEUAPinyinUtility *pu = [[QOEUAPinyinUtility alloc] init];
        BOOL isChinese = [pu isChinese:searchString];
        for (FRSDASelectModel *friend in self.dataSource) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    FRSDASelectedUserTVCell *cell = [tableView dequeueReusableCellWithIdentifier:@"selectedUserT"];
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        [_tableView registerClass:[FRSDASelectedUserTVCell class] forCellReuseIdentifier:@"selectedUserT"];
