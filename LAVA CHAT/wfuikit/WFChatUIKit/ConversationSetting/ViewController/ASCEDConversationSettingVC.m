...
#import "ASCEDConversationSettingMemberCVLayout.h"
...
#import "DSRACSwitchTVCell.h"
#import "REDSACreateGroupVC.h"
#import "RWADCProfileTableVC.h"
#import "ASCEDGroupManageTableVC.h"
#import "ASCEDGroupMemberCollectionVC.h"
#import "ASEDCFilesVC.h"
#import "FRSDASeletedUserVC.h"
#import "MBProgressHUD.h"
#import "QWERTMyProfileTVC.h"
...
@interface ASCEDConversationSettingVC () <UITableViewDataSource, UITableViewDelegate, UICollectionViewDelegate, UICollectionViewDataSource>
@property (nonatomic, strong)UICollectionView *memberCollectionView;
@property (nonatomic, strong)ASCEDConversationSettingMemberCVLayout *memberCollectionViewLayout;
...
@implementation ASCEDConversationSettingVC

- (void)viewDidLoad {
    [super viewDidLoad];
    if (self.conversation.type == Channel_Type) {
        self.navigationItem.title = @"频道详情";
    } else {
        self.navigationItem.title = @"会话详情";
    }
    ...
    footerView.backgroundColor = [QWERConfigManager globalManager].backgroudColor;
//    footerView.backgroundColor = UIColor.whiteColor;
    ...
    if(self.conversation.type == Group_Type) {
        ...
        [self.channelPortrait sd_setImageWithURL:[NSURL URLWithString:[self.channelInfo.portrait stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]] placeholderImage:[QWERImage imageNamed:@"channel_default_portrait"]];
...
- (void)setupMemberCollectionView {
    if (self.conversation.type == Single_Type || self.conversation.type == Group_Type) {
        self.memberCollectionViewLayout = [[ASCEDConversationSettingMemberCVLayout alloc] initWithItemMargin:9];
...
//        self.memberCollectionViewLayout.itemSize = CGSizeMake(self.memberCollectionViewLayout.itemSize.width, 86.0);
...
//        self.memberCollectionView.backgroundColor = [QWERConfigManager globalManager].backgroudColor;
        self.memberCollectionView.backgroundColor = UIColor.whiteColor;
        [self.memberCollectionView registerClass:[DSRACConversationSettingMemberCell class] forCellWithReuseIdentifier:Group_Member_Cell_Reuese_ID];
        ...
        if (self.showMoreMember) {
            ...
            head.backgroundColor = [QWERConfigManager globalManager].backgroudColor;
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    if ([self isGroupAnnouncementCell:indexPath]) {
        float height = [QWERUtilities getTextDrawingSize:self.groupAnnouncement.text font:[UIFont pingFangSCWithWeight:FontWeightStyleRegular size:12] constrainedSize:CGSizeMake(self.view.bounds.size.width - 48, 1000)].height;
        if (height > 12 * 3.2) {
            height = 12 * 3.2;
        }
        return height + 50;
...
- (UITableViewCell *)cellOfTable:(UITableView *)tableView WithTitle:(NSString *)title withDetailTitle:(NSString *)detailTitle withDisclosureIndicator:(BOOL)withDI withSwitch:(BOOL)withSwitch withSwitchType:(SwitchType)type {
    if (withSwitch) {
        DSRACSwitchTVCell *cell = [tableView dequeueReusableCellWithIdentifier:@"styleSwitch"];
        if(cell == nil) {
            cell = [[DSRACSwitchTVCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:@"styleSwitch" conversation:self.conversation];
        }
        cell.textLabel.font = [UIFont pingFangSCWithWeight:FontWeightStyleRegular size:16];
        cell.textLabel.textColor = [QWERConfigManager globalManager].textColor;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath  {
  if ([self isGroupNameCell:indexPath]) {
      ...
      if (cell == nil) {
          ...
          UIImage *qrcode = [QWERImage imageNamed:@"qrcode"];
...
//          return;
//      }
//    REDSACreateGroupVC *vc = [[REDSACreateGroupVC alloc] init];
...
//    [self.navigationController pushViewController:vc animated:YES];
      ...
      ASCEDGroupManageTableVC *gmvc = [[ASCEDGroupManageTableVC alloc] init];
  ...
  } else if([self isGroupAnnouncementCell:indexPath]) { // 群公告点击事件
      TREWQGroupAnnouncementViewController *vc = [[TREWQGroupAnnouncementViewController alloc] init];
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    DSRACConversationSettingMemberCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:Group_Member_Cell_Reuese_ID forIndexPath:indexPath];
    if (indexPath.row < self.memberCollectionCount-self.extraBtnNumber) {
        ...
        if (indexPath.row == self.memberCollectionCount-self.extraBtnNumber) {
            [cell.headerImageView setImage:[UIImage imageNamed:@"coaeisgoxAdd"]]; //
            cell.asoucNameLabel.text = nil;
            cell.asoucNameLabel.hidden = YES;
        } else {
            [cell.headerImageView setImage:[UIImage imageNamed:@"coaeisgoxSub"]];
            cell.asoucNameLabel.text = nil;
            cell.asoucNameLabel.hidden = YES;
        }
...
//            QWERTMyProfileTVC *vc = [[QWERTMyProfileTVC alloc] init];
