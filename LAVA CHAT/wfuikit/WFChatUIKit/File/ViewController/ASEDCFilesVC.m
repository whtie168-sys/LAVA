...
@interface ASEDCFilesVC () <UITableViewDelegate, UITableViewDataSource, UISearchControllerDelegate, UISearchResultsUpdating>
{
    BOOL _isChinese;
}
...
- (void)viewDidLoad {
    ...
    if (@available(iOS 13, *)) {
        ...
        [self.searchController.searchBar setValue:(_isChinese?@"取消":@"Hủy") forKey:@"_cancelButtonText"];
    }
    ...
    self.searchController.searchBar.placeholder = (_isChinese?@"搜索":@"Tìm kiếm");
...
//    }
    ...
    if (self.myFiles) {
        self.title = (_isChinese?@"我的文件":@"Giấy tờ của tôi");
        ...
        if (user.friendAlias.length) {
            self.title = [NSString stringWithFormat:(_isChinese?@"%@ 的文件":@"%@ `s tài liệu"), user.friendAlias];
        } else if (user.displayName.length) {
            self.title = [NSString stringWithFormat:(_isChinese?@"%@ 的文件":@"%@ `s tài liệu"), user.displayName];
        } else {
            self.title = (_isChinese?@"文件":@"Tài liệu");
        }
    } else if(self.conversation) {
        self.title = (_isChinese?@"会话文件":@"Tập tin phiên chạy");
    } else {
        self.title = (_isChinese?@"所有文件":@"Tất cả tài liệu");
    }
...
//        line.backgroundColor = [UIColor colorWithRed:0.9 green:0.9 blue:0.9 alpha:1.f];
        ...
        label.text = (_isChinese?@"已经加载完了":@"Đã nạp xong rồi.");
...
- (nonnull UITableViewCell *)tableView:(nonnull UITableView *)tableView cellForRowAtIndexPath:(nonnull NSIndexPath *)indexPath {
    ASEDCFileRecordTVCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell"];
    if (!cell) {
        cell = [[ASEDCFileRecordTVCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:@"cell"];
    }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    return [ASEDCFileRecordTVCell sizeOfRecord:record withCellWidth:self.view.bounds.size.width];
}
