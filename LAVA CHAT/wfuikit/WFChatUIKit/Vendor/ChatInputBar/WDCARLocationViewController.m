...
@interface WDCARLocationViewController () <CLLocationManagerDelegate, UITableViewDelegate, UITableViewDataSource>
{
    BOOL _isChinese;
}
@property(nonatomic, strong) UIBarButtonItem *sendButton;
@property(nonatomic, strong) CLLocationManager *locationManager;
@property(nonatomic, strong) WDCARLocationPoint *locationPoint;
...
@implementation WDCARLocationViewController
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = (_isChinese?@"位置":@"Vị trí");
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:(_isChinese?@"取消":@"Hủy") style:UIBarButtonItemStylePlain target:self action:@selector(dismiss:)];
    ...
    if (_locationManager) {
        ...
        UIImage *image = [QWERImage imageNamed:@"PinGreen"];
        ...
        self.locationPoint   = [[WDCARLocationPoint alloc] init];
        ...
        if ([CLLocationManager locationServicesEnabled]) {
            ...
            if (status == kCLAuthorizationStatusRestricted || status == kCLAuthorizationStatusDenied) {
                [self.view makeToast:(_isChinese?@"请在设置-隐私里允许程序使用地理位置服务":@"Hãy cho phép chương trình sử dụng dịch vụ vị trí trong cài đặt - riêng tư")
                            duration:2
            ...
            [self.view makeToast:(_isChinese?@"请打开地理位置服务":@"Vui lòng mở dịch vụ địa lý")
                        duration:2
                        position:CSToastPositionCenter];
        }
...
- (void)setUpRightNavButton{
    UIBarButtonItem *item = [[UIBarButtonItem alloc] initWithTitle:(_isChinese?@"选定":@"Được chọn") style:UIBarButtonItemStyleDone target:self action:@selector(onSend:)];
...
- (void)reverseGeoLocation:(CLLocationCoordinate2D)locationCoordinate2D{
    ...
    [self.geoCoder reverseGeocodeLocation:location
         ...
         if (!error) {
             ...
             WDCARLocationPoint *ponit = [[WDCARLocationPoint alloc] initWithCoordinate:locationCoordinate2D andTitle:nil];
             [ws.mapView addAnnotation:ponit];
             ws.locationPoint = [[WDCARLocationPoint alloc] initWithCoordinate:locationCoordinate2D andTitle:title];;
             ...
             ws.title = (self->_isChinese?@"位置":@"Vị trí");
         }
     }];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (cell.selected) {
        cell.imageView.image = [QWERImage imageNamed:@"multi_selected"];
    } else {
