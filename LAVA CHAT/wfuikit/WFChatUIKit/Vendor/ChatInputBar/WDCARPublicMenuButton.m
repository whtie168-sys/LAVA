...
@interface WDCARPublicMenuButton ()
...
@implementation WDCARPublicMenuButton
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event{
    ...
    if (view == nil){
        for(WDCARPublicMenuButton *subMenuView in self.subMenuViews) {
        //转换坐标
...
- (void)setExpended:(BOOL)expended {
    ...
    if (expended) {
        ...
        for (int i = 0; i < self.channelMenu.subMenus.count; i++) {
            ...
            WDCARPublicMenuButton *menuBtn = [[WDCARPublicMenuButton alloc] initWithFrame:CGRectMake(0, i * parentRect.size.height + i - 1, parentRect.size.width+16, parentRect.size.height)];
...
- (void)setChannelMenu:(WFCCChannelMenu *)channelMenu isSubMenu:(BOOL)isSubMenu {
    ...
    if (!isSubMenu && channelMenu.subMenus.count) {
        [self setImage:[QWERImage imageNamed:@"sub_menu"] forState:UIControlStateNormal];
    }
    if (isSubMenu) {
