...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _sendBtn.frame = CGRectMake(self.frame.size.width - 62,self.bounds.size.height-[QWERUtilities wf_safeDistanceBottom] - 30,52, 37);
        [_sendBtn setTitle:(isChinese?@"发送":@"Send") forState:UIControlStateNormal];
        [_sendBtn setTitleColor:[QWERConfigManager globalManager].textColor forState:UIControlStateNormal];
        ...
        [back setImage:[QWERImage imageNamed:@"del_emoji_normal"] forState:UIControlStateNormal];
        [back setImage:[QWERImage imageNamed:@"del_emoji_select"] forState:UIControlStateSelected];
        [back addTarget:self action:@selector(backFace) forControlEvents:UIControlEventTouchUpInside];
        back.frame = CGRectMake( self.frame.size.width - 62 - 62, self.bounds.size.height-[QWERUtilities wf_safeDistanceBottom] - 30, 52, 37);
