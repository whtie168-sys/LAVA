...
- (void)viewDidLoad {
    ...
    [_pagingScrollView addGestureRecognizer:[[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(onLongPressedMedium:)]];
...
- (void)tilePages {
    ...
    for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
        if (![self isDisplayingPageForIndex:index]) {
            ...
            // Add play button if needed    视频中间的播放按钮的icon。 PlayButtonOverlayLarge
...
// 长按手势。 转发、保存图片、识别二维码
- (void)onLongPressedMedium:(id)sender {
...
- (void)popTypeChoose {
    ...
    //    UIAlertAction *forwardAct = [UIAlertAction actionWithTitle:@"转发" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
    //
    //    }];
    //    UIAlertAction *savePictureAct = [UIAlertAction actionWithTitle:isVideo ? @"保存视频" : @"保存图片" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
    //
    //    }];
    //    [actionSheet addAction:forwardAct];
    //    [actionSheet addAction:savePictureAct];
    
    if(isVideo) { // 视频
    }else { // 图片  是否存在可识别的二维码
        CIImage *detectImage = [CIImage imageWithData:UIImagePNGRepresentation(photo.underlyingImage)];
        CIDetector *detetor = [CIDetector detectorOfType:CIDetectorTypeQRCode context:nil options:@{CIDetectorAccuracy : CIDetectorAccuracyLow}];
        CIQRCodeFeature *feature = (CIQRCodeFeature *)[detetor featuresInImage:detectImage options:nil].firstObject;
        if (feature.messageString) { // 存在QR
            __weak typeof(self)ws = self;
            UIAlertAction *qRCodeAct = [UIAlertAction actionWithTitle:WFCString(@"IdentifyQRCode") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                [LBXScanNative recognizeImage:photo.underlyingImage success:^(NSArray<LBXScanResult *> *array) {
                    [ws scanResultWithArraMedium:array];
                }];
            }];
            [actionSheet addAction:qRCodeAct];
            
            
            UIAlertAction *cancelAct = [UIAlertAction actionWithTitle:WFCString(@"Cancel") style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
            }];
...
- (void)scanResultWithArraMedium:(NSArray<LBXScanResult*>*)array {
    if (array.count < 1) {
...
- (void)popAlertMsgWithScanResult:(NSString*)strResult {
    if (!strResult) {
        ...
        MWQrScanResultViewController *scanRvc = [[MWQrScanResultViewController alloc] init];
        scanRvc.hidesBottomBarWhenPushed = YES;
        scanRvc.urlStr = strResult;
        [self.navigationController pushViewController:scanRvc animated:YES];
    }
}
