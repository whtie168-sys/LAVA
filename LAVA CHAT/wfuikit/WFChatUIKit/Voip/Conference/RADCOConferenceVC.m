...
#import "RADCOParticipantCVLayout.h"
#import "FRSDASeletedUserVC.h"
#import "UIView+Toast.h"
#import "RADCOConferenceInviteVC.h"
#import "RADCOConferenceMemberManagerVC.h"
#import "RADCOConferenceCVLayout.h"
#import "RADCOConferenceManager.h"
#import "QWERImage.h"
...
#import "RADCOMoreBoardView.h"
#import "RADCOConferenceAudioCVCell.h"

@interface RADCOConferenceVC () <UITextFieldDelegate
    ,WFAVCallSessionDelegate
    ,UICollectionViewDataSource
    ,UICollectionViewDelegate
    ,RADCOConferenceManagerDelegate
...
@property (nonatomic, strong)RADCOConferenceParticipantCVCell *smallVideoView;
...
@property(nonatomic, strong)RADCOMoreBoardView *boardView;
...
#define TopViewHeigh ([QWERUtilities wf_statusBarHeight] + CONFERENCE_BAR_HEIGHT)
 ...
 */
@implementation RADCOConferenceVC
- (instancetype)initWithSession:(WFAVCallSession *)session conferenceInfo:(RADCOConferenceInfo *)conferenceInfo {
    ...
    if (self) {
        ...
        [RADCOConferenceManager sharedInstance].currentConferenceInfo = conferenceInfo;
        self.currentSession.autoSwitchVideoType = NO;
        self.currentSession.defaultVideoType = WFAVVideoType_None;
        [[RADCOConferenceManager sharedInstance] joinChatroom];
    }
    return self;
}
...
- (instancetype)initWithCallId:(NSString *_Nullable)callId
    ...
    if (self) {
        if (moCall) {
            self.currentSession = [[Chat86AVEngineKit sharedEngineKit] startConference:callId audioOnly:audioOnly pin:pin host:host title:title desc:desc callExtra:extra audience:audience advanced:advanced record:NO maxParticipants:maxParticipants sessionDelegate:self];
            
            [self didChangeState:kWFAVEngineStateOutgoing];
        } else {
            self.currentSession = [[Chat86AVEngineKit sharedEngineKit]
...
- (RADCOConferenceInfo *)conferenceInfo {
    return [RADCOConferenceManager sharedInstance].currentConferenceInfo;
}
...
- (void)rearrangeParticipants {
    ...
    NSString *focus = [RADCOConferenceManager sharedInstance].currentConferenceInfo.focus;
    ...
    [self.managerButton setTitle:[NSString stringWithFormat:@"%@(%ld)",(_isChinese?@"管理":@"Quản lý"), self.participants.count] forState:UIControlStateNormal];
    ...
    RADCOConferenceCVLayout *layout = (RADCOConferenceCVLayout *)self.participantCollectionView.collectionViewLayout;
...
- (void)viewDidLoad {
    ...
    RADCOConferenceCVLayout *layout = [[RADCOConferenceCVLayout alloc] init];
    self.scalingType = kWFAVVideoScalingTypeAspectFit;
    
    //    CGRect collectionRect = CGRectMake(0, TopViewHeigh, self.view.bounds.size.width, self.view.bounds.size.height - TopViewHeigh - [QWERUtilities wf_safeDistanceBottom] - CONFERENCE_BAR_HEIGHT);
    CGRect collectionRect = CGRectMake(0, [QWERUtilities wf_statusBarHeight], self.view.bounds.size.width, self.view.bounds.size.height- [QWERUtilities wf_statusBarHeight] - [QWERUtilities wf_safeDistanceBottom]);
    ...
    [self.participantCollectionView registerClass:[RADCOConferenceParticipantCVCell class] forCellWithReuseIdentifier:@"main"];
    [self.participantCollectionView registerClass:[RADCOConferenceParticipantCVCell class] forCellWithReuseIdentifier:@"sub"];
    [self.participantCollectionView registerClass:[RADCOConferenceAudioCVCell class] forCellWithReuseIdentifier:@"audio_cell"];
    ...
    self.pageControl = [[UIPageControl alloc]initWithFrame:CGRectMake(self.view.bounds.size.width/2-100, self.view.bounds.size.height - [QWERUtilities wf_safeDistanceBottom] - 20, 200, 20)];
    ...
    self.smallVideoView = [[RADCOConferenceParticipantCVCell alloc] initWithFrame:CGRectMake(8, CONFERENCE_BAR_HEIGHT, SmallVideoView, SmallVideoView * 4 /3)];
    ...
    [RADCOConferenceManager sharedInstance].delegate = self;
...
- (UIButton *)chatButton {
    if(!_chatButton) {
        ...
        [_chatButton setTitle:(_isChinese?@"说点什么...":@"Nói gì đi chứ......") forState:UIControlStateNormal];
...
- (UIButton *)rotateButton {
    if(!_rotateButton) {
        ...
        [_rotateButton setImage:[QWERImage imageNamed:@"rotate_screen"] forState:UIControlStateNormal];
...
- (UIButton *)lockButton {
    if(!_lockButton) {
        ...
        [_lockButton setImage:[QWERImage imageNamed:@"rotate_lock_off"] forState:UIControlStateNormal];
        [_lockButton setImage:[QWERImage imageNamed:@"rotate_lock_on"] forState:UIControlStateSelected];
...
- (UIButton *)floatingAudioButton {
    if(!_floatingAudioButton) {
        ...
        _floatingAudioButton = [[UIButton alloc] initWithFrame:CGRectMake((bound.size.width - FLOATING_AUDIO_BUTTON_SIZE)/2, bound.size.height - [QWERUtilities wf_safeDistanceBottom] - FLOATING_AUDIO_BUTTON_SIZE - CONFERENCE_BAR_HEIGHT, FLOATING_AUDIO_BUTTON_SIZE, FLOATING_AUDIO_BUTTON_SIZE)];
        [_floatingAudioButton setImage:[QWERImage imageNamed:@"conference_audio"] forState:UIControlStateNormal];
...
- (void)updateAudioVolume:(NSInteger)volume {
    ...
    [UIView animateWithDuration:0.2 animations:^{
        [ws.floatingAudioButton setImage:[QWERImage imageNamed:[NSString stringWithFormat:@"mic_%d", v]] forState:UIControlStateNormal];
    }];
}
...
- (UIButton *)createBarButtom:(NSString *)title imageName:(NSString *)imageName selectedImageName:(NSString *)selectedImageName select:(SEL)selector frame:(CGRect)frame {
    ...
    [btn setImage:[QWERImage imageNamed:imageName] forState:UIControlStateNormal];
...
//
    ...
    [btn setImage:[QWERImage imageNamed:selectedImageName] forState:UIControlStateHighlighted];
    [btn setImage:[QWERImage imageNamed:selectedImageName] forState:UIControlStateSelected];
...
- (void)updateTopBarViewFrame:(BOOL)landscape {
    CGFloat topPadding = landscape ? 0 : [QWERUtilities wf_statusBarHeight];
...
- (UIView *)topBarView {
    if(!_topBarView) {
        ...
        [_informationButton setImage:[QWERImage imageNamed:@"conference_information"] forState:UIControlStateNormal];
        ...
        [_hangupButton setImage:[QWERImage imageNamed:@"conference_end_call"] forState:UIControlStateNormal];
        ...
        [_speakerButton setImage:[QWERImage imageNamed:@"conference_speaker"] forState:UIControlStateNormal];
        [_speakerButton setImage:[QWERImage imageNamed:@"conference_speaker"] forState:UIControlStateHighlighted];
        [_speakerButton setImage:[QWERImage imageNamed:@"conference_speaker"] forState:UIControlStateSelected];
        ...
        [_switchCameraButton setImage:[QWERImage imageNamed:@"conference_switch_camera"] forState:UIControlStateNormal];
...
- (void)updatebottomBarViewFrame:(BOOL)landscape {
    CGFloat bottomPadding = landscape ? 0 : [QWERUtilities wf_safeDistanceBottom];
...
- (UIView *)bottomBarView {
    if(!_bottomBarView) {
        ...
        int index = 1;
        self.audioButton = [self createBarButtom:(_isChinese?@"静音":@"Tắt âm") imageName:@"conference_audio" selectedImageName:@"conference_audio" select:@selector(audioButtonDidTap:) frame:CGRectMake(0, 0, btnWidth, CONFERENCE_BAR_HEIGHT)];
        [_bottomBarView addSubview:self.audioButton];
        
        self.videoButton = [self createBarButtom:(_isChinese?@"视频":@"Video") imageName:@"conference_video" selectedImageName:@"conference_video" select:@selector(videoButtonDidTap:) frame:CGRectMake(btnWidth, 0, btnWidth, CONFERENCE_BAR_HEIGHT)];
        [_bottomBarView addSubview:self.videoButton];
        
        index++;
        if(!self.currentSession.isAudioOnly) {
            self.screenSharingButton = [self createBarButtom:(_isChinese?@"屏幕共享":@"Chia sẻ màn hình") imageName:@"conference_screen_sharing" selectedImageName:@"conference_screen_sharing_hover" select:@selector(screenSharingButtonDidTap:) frame:CGRectMake(btnWidth*index++, 0, btnWidth, CONFERENCE_BAR_HEIGHT)];
        ...
        self.managerButton = [self createBarButtom:[NSString stringWithFormat:@"%@(%ld)",(_isChinese?@"管理":@"Quản lý"), self.participants.count] imageName:@"conference_members" selectedImageName:@"conference_members" select:@selector(managerButtonDidTap:) frame:CGRectMake(btnWidth*index++, 0, btnWidth, CONFERENCE_BAR_HEIGHT)];
        [_bottomBarView addSubview:self.managerButton];
        
        self.moreButton = [self createBarButtom:(_isChinese?@"更多":@"Thêm") imageName:@"conference_more" selectedImageName:@"conference_more" select:@selector(moreButtonDidTap:) frame:CGRectMake(btnWidth*index++, 0, btnWidth, CONFERENCE_BAR_HEIGHT)];
...
- (void)resizeMessageTable {
    ...
    self.messageTableView.frame = CGRectMake(0, self.view.bounds.size.height - (landscape ? 0 :[QWERUtilities wf_safeDistanceBottom]) - CONFERENCE_BAR_HEIGHT - 16 - 20 - size.height, 200, size.height);
}
...
- (UIView *)inputContainer {
    if(!_inputContainer) {
        CGRect bound = self.view.bounds;
        _inputContainer = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height - [QWERUtilities wf_safeDistanceBottom] - CONFERENCE_BAR_HEIGHT - 28 - 68 + 24, bound.size.width, 40)];
...
- (void)moreButtonDidTap:(UIButton *)button {
    ...
    MoreItem *inviteItem = [[MoreItem alloc] initWithTitle:(_isChinese?@"邀请":@"Mời") image:[QWERImage imageNamed:@"conference_invite"] callback:^MoreItem * _Nonnull{
        RADCOConferenceInviteVC *pvc = [[RADCOConferenceInviteVC alloc] init];
        
        WFCCConferenceInviteMessageContent *invite = [[WFCCConferenceInviteMessageContent alloc] init];
        WFAVCallSession *currentSession = [Chat86AVEngineKit sharedEngineKit].currentSession;
    ...
    MoreItem *chatItem = [[MoreItem alloc] initWithTitle:(_isChinese?@"聊天":@"Trò chuyện") image:[QWERImage imageNamed:@"conference_chat"] callback:^MoreItem * _Nonnull{
    ...
    MoreItem *minimizeItem = [[MoreItem alloc] initWithTitle:(_isChinese?@"悬浮":@"Trang chủ") image:[QWERImage imageNamed:@"conference_minimize"] callback:^MoreItem * _Nonnull {
        [ws minimize];
        return nil;
    }];
    
    MoreItem *recordItem = [RADCOConferenceManager sharedInstance].currentConferenceInfo.recording ?
    [[MoreItem alloc] initWithTitle:(_isChinese?@"取消录制":@"Cancel record") image:[QWERImage imageNamed:@"conference_recording"] callback:^MoreItem * _Nonnull {
        if(![ws.conferenceInfo.owner isEqualToString:[WFCCNetworkService sharedInstance].userId]) {
            [ws showCommandToast:(self->_isChinese?@"请联系主持人取消录制":@"Please contact the host to cancel the recording")];
            return nil;
        }
        [[RADCOConferenceManager sharedInstance] requestRecording:NO];
        return nil;
    }]
    :
    [[MoreItem alloc] initWithTitle:(_isChinese?@"录制":@"Ghi âm") image:[QWERImage imageNamed:@"conference_record"] callback:^MoreItem * _Nonnull {
        if(![ws.conferenceInfo.owner isEqualToString:[WFCCNetworkService sharedInstance].userId]) {
            [ws showCommandToast:(self->_isChinese?@"请联系主持人录制":@"Please contact the host to record")];
            return nil;
        }
        [[RADCOConferenceManager sharedInstance] requestRecording:YES];
        return nil;
    }];
    
    MoreItem *settingItem = [[MoreItem alloc] initWithTitle:(_isChinese?@"设置":@"Thiết lập") image:[QWERImage imageNamed:@"conference_setting"] callback:^MoreItem * _Nonnull {
        return nil;
    }];
    
    MoreItem *handupItem = [RADCOConferenceManager sharedInstance].isHandup ?
    [[MoreItem alloc] initWithTitle:(_isChinese?@"方向":@"Hướng dẫn") image:[QWERImage imageNamed:@"conference_handup_hover"] callback:^MoreItem * _Nonnull{
        [[RADCOConferenceManager sharedInstance] handup:NO];
        [ws showCommandToast:(self->_isChinese?@"已放下举手":@"Put one's hand down")];
        return nil;
    }]
    :
    [[MoreItem alloc] initWithTitle:(_isChinese?@"举手":@"Giơ tay lên") image:[QWERImage imageNamed:@"conference_handup"] callback:^MoreItem * _Nonnull{
        [[RADCOConferenceManager sharedInstance] handup:YES];
        [ws showCommandToast:(self->_isChinese?@"已举手，等待管理员处理":@"Hands are raised, waiting for the administrator to process")];
        return nil;
    }];
    ...
    self.boardView = [[RADCOMoreBoardView alloc] initWithWidth:self.view.bounds.size.width
                                                        items:boardItems
                                                       cancel:^(RADCOMoreBoardView * _Nonnull boardView) {
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        frame.origin.y = ws.view.bounds.size.height - [QWERUtilities wf_safeDistanceBottom] - boardFrame.size.height;
...
- (void)hanupButtonDidTap:(UIButton *)button {
    if([self.currentSession.host isEqualToString:[WFCCNetworkService sharedInstance].userId]) {
        __weak typeof(self)ws = self;
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:nil message:(_isChinese?@"如果您想让与会人员继续开会，请选择退出会议":@"If you want participants to continue the meeting, opt out of the meeting") preferredStyle:UIAlertControllerStyleAlert];
        
        UIAlertAction *action1 = [UIAlertAction actionWithTitle:(_isChinese?@"退出会议":@"Quit") style:UIAlertActionStyleCancel handler:^(UIAlertAction *action) {
            [[RADCOConferenceManager sharedInstance] leaveConference:NO];
        }];
        [alertController addAction:action1];
        
        UIAlertAction *action2 = [UIAlertAction actionWithTitle:(_isChinese?@"结束会议":@"Destroy conference") style:UIAlertActionStyleDestructive handler:^(UIAlertAction *action) {
            [[RADCOConferenceManager sharedInstance] leaveConference:YES];
        }];
...
- (void)managerButtonDidTap:(UIButton *)button {
    RADCOConferenceMemberManagerVC *vc = [[RADCOConferenceMemberManagerVC alloc] init];
...
- (void)screenSharingButtonDidTap:(UIButton *)button {
    [[RADCOConferenceManager sharedInstance] switchAudioAndScreansharing:self.view];
}

- (void)updateScreenSharingButton {
    self.screenSharingButton.selected = self.currentSession.isBroadcasting;
    if([Chat86AVEngineKit sharedEngineKit].screenSharingReplaceMode) {
...
- (void)minimize {
    ...
    [RADCOFloatingWindow startCallFloatingWindow:self.currentSession focusUser:focusUser withTouchedBlock:^(WFAVCallSession *callSession, RADCOConferenceInfo *conferenceInfo) {
        RADCOConferenceVC *vc = [[RADCOConferenceVC alloc] initWithSession:callSession conferenceInfo:conferenceInfo];
        vc.focusUserProfile = focusUser;
        [[Chat86AVEngineKit sharedEngineKit] presentViewController:vc];
     }];
...
- (void)audioButtonDidTap:(UIButton *)button {
    if (self.currentSession.state != kWFAVEngineStateIdle) {
        if(!self.currentSession.isAudience && !self.currentSession.audioMuted) {
            [[RADCOConferenceManager sharedInstance] muteAudio:!(self.currentSession.audioMuted || self.currentSession.audience)];
            [self updateAudioButton];
        } else {
            if (self.conferenceInfo.allowTurnOnMic || [self.conferenceInfo.owner isEqualToString:[WFCCNetworkService sharedInstance].userId]) {
                ...
                __weak typeof(self)ws = self;
                if([RADCOConferenceManager sharedInstance].isApplyingUnmute) {
                    [[RADCOConferenceManager sharedInstance] presentCommandAlertView:self message:(_isChinese?@"您正在申请解除静音":@"You are requesting to unmute") actionTitle:(_isChinese?@"继续申请":@"Continue to apply") cancelTitle:(_isChinese?@"取消申请":@"Cancel application") contentText:(_isChinese?@"主持人不允许解除静音，您已经申请解除静音，正在等待主持人操作":@"The moderator does not allow unmuting. You have requested unmuting and are waiting for the moderator to do so") checkBox:NO actionHandler:^(BOOL checked) {
                        [[RADCOConferenceManager sharedInstance] applyUnmute:NO];
                        [ws showCommandToast:(self->_isChinese?@"已重新发送申请，请耐心等待主持人操作":@"The application has been sent again. Please wait for the moderator to perform the operation")];
                    } cancelHandler:^{
                        [[RADCOConferenceManager sharedInstance] applyUnmute:YES];
                        [ws showCommandToast:(self->_isChinese?@"已取消申请":@"Cancelled application")];
                    }];
                } else {
                    [[RADCOConferenceManager sharedInstance] presentCommandAlertView:self message:(_isChinese?@"你已静音":@"You have been muted") actionTitle:(_isChinese?@"申请解除静音":@"Request unmuting") cancelTitle:nil contentText:(_isChinese?@"主持人不允许解除静音，您可以向主持人申请解除静音":@"The moderator is not allowed to unmute the phone. You can ask the moderator to unmute the phone") checkBox:NO actionHandler:^(BOOL checked) {
                        [[RADCOConferenceManager sharedInstance] applyUnmute:NO];
                        [ws showCommandToast:(self->_isChinese?@"已发送申请，请耐心等待主持人操作":@"The application has been sent. Please wait for the moderator to operate")];
                    } cancelHandler:nil];
                }
            }
        }
...
- (void)updateAudioButton {
    if (self.currentSession.audioMuted || self.currentSession.isAudience) {
        [self.audioButton setImage:[QWERImage imageNamed:@"conference_audio_mute"] forState:UIControlStateNormal];
        [self.floatingAudioButton setImage:[QWERImage imageNamed:@"conference_audio_mute"] forState:UIControlStateNormal];
    } else {
        [self.audioButton setImage:[QWERImage imageNamed:@"conference_audio"] forState:UIControlStateNormal];
        [self.floatingAudioButton setImage:[QWERImage imageNamed:@"conference_audio"] forState:UIControlStateNormal];
    }
}
...
- (void)updateSpeakerButton {
    if (!self.currentSession.isSpeaker) {
        if([self.currentSession isHeadsetPluggedIn]) {
            [self.speakerButton setImage:[QWERImage imageNamed:@"conference_speaker_headset"] forState:UIControlStateNormal];
        } else if([self.currentSession isBluetoothSpeaker]) {
            [self.speakerButton setImage:[QWERImage imageNamed:@"conference_speaker_bluetooth"] forState:UIControlStateNormal];
        } else {
            [self.speakerButton setImage:[QWERImage imageNamed:@"conference_speaker_hover"] forState:UIControlStateNormal];
        }
    } else {
        [self.speakerButton setImage:[QWERImage imageNamed:@"conference_speaker"] forState:UIControlStateNormal];
    }
}

- (void)updateVideoButton {
    if (self.currentSession.videoMuted || self.currentSession.isAudience) {
        [self.videoButton setImage:[QWERImage imageNamed:@"conference_video_mute"] forState:UIControlStateNormal];
        self.switchCameraButton.hidden = YES;
    } else {
        [self.videoButton setImage:[QWERImage imageNamed:@"conference_video"] forState:UIControlStateNormal];
...
- (void)updateUIByOrientationChanged:(BOOL)landscape {
    ...
    if(landscape) {
        ...
        self.participantCollectionView.frame = CGRectMake(0, [QWERUtilities wf_statusBarHeight], self.view.bounds.size.width, self.view.bounds.size.height- [QWERUtilities wf_statusBarHeight] - [QWERUtilities wf_safeDistanceBottom]);
    }
    
    self.floatingAudioButton.frame = CGRectMake((bounds.size.width - FLOATING_AUDIO_BUTTON_SIZE)/2, bounds.size.height - (landscape ? 0 : [QWERUtilities wf_safeDistanceBottom]) - FLOATING_AUDIO_BUTTON_SIZE - CONFERENCE_BAR_HEIGHT, FLOATING_AUDIO_BUTTON_SIZE, FLOATING_AUDIO_BUTTON_SIZE);
    ...
    [self resizeMessageTable];
    self.chatButton.frame = CGRectMake(8, bounds.size.height - (landscape ? 0 : [QWERUtilities wf_safeDistanceBottom]) - CONFERENCE_BAR_HEIGHT - 16 - 20, 80, 20);
    self.rotateButton.frame = CGRectMake(bounds.size.width - 30 - 8, bounds.size.height - (landscape ? 0 : [QWERUtilities wf_safeDistanceBottom]) - CONFERENCE_BAR_HEIGHT - 16 - 30 + 3, 30, 30);
    self.lockButton.frame = CGRectMake(bounds.size.width - 30 - 8 - 30 - 8, bounds.size.height - (landscape ? 0 : [QWERUtilities wf_safeDistanceBottom]) - CONFERENCE_BAR_HEIGHT - 16 - 30 + 3, 30, 30);
    if(landscape) {
        [self.inputTextField resignFirstResponder];
    }
    [self.boardView removeFromSuperview];
    self.pageControl.frame = CGRectMake(self.view.bounds.size.width/2-100, self.view.bounds.size.height - (landscape ? 0 : [QWERUtilities wf_safeDistanceBottom]) - 20, 200, 20);
}
...
- (void)videoButtonDidTap:(UIButton *)button {
    if (self.currentSession.state != kWFAVEngineStateIdle) {
        ...
        [[RADCOConferenceManager sharedInstance] muteVideo:!(self.currentSession.audience || self.currentSession.videoMuted)];
        [self updateVideoButton];
        [self startHidePanelTimer];
    }
}
...
- (void)showConferenceInfoView {
    ...
    numberTitle.text = (_isChinese?@"会议号":@"Conference number");
    ...
    [numberCopyBtn setImage:[QWERImage imageNamed:@"copy"] forState:UIControlStateNormal];
    ...
    if(self.conferenceInfo.password.length) {
        ...
        pwdTitle.text = (_isChinese?@"会议密码":@"Conference password");
        ...
        [pwdCopyBtn setImage:[QWERImage imageNamed:@"copy"] forState:UIControlStateNormal];
    ...
    ownerTitle.text = (_isChinese?@"主持人":@"Emcee");
    ...
    linkTitle.text = (_isChinese?@"会议链接":@"Conference link");
    ...
    [linkCopyBtn setImage:[QWERImage imageNamed:@"copy"] forState:UIControlStateNormal];
    ...
    offset += 40;
    offset += [QWERUtilities wf_safeDistanceBottom];
...
- (void)onCopy:(id)sender {
    ...
    [self.view makeToast:(_isChinese?@"已拷贝到剪贴板！":@"Copied to clipboard!") duration:1 position:CSToastPositionCenter];
}

- (NSString *)conferenceLink {
    return [[RADCOConferenceManager sharedInstance] linkFromConferenceId:self.conferenceInfo.conferenceId password:self.conferenceInfo.password];
}
...
- (void)updatePortraitAndStateViewFrame {
        ...
        self.trewqPortraitView.frame = CGRectMake((containerWidth-64)/2, [QWERUtilities wf_navigationFullHeight], 64, 64);
        
        self.userasoucNameLabel.frame = CGRectMake((containerWidth - 240)/2, [QWERUtilities wf_navigationFullHeight] + 64 + 8, 240, 26);
...
- (void)showPanel {
    ...
    CGRect floatingAudioBtnFrame = CGRectMake((bounds.size.width - FLOATING_AUDIO_BUTTON_SIZE)/2, bounds.size.height - (landscape ? 0 : [QWERUtilities wf_safeDistanceBottom]) - FLOATING_AUDIO_BUTTON_SIZE - CONFERENCE_BAR_HEIGHT, FLOATING_AUDIO_BUTTON_SIZE, FLOATING_AUDIO_BUTTON_SIZE);
    floatingAudioBtnFrame.origin.y = self.view.bounds.size.height;
    self.floatingAudioButton.frame = floatingAudioBtnFrame;
    
    CGFloat previousBottomBarHeigh = landscape ? CONFERENCE_BAR_HEIGHT : CONFERENCE_BAR_HEIGHT+[QWERUtilities wf_safeDistanceBottom];
    ...
    [UIView animateWithDuration:0.5 animations:^{
        CGFloat bottomBarHeigh = landscape ? CONFERENCE_BAR_HEIGHT : CONFERENCE_BAR_HEIGHT+[QWERUtilities wf_safeDistanceBottom];
        ...
        self.topBarView.frame = CGRectMake(0, 0, self.view.bounds.size.width, landscape ? CONFERENCE_TOP_BAR_WIDTH : [QWERUtilities wf_statusBarHeight] + CONFERENCE_TOP_BAR_WIDTH);
...
- (void)hidePanel {
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        CGFloat topViewHeigh =  landscape ? CONFERENCE_TOP_BAR_WIDTH : CONFERENCE_TOP_BAR_WIDTH + [QWERUtilities wf_statusBarHeight];
        ...
        if(smallVideoRect.origin.y + smallVideoRect.size.height == self.view.bounds.size.height - bottomBarHeigh) {
            smallVideoRect.origin.y = self.view.bounds.size.height - (landscape ? 0 : [QWERUtilities wf_safeDistanceBottom]) - smallVideoRect.size.height;
        }
        self.smallVideoView.frame = smallVideoRect;
        ...
        [UIView animateWithDuration:0.3 animations:^{
            ...
            floatingAudioBtnFrame.origin.y = self.view.bounds.size.height - (landscape ? 0 : [QWERUtilities wf_safeDistanceBottom]) - FLOATING_AUDIO_BUTTON_SIZE - CONFERENCE_BAR_HEIGHT;
            self.floatingAudioButton.frame = floatingAudioBtnFrame;
        }];
    }];
}
...
- (void)keyboardWillHide:(NSNotification *)notification {
    ...
    CGRect frame = self.inputContainer.frame;
    frame.origin.y = self.view.frame.size.height - [QWERUtilities wf_safeDistanceBottom] - CONFERENCE_BAR_HEIGHT - 28 - 68 + 20;
    [UIView animateWithDuration:duration animations:^{
        self.inputContainer.frame = frame;
...
- (void)didChangeState:(WFAVEngineState)state {
    ...
    switch (state) {
        ...
        case kWFAVEngineStateOutgoing:
            ...
            self.screenSharingButton.hidden = YES;
            self.videoButton.hidden = YES;
            self.stateLabel.text = (_isChinese?@"正在等待对方接受邀请...":@"Đang đợi phản hồi");
            self.participantCollectionView.hidden = YES;
            self.userasoucNameLabel.hidden = YES;
            self.trewqPortraitView.hidden = YES;
            [self updatePortraitAndStateViewFrame];
        ...
        case kWFAVEngineStateIncomming:
            ...
            self.audioButton.hidden = YES;
            self.videoButton.hidden = YES;
            self.stateLabel.text = (_isChinese?@"对方正在邀请您通话...":@"Họ đang mời bạn nói chuyện....");
            self.participantCollectionView.hidden = YES;
            break;
        default:
            break;
...
- (void)didCallEndWithReason:(WFAVCallEndReason)reason {
    ...
    [self.view makeToast:(_isChinese?@"会议已结束":@"The meeting is over") duration:1 position:CSToastPositionCenter];
    if(reason == kWFAVCallEndReasonRoomNotExist) {
        ...
        [[RADCOConferenceManager sharedInstance] addHistory:self.conferenceInfo duration:(int)(self.currentSession.endTime - self.currentSession.startTime)];
...
- (void)didParticipantJoined:(NSString *)userId screenSharing:(BOOL)screenSharing {
    ...
    NSString *text;
    if(screenSharing) {
        text = [NSString stringWithFormat:@"%@ %@", userInfo.friendAlias.length ? userInfo.friendAlias : userInfo.displayName, (_isChinese?@"开始屏幕分享":@"Start screen sharing")];
    } else {
        text = [NSString stringWithFormat:@"%@ %@", userInfo.friendAlias.length ? userInfo.friendAlias : userInfo.displayName, (_isChinese?@"加入了会议":@"Joined the meeting")];
    }
...
- (void)didParticipantLeft:(NSString *)userId screenSharing:(BOOL)screenSharing withReason:(WFAVCallEndReason)reason {
    ...
    NSString *reasonStr;
    if(screenSharing) {
        reasonStr = (_isChinese?@"结束了屏幕分享":@"Finished screen sharing");
    } else if (reason == kWFAVCallEndReasonTimeout) {
        reasonStr = (_isChinese?@"未接听":@"No answer");
    } else if(reason == kWFAVCallEndReasonBusy) {
        reasonStr = (_isChinese?@"网络忙":@"Dòng bận");
    } else if(reason == kWFAVCallEndReasonInterrupted) {
        reasonStr = (_isChinese?@"通话中断":@"Ngắt cuộc gọi");
    } else if(reason == kWFAVCallEndReasonRemoteInterrupted) {
        reasonStr = (_isChinese?@"对方通话中断":@"Cuộc gọi bị ngắt");
    } else if(reason == kWFAVCallEndReasonRemoteHangup) {
        reasonStr = (_isChinese?@"离开会议":@"Leave the meeting");
    } else {
        reasonStr = (_isChinese?@"离开会议":@"Leave the meeting"); //"网络错误";
    }
...
- (void)didError:(NSError *)error {
    if([error.domain isEqualToString:@"room_participants_full"]) {
        [self.view makeToast:(_isChinese?@"发言人数已满，无法切换到发言人!":@"Speakers are full, cannot switch to speakers!") duration:1 position:CSToastPositionCenter];
    }
}
...
- (void)didMedia:(NSString *_Nullable)media lostPackage:(int)lostPackage screenSharing:(BOOL)screenSharing {
    //发送方丢包超过6为网络不好
    if(lostPackage > 6) {
        [self.view makeToast:(_isChinese?@"您的网络不好":@"Your Internet is not good") duration:3 position:CSToastPositionCenter];
    }
}
...
- (void)didMedia:(NSString *)media lostPackage:(int)lostPackage uplink:(BOOL)uplink ofUser:(NSString *)userId screenSharing:(BOOL)screenSharing {
    ...
    if(lostPackage > 10) {
        if(uplink) {
            NSLog(@"对方的网络不好");
            [self.view makeToast:(_isChinese?@"对方的网络不好":@"The other party's Internet is not good") duration:3 position:CSToastPositionCenter];
        } else {
            NSLog(@"您的网络不好");
            [self.view makeToast:(_isChinese?@"您的网络不好":@"Your Internet is not good") duration:3 position:CSToastPositionCenter];
        }
...
- (void)rejoinConferenceAsAudience {
    __weak typeof(self)ws = self;
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:nil message:(_isChinese?@"会议发言人数已满，是否以观众身份入会？":@"The number of speakers is full, do you want to join as an audience?") preferredStyle:UIAlertControllerStyleAlert];
    
    UIAlertAction *action1 = [UIAlertAction actionWithTitle:(_isChinese?@"取消":@"Cancel") style:UIAlertActionStyleCancel handler:^(UIAlertAction *action) {
        [ws leftVC];
    }];
    [alertController addAction:action1];
    ...
    UIAlertAction *action2 = [UIAlertAction actionWithTitle:_isChinese ? @"确定":@"OK" style:UIAlertActionStyleDestructive handler:^(UIAlertAction *action) {
        [ws leftVC];
        
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            RADCOConferenceVC *vc = [[RADCOConferenceVC alloc] initWithCallId:conferenceId audioOnly:audioOnly pin:pin host:host title:title desc:desc audience:YES advanced:advanced record:NO moCall:NO maxParticipants:0 extra:nil];
...
- (void)restartConference {
    if([self.currentSession.host isEqualToString:[WFCCNetworkService sharedInstance].userId]) {
        __weak typeof(self)ws = self;
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:nil message:(_isChinese?@"会议未开始或者已经结束，请点击启动来开始会议":@"The meeting is not started or has ended. Click Start to start the meeting") preferredStyle:UIAlertControllerStyleAlert];
        ...
        UIAlertAction *action2 = [UIAlertAction actionWithTitle:_isChinese?@"启动":@"Startup" style:UIAlertActionStyleDestructive handler:^(UIAlertAction *action) {
            [ws leftVC];
            
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                RADCOConferenceVC *vc = [[RADCOConferenceVC alloc] initWithCallId:conferenceId audioOnly:audioOnly pin:pin host:[WFCCNetworkService sharedInstance].userId title:title desc:desc audience:defaultAudience advanced:advanced record:NO moCall:YES maxParticipants:self.conferenceInfo.maxParticipants extra:nil];
                vc.conferenceInfo = self.conferenceInfo;
                [[Chat86AVEngineKit sharedEngineKit] presentViewController:vc];
            });
        }];
        ...
        NSString *message = @"";
        if (_isChinese) {
            message = [NSString stringWithFormat:@"会议未开始或者已经结束，请联系 %@ 启动会议", userInfo.friendAlias.length ? userInfo.friendAlias : userInfo.displayName];
        }else {
            message = [NSString stringWithFormat:@"If the conference is not started or has ended, contact %@ to start the conference", userInfo.friendAlias.length ? userInfo.friendAlias : userInfo.displayName];
        }
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:nil message:message preferredStyle:UIAlertControllerStyleAlert];

        UIAlertAction *action1 = [UIAlertAction actionWithTitle:_isChinese ? @"确定":@"OK" style:UIAlertActionStyleCancel handler:^(UIAlertAction *action) {
            [ws leftVC];
        }];
        [alertController addAction:action1];
        dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)updateVideoStreams {
    if([[RADCOConferenceManager sharedInstance] isBroadcasting]) {
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    ...
    RADCOConferenceCVLayout *layout = (RADCOConferenceCVLayout *)collectionView.collectionViewLayout;
    if(layout.audioOnly) {
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    if([[RADCOConferenceManager sharedInstance] isBroadcasting]) {
        ...
        [stopBtn setImage:[QWERImage imageNamed:@"conference_stop_screen_sharing"] forState:UIControlStateNormal];
        [stopBtn setTitle:_isChinese?@"停止共享":@"Stop sharing" forState:UIControlStateNormal];
    ...
    RADCOConferenceCVLayout *layout = (RADCOConferenceCVLayout *)self.participantCollectionView.collectionViewLayout;
    if(layout.audioOnly) {
        RADCOConferenceAudioCVCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"audio_cell" forIndexPath:indexPath];
        ...
        WFAVParticipantProfile *refreshProfile = [[Chat86AVEngineKit sharedEngineKit].currentSession profileOfUser:user.userId isScreenSharing:user.screeSharing];
        if(refreshProfile) {
            user = refreshProfile;
        }
        
        BOOL isMain = (indexPath.row == 0);
        RADCOConferenceParticipantCVCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:isMain ? @"main" : @"sub" forIndexPath:indexPath];
        if(isMain) {
...
- (void)onDoubleClickedParticipantCollectionView:(id)sender {
    ...
    while (view && ![view isKindOfClass:[RADCOConferenceParticipantCVCell class]]) {
        view = view.superview;
    }
    if(view) {
        NSIndexPath *indexPath = [self.participantCollectionView indexPathForCell:(RADCOConferenceParticipantCVCell *)view];
...
- (void)onItemsEnter:(NSArray<NSIndexPath *> *)enterItems itemsLeave:(NSMutableArray<NSIndexPath *> *)leaveItems {
    ...
    [enterItems enumerateObjectsUsingBlock:^(NSIndexPath * _Nonnull indexPath, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        BOOL isMain = (indexPath.row == 0);
        WFAVParticipantProfile *profile = ((RADCOConferenceParticipantCVCell *)cell).profile;
...
#pragma mark - RADCOConferenceManagerDelegate
-(void)onChangeModeRequest:(BOOL)isAudience {
    if(isAudience) {
        [[Chat86AVEngineKit sharedEngineKit].currentSession switchAudience:isAudience];
    } else {
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"" message:_isChinese?@"主持人邀请您发言":@"The moderator invites you to speak" preferredStyle:UIAlertControllerStyleAlert];
        
        UIAlertAction *action1 = [UIAlertAction actionWithTitle:(_isChinese?@"忽略":@"Ignore") style:UIAlertActionStyleCancel handler:^(UIAlertAction *action) {
            
        }];
        [alertController addAction:action1];
        
        UIAlertAction *action2 = [UIAlertAction actionWithTitle:(_isChinese?@"接受":@"Accept") style:UIAlertActionStyleDestructive handler:^(UIAlertAction *action) {
            [[Chat86AVEngineKit sharedEngineKit].currentSession switchAudience:isAudience];
        }];
        [alertController addAction:action2];
...
-(void)onReceiveCommand:(WFCUConferenceCommandType)commandType content:(RADCOConferenceCommandContent *)commandContent fromUser:(NSString *)sender {
    WFCCUserInfo *userInfo = [[WFCCIMService sharedWFCIMService] getUserInfo:sender refresh:NO];
    NSString *userName = (_isChinese?@"对方":@"Each other");
    ...
    switch(commandType) {
        case MUTE_ALL: {
            [self showCommandToast:(_isChinese?@"主持人开启了全员静音":@"The host turned on the silence for everyone")];
            break;
        }
        case CANCEL_MUTE_ALL: {
            if(commandContent.boolValue && [Chat86AVEngineKit sharedEngineKit].currentSession.isConference && ([Chat86AVEngineKit sharedEngineKit].currentSession.isAudience || [Chat86AVEngineKit sharedEngineKit].currentSession.isAudioMuted)) {
                UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"" message:_isChinese?@"主持人关闭了全员静音，是否要打开麦克风":@"The host has turned off all the silence. Do you want to turn on the microphone" preferredStyle:UIAlertControllerStyleAlert];
                ...
                UIAlertAction *action2 = [UIAlertAction actionWithTitle:(_isChinese?@"打开":@"Open") style:UIAlertActionStyleDestructive handler:^(UIAlertAction *action) {
                    [[RADCOConferenceManager sharedInstance] muteAudio:NO];
                }];
                [alertController addAction:action2];
                
                [self presentViewController:alertController animated:YES completion:nil];
            } else {
                [self showCommandToast:(_isChinese?@"主持人关闭了全员静音":@"The host turned off the silence")];
            }
            break;
        }
        case REQUEST_MUTE:
            if(commandContent.boolValue) {
                [self showCommandToast:(_isChinese?@"主持人关闭了您的发言":@"The moderator has closed your speech")];
            } else {
                UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"" message:(_isChinese?@"主持人邀请您发言":@"The moderator invites you to speak") preferredStyle:UIAlertControllerStyleAlert];
                
                UIAlertAction *action1 = [UIAlertAction actionWithTitle:(_isChinese?@"拒绝":@"Reject") style:UIAlertActionStyleCancel handler:^(UIAlertAction *action) {
                    [[RADCOConferenceManager sharedInstance] rejectUnmuteRequest];
                }];
                [alertController addAction:action1];
        ...
        case REJECT_UNMUTE_REQUEST:
            [self showCommandToast:[NSString stringWithFormat:@"%@ %@",userName, (_isChinese?@"拒绝了您的发言邀请":@" declined your invitation to speak")]];
            break;
            
        case APPLY_UNMUTE:
            [self showCommandToast:[NSString stringWithFormat:@"%@ %@",userName, (_isChinese?@"请求发言":@"Request to speak")]];
            break;
            
        case APPROVE_UNMUTE:
            if(!commandContent.boolValue) {
                [self showCommandToast:(_isChinese?@"主持人拒绝了您的发言请求":@"The moderator has refused your request to speak")];
            }
            break;
        case APPROVE_ALL_UNMUTE:
            [self showCommandToast:commandContent.boolValue ? (_isChinese?@"主持人同意了所有的发言请求":@"The moderator granted all requests to speak") :  (_isChinese?@"主持人拒绝了所有的发言请求":@"The moderator declined all requests to speak")];
            break;
        ...
        case HANDUP:
            ...
            //主持人放下成员的举手
        case PUT_HAND_DOWN:
            [self showCommandToast:(_isChinese?@"主持人放下您的举手":@"The host drops your hand")];
            break;
            //主持人放下全体成员的举手
        case PUT_ALL_HAND_DOWN:
            [self showCommandToast:(_isChinese?@"主持人放下所有举手":@"The host put down all hands")];
            break;
            
        case RECORDING:
            [self showCommandToast:commandContent.boolValue ? (_isChinese?@"主持人开始录制":@"The host starts recording.") : (_isChinese?@"主持人结束录制":@"Host ends recording")];
            break;
        case FOCUS:
            [self showCommandToast:(_isChinese?@"主持人锁定焦点用户":@"The moderator locks the focus user")];
            if(![self.focusUserProfile.userId isEqualToString:[RADCOConferenceManager sharedInstance].currentConferenceInfo.focus]) {
                [self rearrangeParticipants];
                [self reloadParticipantCollectionView];
            }
            break;
        case CANCEL_FOCUS:
            [self showCommandToast:(_isChinese?@"主持人取消锁定焦点用户":@"The moderator cancels the locking of a focus user")];
            [self rearrangeParticipants];
            [self reloadParticipantCollectionView];
            break;
        default:
