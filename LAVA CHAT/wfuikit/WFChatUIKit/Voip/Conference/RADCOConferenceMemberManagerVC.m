//
//  RADCOConferenceMemberManagerVC.m
//  WFChatUIKit
//
...
#import "UIColor+YH.h"
#import "RADCOConferenceUnmuteRequestTableVC.h"
#import "RADCOConferenceHandupTableVC.h"
#import <Chat86AVEngineKit/Chat86AVEngineKit.h>
#import "QWERUtilities.h"
#import "RADCOConferenceMember.h"
...
@interface RADCOConferenceMemberManagerVC () <UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate>
@property (nonatomic, strong)UITableView *tableView;
@property (nonatomic, strong)UISearchBar *searchBar;
@property (nonatomic, strong) NSMutableArray<RADCOConferenceMember *> *participants;
...
@implementation RADCOConferenceMemberManagerVC
...
- (void)viewDidLoad {
    ...
    [self.tableView registerClass:[RADCOConferenceMemberTVCell class] forCellReuseIdentifier:@"cell"];
...
- (void)onUnmuteRequestBtnPressed:(id)sender {
    RADCOConferenceUnmuteRequestTableVC *vc = [[RADCOConferenceUnmuteRequestTableVC alloc] init];
    UINavigationController *nav = [[UINavigationController alloc] initWithRootViewController:vc];
    [self presentViewController:nav animated:YES completion:nil];
}

- (void)onHandupBtnPressed:(id)sender {
    RADCOConferenceHandupTableVC *vc = [[RADCOConferenceHandupTableVC alloc] init];
    UINavigationController *nav = [[UINavigationController alloc] initWithRootViewController:vc];
    [self presentViewController:nav animated:YES completion:nil];
}

- (void)updateTableViewHeader {
    if([self.searchBar isFirstResponder] || ([RADCOConferenceManager sharedInstance].applyingUnmuteMembers.count == 0 && [RADCOConferenceManager sharedInstance].handupMembers.count == 0)) {
        ...
        int height = 40;
        NSMutableArray<NSString *> *applyingUnmuteMembers = [RADCOConferenceManager sharedInstance].applyingUnmuteMembers;
        ...
        NSMutableArray<NSString *> *handupMembers = [RADCOConferenceManager sharedInstance].handupMembers;
        if([RADCOConferenceManager sharedInstance].handupMembers.count) {
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if(!self.muteAllBtn && [[RADCOConferenceManager sharedInstance] isOwner]) {
        CGRect bounds = self.view.bounds;
        CGFloat buttonHeight = 48;
        CGFloat buttonWidth = bounds.size.width/2 - 16 - 8;
        
        self.muteAllBtn = [self createBtn:CGRectMake(16, bounds.size.height - [QWERUtilities wf_safeDistanceBottom] - buttonHeight - 16, buttonWidth, buttonHeight) title:WFCString(@"MuteAll") action:@selector(onMuteAllBtnPressed:)];
        self.unmuteAllBtn = [self createBtn:CGRectMake(bounds.size.width - 16 - buttonWidth, bounds.size.height - [QWERUtilities wf_safeDistanceBottom] - buttonHeight - 16, buttonWidth, buttonHeight) title:WFCString(@"UnmuteAll") action:@selector(onUnmuteAllBtnPressed:)];
    }
...
- (void)onMuteAllBtnPressed:(id)sender {
    [[RADCOConferenceManager sharedInstance] presentCommandAlertView:self message:@"所有成员将被静音" actionTitle:@"全体静音" cancelTitle:nil contentText:@"允许成员自主解除静音" checkBox:YES actionHandler:^(BOOL checked) {
        [[RADCOConferenceManager sharedInstance] requestMuteAll:checked];
    } cancelHandler:nil];
}

- (void)onUnmuteAllBtnPressed:(id)sender {
    [[RADCOConferenceManager sharedInstance] presentCommandAlertView:self message:@"允许全体成员开麦" actionTitle:@"取消全体静音" cancelTitle:nil contentText:@"是否要求成员开麦" checkBox:YES actionHandler:^(BOOL checked) {
        [[RADCOConferenceManager sharedInstance] requestUnmuteAll:checked];
    } cancelHandler:nil];
}
...
- (void)loadData {
    ...
    WFAVCallSession *callSession = [Chat86AVEngineKit sharedEngineKit].currentSession;
    NSArray<WFAVParticipantProfile *> *ps =  [Chat86AVEngineKit sharedEngineKit].currentSession.participants;
    
    NSMutableArray *audiences = [[NSMutableArray alloc] init];
    for (WFAVParticipantProfile *p in ps) {
        RADCOConferenceMember *member = [[RADCOConferenceMember alloc] init];
        member.userId = p.userId;
        member.isHost = [p.userId isEqualToString:[RADCOConferenceManager sharedInstance].currentConferenceInfo.owner];
    ...
    member.isHost = [member.userId isEqualToString:[RADCOConferenceManager sharedInstance].currentConferenceInfo.owner];
...
- (void)onInvite:(id)sender {
    ...
    invite.password = [RADCOConferenceManager sharedInstance].currentConferenceInfo.password;
...
- (nonnull UITableViewCell *)tableView:(nonnull UITableView *)tableView cellForRowAtIndexPath:(nonnull NSIndexPath *)indexPath {
    RADCOConferenceMemberTVCell *cell = (RADCOConferenceMemberTVCell *)[tableView dequeueReusableCellWithIdentifier:@"cell"];
...
//        [[RADCOConferenceManager sharedInstance] request:member.userId changeModel:NO inConference:[Chat86AVEngineKit sharedEngineKit].currentSession.callId];
        [[RADCOConferenceManager sharedInstance] requestMember:member.userId Mute:NO];
    }];
    ...
    UIAlertAction *requestUnpublish = [UIAlertAction actionWithTitle:@"取消发言" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        [[RADCOConferenceManager sharedInstance] requestMember:member.userId Mute:YES];
    }];
    ...
    UIAlertAction *enableAudio = [UIAlertAction actionWithTitle:@"开启音频" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        [[RADCOConferenceManager sharedInstance] muteAudio:member.isAudioEnabled];
    }];
    UIAlertAction *enableVideo = [UIAlertAction actionWithTitle:@"开启视频" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        [[RADCOConferenceManager sharedInstance] muteVideo:member.isVideoEnabled];
    }];
    UIAlertAction *enableAudioVideo = [UIAlertAction actionWithTitle:@"开启音视频" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        [[RADCOConferenceManager sharedInstance] muteAudioVideo:member.isVideoEnabled];
    }];
    ...
    UIAlertAction *focusMember = [UIAlertAction actionWithTitle:@"设置为焦点用户" style:UIAlertActionStyleDestructive handler:^(UIAlertAction *action) {
        [[RADCOConferenceManager sharedInstance] requestFocus:member.userId];
    }];
    ...
    UIAlertAction *cancelFocusMember = [UIAlertAction actionWithTitle:@"取消焦点用户" style:UIAlertActionStyleDestructive handler:^(UIAlertAction *action) {
        [[RADCOConferenceManager sharedInstance] requestCancelFocus];
    }];
    
    [alertController addAction:actionCancel];
    [alertController addAction:showProfile];
    
    if([[RADCOConferenceManager sharedInstance].currentConferenceInfo.owner isEqualToString:[WFCCNetworkService sharedInstance].userId] && !member.isMe) {
        ...
        if(member.isAudience) {
            if([[RADCOConferenceManager sharedInstance].currentConferenceInfo.owner isEqualToString:[WFCCNetworkService sharedInstance].userId] || [RADCOConferenceManager sharedInstance].currentConferenceInfo.allowTurnOnMic) {
    ...
    if([[RADCOConferenceManager sharedInstance].currentConferenceInfo.owner isEqualToString:[WFCCNetworkService sharedInstance].userId]) {
        if([member.userId isEqualToString:[RADCOConferenceManager sharedInstance].currentConferenceInfo.focus]) {
