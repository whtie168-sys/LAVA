...
@interface RADCOMultiVideoVC () <UITextFieldDelegate
#if WFCU_SUPPORT_VOIP
...
@implementation RADCOMultiVideoVC
#if !WFCU_SUPPORT_VOIP
...
- (void)viewDidLoad {
    ...
    self.smallCollectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, [QWERUtilities wf_navigationFullHeight], self.view.frame.size.width, itemWidth*lines + layout.minimumLineSpacing * (lines - 1)) collectionViewLayout:layout];
    
    self.smallCollectionView.dataSource = self;
    self.smallCollectionView.delegate = self;
    [self.smallCollectionView registerClass:[RADCOParticipantCVCell class] forCellWithReuseIdentifier:@"cell"];
    ...
    RADCOParticipantCVLayout *layout2 = [[RADCOParticipantCVLayout alloc] init];
    ...
    [self.portraitCollectionView registerClass:[RADCOPortraitCVCell class] forCellWithReuseIdentifier:@"cell2"];
    ...
    self.trewqPortraitView.layer.cornerRadius = 8.f;
...
//        _minimizeButton = [[UIButton alloc] initWithFrame:CGRectMake(16, 26, 30, 30)];
        ...
        _minimizeButton = [[UIButton alloc] initWithFrame:CGRectMake(16, (UIScreen.mainScreen.bounds.size.height >= 812.0 ? 50.0 : 26.0), 44, 44)];
...
//        _addParticipantButton = [[UIButton alloc] initWithFrame:CGRectMake(self.view.frame.size.width - 16 - 30, 26, 30, 30)];
        ...
        _addParticipantButton = [[UIButton alloc] initWithFrame:CGRectMake(self.view.frame.size.width - 16 - 30, (UIScreen.mainScreen.bounds.size.height >= 812.0 ? 50.0 : 26.0), 44, 44)];

        
        [_addParticipantButton setImage:[QWERImage imageNamed:@"plus-circle"] forState:UIControlStateNormal];
        [_addParticipantButton setImage:[QWERImage imageNamed:@"plus-circle"] forState:UIControlStateHighlighted];
        [_addParticipantButton setImage:[QWERImage imageNamed:@"plus-circle"] forState:UIControlStateSelected];
...
//        [_audioButton setImage:[QWERImage imageNamed:@"mute_hover"] forState:UIControlStateSelected];
        [_audioButton setImage:[QWERImage imageNamed:@"mute_0129_E"] forState:UIControlStateNormal];
        [_audioButton setImage:[QWERImage imageNamed:@"mute_hover_0129_E"] forState:UIControlStateHighlighted];
        [_audioButton setImage:[QWERImage imageNamed:@"mute_hover_0129_E"] forState:UIControlStateSelected];
...
- (UIButton *)videoButton {
    if (!_videoButton) {
        ...
        [_videoButton setImage:[QWERImage imageNamed:@"enable_video"] forState:UIControlStateNormal];
...
- (void)minimizeButtonDidTap:(UIButton *)button {
    ...
    [RADCOFloatingWindow startCallFloatingWindow:self.currentSession focusUser:focusUser withTouchedBlock:^(WFAVCallSession *callSession, RADCOConferenceInfo *conferenceInfo) {
        RADCOMultiVideoVC *vc = [[RADCOMultiVideoVC alloc] initWithSession:callSession];
...
- (void)addParticipantButtonDidTap:(UIButton *)button {
    ...
    pvc.maxSelectCount = self.currentSession.audioOnly ? [Chat86AVEngineKit sharedEngineKit].maxAudioCallCount : [Chat86AVEngineKit sharedEngineKit].maxVideoCallCount;
...
- (void)updateAudioButton {
    if (self.currentSession.audioMuted) {
        [self.audioButton setImage:[QWERImage imageNamed:@"mute_hover_0129_E"] forState:UIControlStateNormal];
    } else {
        [self.audioButton setImage:[QWERImage imageNamed:@"mute_0129_E"] forState:UIControlStateNormal];
    }
}
...
- (void)updateSpeakerButton {
    if (!self.currentSession.isSpeaker) {
        if([self.currentSession isHeadsetPluggedIn]) {
            ...
            [self.speakerButton setImage:[QWERImage imageNamed:@"speaker"] forState:UIControlStateNormal];
        }
    } else {
        [self.speakerButton setImage:[QWERImage imageNamed:@"speaker_hover"] forState:UIControlStateNormal];
    }
}

- (void)updateVideoButton {
    if (self.currentSession.videoMuted) {
        [self.videoButton setImage:[QWERImage imageNamed:@"disable_video"] forState:UIControlStateNormal];
    } else {
        [self.videoButton setImage:[QWERImage imageNamed:@"enable_video"] forState:UIControlStateNormal];
    }
}
...
//    } else {
//        self.trewqPortraitView.frame = CGRectMake(16, [QWERUtilities wf_navigationFullHeight], 64, 64);
//        self.userasoucNameLabel.frame = CGRectMake(96, [QWERUtilities wf_navigationFullHeight] + 8, 240, 26);
...
//        } else {
//            self.stateLabel.frame = CGRectMake(96, [QWERUtilities wf_navigationFullHeight] + 26 + 14, 240, 16);
//        }
...
- (void)didReportAudioVolume:(NSInteger)volume ofUser:(NSString *)userId {
    if([QWERConfigManager globalManager].displaySpeakingInMultiCall) {
...
- (void)didParticipantLeft:(NSString *)userId screenSharing:(BOOL)screenSharing withReason:(WFAVCallEndReason)reason {
    ...
    if (reason == kWFAVCallEndReasonTimeout) {
        reasonStr = (_isChinese ? @"未接听" : @"Chưa trả lời");
    } else if(reason == kWFAVCallEndReasonBusy) {
        reasonStr = (_isChinese ? @"网络忙" : @"Internet bận rộn");
    } else if(reason == kWFAVCallEndReasonInterrupted) {
        reasonStr = (_isChinese ? @"通话中断" : @"Cuộc gọi bị ngắt.");
        ...
        reasonStr = (_isChinese ? @"离开会议" : @"đã rời cuộc trò chuyện");
    } else {
        reasonStr = (_isChinese ? @"离开会议" : @"đã rời cuộc trò chuyện"); //"网络错误";
    }
    ...
    if (!self.currentSession.audioOnly) {
        ...
        if (self.currentSession.state == kWFAVEngineStateConnecting || self.currentSession.state == kWFAVEngineStateConnected) {
            ...
            if (self.participants.count - 1 > 3) {
                self.smallCollectionView.frame = CGRectMake(0, [QWERUtilities wf_navigationFullHeight], self.view.frame.size.width, itemWidth * 2 + layout.minimumLineSpacing);
            } else {
                self.smallCollectionView.frame = CGRectMake(0, [QWERUtilities wf_navigationFullHeight], self.view.frame.size.width, itemWidth);
            }
...
- (void)didMedia:(NSString *)media lostPackage:(int)lostPackage uplink:(BOOL)uplink ofUser:(NSString *)userId screenSharing:(BOOL)screenSharing {
    ...
    if(lostPackage > 10) {
        if(uplink) {
            NSLog(@"对方的网络不好");
            [self.view makeToast:(_isChinese?@"对方的网络不好":@"Mạng lưới của nhau không tốt.") duration:3 position:CSToastPositionCenter];
        } else {
            NSLog(@"您的网络不好");
            [self.view makeToast:(_isChinese?@"您的网络不好":@"Mạng của bạn không tốt") duration:3 position:CSToastPositionCenter];
        }
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (collectionView == self.smallCollectionView) {
        RADCOParticipantCVCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"cell" forIndexPath:indexPath];
        ...
        RADCOPortraitCVCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"cell2" forIndexPath:indexPath];
