...
@implementation ASDFGMessageCellBase
...
- (void)setModel:(QWERTMessageModel *)model {
  ...
  _model = model;
    ...
    if (model.lastReadMessage) {
        if (!self.lastReadContainerView) {
            ...
            CGSize size = [QWERUtilities getTextDrawingSize:label.text font:label.font constrainedSize:CGSizeMake(screenWidth-16, 8000)];
            size.width += 16;
  ...
  if (model.showTimeLabel) {
    ...
    _timeLabel.text = [QWERUtilities formatTimeDetailLabel:model.message.serverTime];

    
    CGSize size = [QWERUtilities getTextDrawingSize:_timeLabel.text font:_timeLabel.font constrainedSize:CGSizeMake(screenWidth, 8000)];
