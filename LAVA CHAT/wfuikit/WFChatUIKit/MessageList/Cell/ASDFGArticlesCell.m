...
@implementation ASDFGArticlesCell
+ (CGSize)sizeForCell:(QWERTMessageModel *)msgModel withViewWidth:(CGFloat)width {
    ...
    if(content.subArticles.count) {
        ...
        CGSize titleSize = [QWERUtilities getTextDrawingSize:content.topArticle.title font:[UIFont systemFontOfSize:TOP_TITLE_FONT_SIZE] constrainedSize:CGSizeMake(containerWidth-CELL_PADDING-CELL_PADDING, 50)];
...
- (void)setModel:(QWERTMessageModel *)model {
    ...
    [self.coverImageView sd_setImageWithURL:[NSURL URLWithString:content.topArticle.cover] placeholderImage:nil options:SDWebImageScaleDownLargeImages
...
- (CGFloat)addArticle:(WFCCArticle *)article offset:(CGFloat)offset containerWidth:(CGFloat)containerWidth index:(NSUInteger)index {
    ...
    [subCover sd_setImageWithURL:[NSURL URLWithString:article.cover] placeholderImage:nil options:SDWebImageScaleDownLargeImages
                         context:@{SDWebImageContextImageForceDecodePolicy : @(SDImageForceDecodePolicyNever), SDWebImageContextStoreCacheType : @(SDImageCacheTypeDisk)}];
