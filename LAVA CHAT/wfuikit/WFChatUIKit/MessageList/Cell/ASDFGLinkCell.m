...
@interface ASDFGLinkCell ()
@property (nonatomic, strong)UIImageView *thumbnailImageView;
@property (nonatomic, strong)UILabel *TitleLabel;
@property (nonatomic, strong)UILabel *contentLabel;
@end

@implementation ASDFGLinkCell
...
+ (CGSize)sizeForClientArea:(QWERTMessageModel *)msgModel withViewWidth:(CGFloat)width {
    ...
    CGSize titleSize = [QWERUtilities getTextDrawingSize:content.title font:[UIFont systemFontOfSize:18] constrainedSize:CGSizeMake(width, 50)];
    ...
    CGSize contentSize = [QWERUtilities getTextDrawingSize:contentTxt font:[UIFont systemFontOfSize:14] constrainedSize:CGSizeMake(contentWidth, 68)];
...
- (void)setModel:(QWERTMessageModel *)model {
    ...
    [self.thumbnailImageView sd_setImageWithURL:[NSURL URLWithString:content.thumbnailUrl] placeholderImage:[QWERImage imageNamed:@"default_link"] options:SDWebImageScaleDownLargeImages
...
- (UIImageView *)thumbnailImageView {
    if (!_thumbnailImageView) {
        ...
        [self.asoucContentArea addSubview:_thumbnailImageView];
    }
