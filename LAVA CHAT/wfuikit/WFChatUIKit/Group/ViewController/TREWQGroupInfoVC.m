...
- (void)setGroupInfo:(WFCCGroupInfo *)groupInfo {
    ...
    if(groupInfo) {
        if(groupInfo.portrait.length) {
            [self.groupProtraitView sd_setImageWithURL:[NSURL URLWithString:groupInfo.portrait] placeholderImage:[QWERImage imageNamed:@"groupIcon"]];
        }
        
        self.groupasoucNameLabel.text = [NSString stringWithFormat:@"%@(%ld)", groupInfo.displayName, groupInfo.memberCount];
    }
}
...
- (void)setMembers:(NSArray<WFCCGroupMember *> *)members {
    ...
    if(!isContainMe) {
        __weak typeof(self)ws = self;
        [[QWERConfigManager globalManager].appServiceProvider getGroupMembersForPortrait:self.groupId success:^(NSArray<NSDictionary<NSString *, NSString *> *> *groupMembers) {
...
- (UILabel *)groupasoucNameLabel {
    if (!_groupasoucNameLabel) {
        CGFloat width = [UIScreen mainScreen].bounds.size.width;
        _groupasoucNameLabel = [[UILabel alloc] initWithFrame:CGRectMake(width/2 - 100, 220, 200, 24)];
        _groupasoucNameLabel.textAlignment = NSTextAlignmentCenter;
        [self.view addSubview:_groupasoucNameLabel];
    }
    return _groupasoucNameLabel;
}
