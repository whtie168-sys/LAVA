...
@interface ASECWConversationTableVC () <UISearchControllerDelegate, UISearchResultsUpdating, UITableViewDelegate, UITableViewDataSource>
...
@implementation ASECWConversationTableVC
...
- (void)onRightBarBtn:(UIBarButtonItem *)sender {
    ...
    if ([[WFCCIMService sharedWFCIMService] isEnableSecretChat] && [[WFCCIMService sharedWFCIMService] isUserEnableSecretChat]) {
        menuItems = @[
            [KxMenuItem menuItem:WFCString(@"StartChat")
                           image:[QWERImage imageNamed:@"menu_start_chat"]
                          target:self
    ...
    [KxMenu showMenuInView:self.navigationController.view
                  fromRect:CGRectMake(self.view.bounds.size.width - 56, [QWERUtilities wf_navigationFullHeight] + searchExtra, 48, 5)
                 menuItems:menuItems];
}
...
- (void)viewDidLoad {
    ...
    self.definesPresentationContext = YES;
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[QWERImage imageNamed:@"bar_plus"] style:UIBarButtonItemStyleDone target:self action:@selector(onRightBarBtn:)];
...
- (UIView *)pcSessionView {
    ...
    if (infos.count) {
        if (infos[0].platform == PlatformType_Windows) {
            self.pcSessionLabel.text = [NSString stringWithFormat:@"Windows %@", @"已登录"];
        } else if(infos[0].platform == PlatformType_OSX) {
            self.pcSessionLabel.text = [NSString stringWithFormat:@"Mac %@", @"已登录"];
        } else if(infos[0].platform == PlatformType_Linux) {
            self.pcSessionLabel.text = [NSString stringWithFormat:@"Linux %@", @"已登录"];
        } else if(infos[0].platform == PlatformType_WEB) {
            self.pcSessionLabel.text = [NSString stringWithFormat:@"Web %@", @"已登录"];
            ...
            self.pcSessionLabel.text = [NSString stringWithFormat:@"iPad %@", @"已登录"];
