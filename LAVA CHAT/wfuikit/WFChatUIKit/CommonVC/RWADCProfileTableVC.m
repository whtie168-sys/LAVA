...
@interface RWADCProfileTableVC () <UITableViewDelegate, UITableViewDataSource, MWPhotoBrowserDelegate>
@property (strong, nonatomic)UIImageView *trewqPortraitView;
@property (strong, nonatomic)UILabel *aliasLabel;
@property (strong, nonatomic)UILabel *displayasoucNameLabel;
@property (strong, nonatomic)UILabel *userasoucNameLabel;
...
@implementation RWADCProfileTableVC
...
- (void)loadOrganizationData:(BOOL)refresh {
    ESZQSCEmployeeEx *employeeEx = [[ESZQSCOrganizationCache sharedCache] getEmployeeEx:self.userId refresh:refresh];
    if(employeeEx.relationships.count) {
        NSMutableArray<NSNumber *> *arr = [[NSMutableArray alloc] init];
        [employeeEx.relationships enumerateObjectsUsingBlock:^(ESZQSCOrgRelationship * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if(obj.bottom) {
...
- (void)onRightBtn:(id)sender {
    ...
    if ([[WFCCIMService sharedWFCIMService] isMyFriend:self.userId]) {
        ...
        UIAlertAction *addFriendAction = [UIAlertAction actionWithTitle:WFCString(@"AddFriend") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            ATERWVerifyRequestVC *vc = [[ATERWVerifyRequestVC alloc] init];
            vc.userId = ws.userId;
...
- (void)loadData {
    ...
    self.trewqPortraitView = [[UIImageView alloc] initWithFrame:CGRectMake(16, 14, 58, 58)];
    
    self.trewqPortraitView.layer.cornerRadius = 29;
    ...
    [self.trewqPortraitView addGestureRecognizer:tap];
    self.trewqPortraitView.userInteractionEnabled = YES;
    
    
    [self.trewqPortraitView sd_setImageWithURL:[NSURL URLWithString:[self.userInfo.portrait stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]] placeholderImage: [QWERImage imageNamed:@"PersonalChat"]];
    ...
    if (alias.length) {
        ...
        self.aliasLabel.font = [UIFont pingFangSCWithWeight:FontWeightStyleMedium size:18];
        self.aliasLabel.text = alias;
        
        self.displayasoucNameLabel = [[UILabel alloc] initWithFrame:CGRectMake(94, 32, width - 94 - 8, 21)];
        self.displayasoucNameLabel.font = [UIFont pingFangSCWithWeight:FontWeightStyleRegular size:14];
        self.displayasoucNameLabel.text = self.userInfo.displayName;
        
        self.userasoucNameLabel = [[UILabel alloc] initWithFrame:CGRectMake(94, 60, width - 94 - 8, 11)];
        self.userasoucNameLabel.text = [NSString stringWithFormat:@"ID: %@", self.userInfo.name];
        self.userasoucNameLabel.font = [UIFont pingFangSCWithWeight:FontWeightStyleRegular size:12];
        self.userasoucNameLabel.textColor = [UIColor grayColor];
    } else {
        self.aliasLabel = [[UILabel alloc] initWithFrame:CGRectZero];

        self.displayasoucNameLabel = [[UILabel alloc] initWithFrame:CGRectMake(94, 23, width - 94 - 8, 21)];
        self.displayasoucNameLabel.text = self.userInfo.displayName;
        self.displayasoucNameLabel.font = [UIFont pingFangSCWithWeight:FontWeightStyleMedium size:20];
        
        self.userasoucNameLabel = [[UILabel alloc] initWithFrame:CGRectMake(94, 50, width - 94 - 8, 21)];
        self.userasoucNameLabel.text = [NSString stringWithFormat:@"ID: %@", self.userInfo.name];
        self.userasoucNameLabel.font = [UIFont pingFangSCWithWeight:FontWeightStyleRegular size:12];
        self.userasoucNameLabel.textColor = [UIColor grayColor];
    }
    if (self.fromConversation.type == Group_Type) {
        if (self.isManager) { // 是管理员 可以看到ID
            self.userasoucNameLabel.hidden = NO;
        }else {
            if ([[WFCCIMService sharedWFCIMService] isMyFriend:self.userId]) { // 是好友关系
                self.userasoucNameLabel.hidden = NO;
            }else {
                if ([[WFCCNetworkService sharedInstance].userId isEqualToString:self.userId]) {
                    self.userasoucNameLabel.hidden = NO;
    ...
    if ([[WFCCIMService sharedWFCIMService] isFavUser:self.userId]) {
        self.starLabel = [[UILabel alloc] initWithFrame:CGRectMake(width - 16 - 20, self.displayasoucNameLabel.frame.origin.y, 20, 20)];
        self.starLabel.text = @"☆";
    ...
    [self.headerCell.contentView addSubview:self.trewqPortraitView];
    [self.headerCell.contentView addSubview:self.displayasoucNameLabel];
    [self.headerCell.contentView addSubview:self.userasoucNameLabel];
    ...
    if (self.userInfo.type == 1) {
        ...
        if ([[WFCCIMService sharedWFCIMService] isMyFriend:self.userId]) {
            ...
            [btn setTitleColor:[QWERConfigManager globalManager].textColor forState:UIControlStateNormal];
        ...
        if (self.userInfo.type == 0) {
            if(NSClassFromString(@"SDTimeLineTableViewController")) {
                ...
                [momentButton setTitleColor:[QWERConfigManager globalManager].textColor forState:UIControlStateNormal];
        ...
        if ([[WFCCIMService sharedWFCIMService] isMyFriend:self.userId]) {
            ...
            [btn setImage:[QWERImage imageNamed:@"video"] forState:UIControlStateNormal];
            ...
            UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(20, 5, width - 40, 40)];
            [btn setTitle:WFCString(@"AddFriend") forState:UIControlStateNormal];
            [btn setBackgroundColor:RGBCOLOR(92, 226, 83)];
...
- (void)setupSendMessageCell:(CGFloat)width {
    ...
    UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, width, 50)];
    [btn setImage:[QWERImage imageNamed:@"message"] forState:UIControlStateNormal];
    btn.imageEdgeInsets = UIEdgeInsetsMake(0, 0, 0, 10);
...
- (void)onSendMessageBtn:(id)sender {
    ...
    for (UIViewController *vc in self.navigationController.viewControllers) {
        if ([vc isKindOfClass:[QWERMessageListVC class]]) {
            QWERMessageListVC *old = (QWERMessageListVC*)vc;
...
- (void)onVoipCallBtn:(id)sender {
    ...
    UIAlertAction *actionVoice = [UIAlertAction actionWithTitle:WFCString(@"VoiceCall") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        RADCOVideoVC *videoVC = [[RADCOVideoVC alloc] initWithTargets:@[ws.userInfo.userId] conversation:conversation audioOnly:YES];
    ...
    UIAlertAction *actionVideo = [UIAlertAction actionWithTitle:WFCString(@"VideoCall") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        WFCCConversation *conversation = [WFCCConversation conversationWithType:Single_Type target:ws.userInfo.userId line:0];
        RADCOVideoVC *videoVC = [[RADCOVideoVC alloc] initWithTargets:@[ws.userInfo.userId] conversation:conversation audioOnly:NO];
        [[Chat86AVEngineKit sharedEngineKit] presentViewController:videoVC];
    }];
    ...
    if (indexPath.section == 0) {
       return self.headerCells[indexPath.row];
        ...
        if(index < self.organizationIds.count) {
            ...
            ESZQSCOrganization *org = [[ESZQSCOrganizationCache sharedCache] getOrganization:orgId refresh:NO];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if([tableView cellForRowAtIndexPath:indexPath] == self.userMessagesCell) {
        QWERUserMessageListVC *vc = [[QWERUserMessageListVC alloc] init];
        vc.userId = self.userId;
        vc.conversation = self.fromConversation;
        [self.navigationController pushViewController:vc animated:YES];
    } else if([tableView cellForRowAtIndexPath:indexPath] == self.moreCell) {
        RWADCProfileMoreTableVC *moreVC = [[RWADCProfileMoreTableVC alloc] init];
        ...
        if(index < self.organizationIds.count) {
            NSInteger orgId = [self.organizationIds[index] integerValue];
            NSArray<ESZQSCOrgRelationship *> *rs = [[ESZQSCOrganizationCache sharedCache] getRelationship:self.userId refresh:YES];
