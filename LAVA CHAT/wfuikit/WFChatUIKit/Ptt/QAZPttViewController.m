...
#import <LavaWFChatClient/WFCChatClient.h>
@interface QAZPttViewController ()
@property(nonatomic, strong)UIButton *startButton;
@end

@implementation QAZPttViewController
...
- (void)updateStartBtnStatus {
    ...
    BOOL isChinese = [WFCCIMService.main isChinese];
        CGFloat btnSize = 100;
    NSString *title = (isChinese?@"按下讲话":@"Bấm vào bài phát biểu");
        UIColor *color = [UIColor redColor];
        if([[WFPttClient sharedClient] getTalkingConversation] == self.conversation) {
            btnSize = 150;
            title = (isChinese?@"松开结束对讲":@"Thả lỏng liên lạc");
            color = [UIColor greenColor];
        }
        
        CGRect bound = self.view.bounds;
        self.startButton.frame = CGRectMake((bound.size.width - btnSize)/2, self.view.bounds.size.height - [QWERUtilities wf_safeDistanceBottom] - btnSize/2 - 160, btnSize, btnSize);
