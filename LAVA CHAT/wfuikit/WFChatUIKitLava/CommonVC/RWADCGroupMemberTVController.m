...
@interface RWADCGroupMemberTVController ()
@property (nonatomic, strong)NSMutableArray *members;
@property (nonatomic, strong)NSMutableArray *selectedMembers;;
@end

@implementation RWADCGroupMemberTVController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    RWADCGroupMemberTVCell *cell = [tableView dequeueReusableCellWithIdentifier:@"memberSelect"];
    
    if (cell == nil) {
        cell = [[RWADCGroupMemberTVCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"memberSelect"];
        if (self.selectable) {
    ...
    cell.groupNameView.text = (user.friendAlias.length > 0 ? user.friendAlias : user.displayName);;
    if (self.selectable) {
