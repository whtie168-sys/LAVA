...
- (void)setUserInfo:(WFCCUserInfo *)userInfo {
    ...
    if (userInfo.displayName.length) {
        self.labelStr = (userInfo.friendAlias.length > 0 ? userInfo.friendAlias : userInfo.displayName);
    } else {
        self.labelStr = @"用户";
    }
}
...
- (void)setQrLogo:(NSString *)qrLogo {
    ...
    if(self.qrType == 1) {
        defaultPortrait = @"groupIcon";
    } else if(self.qrType == 2) {
        defaultPortrait = @"channel_default_portrait";
    }
    [self.logoImgView sd_setImageWithURL:[NSURL URLWithString:self.qrLogo] placeholderImage:[QWERImage imageNamed:@"groupIcon"]];
    [self.qrView addSubview:self.logoImgView];
}

- (void)setLabelStr:(NSString *)labelStr {
    _labelStr = labelStr;
    self.asoucNameLabel.text = labelStr;
}

- (UILabel *)asoucNameLabel {
    if (!_asoucNameLabel) {
        _asoucNameLabel = [[UILabel alloc] initWithFrame:CGRectMake(72, 28, self.qrView.bounds.size.width - 72 - 16, 22)];
        [self.qrView addSubview:_asoucNameLabel];
    }
