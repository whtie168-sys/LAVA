...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        self.asofaTextLabel = [[XLPageTitleLabel alloc] init];
        self.asofaTextLabel.textAlignment = NSTextAlignmentCenter;
        [self.contentView addSubview:self.asofaTextLabel];
...
- (void)layoutSubviews {
    ...
    self.asofaTextLabel.frame = self.bounds;
}

- (void)configCellOfSelected:(BOOL)selected {
    self.asofaTextLabel.textColor = selected ? self.config.titleSelectedColor : self.config.titleNormalColor;
    self.asofaTextLabel.font = selected ? self.config.titleSelectedFont : self.config.titleNormalFont;
    XLPageTitleLabel *label = (XLPageTitleLabel *)self.asofaTextLabel;
...
- (void)showAnimationOfProgress:(CGFloat)progress type:(XLPageTitleCellAnimationType)type {
    if (type == XLPageTitleCellAnimationTypeSelected) {
        self.asofaTextLabel.textColor = [XLPageViewControllerUtil colorTransformFrom:self.config.titleSelectedColor to:self.config.titleNormalColor progress:progress];
    }else if (type == XLPageTitleCellAnimationTypeWillSelected){
        self.asofaTextLabel.textColor = [XLPageViewControllerUtil colorTransformFrom:self.config.titleNormalColor to:self.config.titleSelectedColor progress:progress];
    }
}
