...
@interface ASECWReceiptVC () <XLPageViewControllerDelegate, XLPageViewControllerDataSrouce>
...
//    config.showTitleInNavigationBar = true;
    ...
    bounds.origin.y = [QWERUtilities wf_navigationFullHeight];
    bounds.size.height -= ([QWERUtilities wf_navigationFullHeight] + [QWERUtilities wf_safeDistanceBottom]);
...
- (UIViewController *)pageViewController:(XLPageViewController *)pageViewController viewControllerForIndex:(NSInteger)index {
    ASECWReadVC *vc = [[ASECWReadVC alloc] init];
    if(index == 0) {
