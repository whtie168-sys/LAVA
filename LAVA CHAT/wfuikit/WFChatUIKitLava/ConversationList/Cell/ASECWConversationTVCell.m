...
#import "ASECWConversationTVCell.h"
...
@interface ASECWConversationTVCell ()

@property (strong, nonatomic) UIView *asoucOnlineView; // 0129新增
@property (strong, nonatomic) WFCCUserInfo *userInfo; // 0129新增

@end


@implementation ASECWConversationTVCell
- (void)awakeFromNib {
    [super awakeFromNib];
...
- (void)updateChannelInfo:(WFCCChannelInfo *)channelInfo {
    ...
    [self.wsedcPotraitView sd_setImageWithURL:[NSURL URLWithString:[channelInfo.portrait stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]] placeholderImage:[QWERImage imageNamed:@"channel_default_portrait"] options:SDWebImageScaleDownLargeImages
                                      context:@{SDWebImageContextImageForceDecodePolicy : @(SDImageForceDecodePolicyNever), SDWebImageContextStoreCacheType : @(SDImageCacheTypeDisk)}];
...
- (void)updateGroupInfo:(WFCCGroupInfo *)groupInfo {
    ...
    if(groupInfo.type == GroupType_Organization) {
        if(groupInfo.portrait.length) {
            [self.wsedcPotraitView sd_setImageWithURL:[NSURL URLWithString:[groupInfo.portrait stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]] placeholderImage:[QWERImage imageNamed:@"organization_icon"] options:SDWebImageScaleDownLargeImages
                                              context:@{SDWebImageContextImageForceDecodePolicy : @(SDImageForceDecodePolicyNever), SDWebImageContextStoreCacheType : @(SDImageCacheTypeDisk)}];
        } else {
        ...
        if (groupInfo.portrait.length) { // 未传群头像 将会生成以9个用户头像组成的群头像、 WFCCIMService 里2277行被注视，实现未传群头像进行显示默认群头像的功能
            [self.wsedcPotraitView sd_setImageWithURL:[NSURL URLWithString:[groupInfo.portrait stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]] placeholderImage:[QWERImage imageNamed:@"groupIcon"] options:SDWebImageScaleDownLargeImages
                                              context:@{SDWebImageContextImageForceDecodePolicy : @(SDImageForceDecodePolicyNever), SDWebImageContextStoreCacheType : @(SDImageCacheTypeDisk)}];
        } else {
...
//            }];
//            if (path) {
//                [self.wsedcPotraitView sd_setImageWithURL:[NSURL fileURLWithPath:path] placeholderImage:[QWERImage imageNamed:@"groupIcon"]];
//            } else {
...
- (void)update:(WFCCConversation *)conversation {
    ...
    self.wsedcTargetLabel.textColor = [QWERConfigManager globalManager].textColor;
...
//        WFCCSecretChatInfo *secretInfo = [[WFCCIMService sharedWFCIMService] getSecretChatInfo:conversation.target];
    ...
    CGSize size = [QWERUtilities getTextDrawingSize:self.wsedcTargetLabel.text font:self.wsedcTargetLabel.font constrainedSize:CGSizeMake([UIScreen mainScreen].bounds.size.width - 76  - 68 - 24, 8000)];
    ...
    if(secretChatStateText) {
        ...
        NSMutableAttributedString *attString = [[NSMutableAttributedString alloc] initWithString:WFCString(@"[Dự thảo]") attributes:@{NSForegroundColorAttributeName : [UIColor redColor]}];
...
- (UIImageView *)wsedcStatusView {
    if (!_wsedcStatusView) {
        ...
        _wsedcStatusView.image = [QWERImage imageNamed:@"conversation_message_sending"];
...
- (UILabel *)wsedcTargetLabel {
    if (!_wsedcTargetLabel) {
        ...
        _wsedcTargetLabel.textColor = [QWERConfigManager globalManager].textColor;
...
- (UILabel *)offcialView {
    if(!_offcialView) {
        ...
        CGSize size = [QWERUtilities getTextDrawingSize:_offcialView.text font:_offcialView.font constrainedSize:CGSizeMake(200, 200)];
...
- (UIImageView *)secretChatView {
    if(!_secretChatView) {
        ...
        _secretChatView.image = [QWERImage imageNamed:@"secret_chat_icon"];
...
- (UIView *)asoucOnlineView {
    if (!_asoucOnlineView) {
        _asoucOnlineView = [[UIView alloc] initWithFrame:CGRectMake(CGRectGetMaxX(self.wsedcPotraitView.frame)-10, CGRectGetMaxY(self.wsedcPotraitView.frame)-10.0, 10.0, 10.0)];
        ...
        _asoucOnlineView.backgroundColor = RGBCOLOR(43, 221, 48);
        [self.contentView addSubview:_asoucOnlineView];
        [self.contentView bringSubviewToFront:_asoucOnlineView];
