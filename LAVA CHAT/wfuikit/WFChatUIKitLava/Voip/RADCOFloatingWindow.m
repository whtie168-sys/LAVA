...
@interface RADCOFloatingWindow () <WFAVCallSessionDelegate, RADCOConferenceManagerDelegate>

@property(nonatomic, strong) NSTimer *activeTimer;
@property(nonatomic, copy) void (^touchedBlock)(WFAVCallSession *callSession, RADCOConferenceInfo *conferenceInfo);
...
static RADCOFloatingWindow *staticWindow = nil;

static NSString *kFloatingWindowPosX = @"kFloatingWindowPosX";
static NSString *kFloatingWindowPosY = @"kFloatingWindowPosY";

@implementation RADCOFloatingWindow

+ (void)startCallFloatingWindow:(WFAVCallSession *)callSession focusUser:(WFAVParticipantProfile *)focusUserProfile
              withTouchedBlock:(void (^)(WFAVCallSession *callSession, RADCOConferenceInfo *conferenceInfo))touchedBlock {
    staticWindow = [[RADCOFloatingWindow alloc] init];
    ...
    if(self.callSession.isInAppScreenSharing) {
        ...
        if (self.callSession.state == kWFAVEngineStateIdle) {
            ...
            if (self.callSession.state == kWFAVEngineStateOutgoing) {
                ...
                [self.floatingButton setTitle:@"" forState:UIControlStateNormal];
                [self.floatingButton setImage:[QWERImage imageNamed:@"floatingaudio"]
            forState:UIControlStateNormal];
            }
        }
    }
...
- (void)updateVideoView {
    ...
    if ([focusUserId isEqualToString:[WFCCNetworkService sharedInstance].userId]) {
        isMuted = [Chat86AVEngineKit sharedEngineKit].currentSession.isVideoMuted;
    } else {
...
- (UIButton *)floatingButton {
    if (!_floatingButton) {
        ...
        _floatingButton.titleLabel.numberOfLines = 2;
        CGRect windowFrame = self.window.frame;
...
- (void)touchedWindow:(id)sender {
    ...
    if (self.touchedBlock) {
        self.touchedBlock(self.callSession, [RADCOConferenceManager sharedInstance].currentConferenceInfo);
    }
...
- (void)didCallEndWithReason:(WFAVCallEndReason)reason {
    if(self.callSession.isConference) {
        [[RADCOConferenceManager sharedInstance] addHistory:[RADCOConferenceManager sharedInstance].currentConferenceInfo duration:(int)([Chat86AVEngineKit sharedEngineKit].currentSession.endTime - [Chat86AVEngineKit sharedEngineKit].currentSession.startTime)];
    }
}
...
- (void)didMuteStateChanged:(NSArray<NSString *> *)userIds {
    if(![Chat86AVEngineKit sharedEngineKit].currentSession.isAudioOnly) {
        [self updateVideoView];
    }
}
