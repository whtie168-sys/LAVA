...
#import "RADCOHomeVC.h"
#import "RADCOStartConferenceVC.h"
#import "RADCOOrderConferenceVC.h"
...
#import "QWERImage.h"
#import "RADCOConferenceHistoryListVC.h"
#import "QWERUtilities.h"
#import "QWERConfigManager.h"

@interface RADCOHomeVC () <UITableViewDataSource, UITableViewDelegate>
...
@property(nonatomic, strong)NSArray<RADCOConferenceInfo *> *favConferences;
...
- (void)viewDidLoad {
    ...
    for (NSDictionary *dict in arrays) {
        RADCOConferenceInfo *info = [RADCOConferenceInfo fromDictionary:dict];
...
- (void)initTopPannel {
    ...
    CGFloat offset = [QWERUtilities wf_navigationFullHeight] + 32;
    ...
    [self.joinButton setImage:[QWERImage imageNamed:@"join_conference"] forState:UIControlStateNormal];
    ...
    [self.startButton setImage:[QWERImage imageNamed:@"start_conference"] forState:UIControlStateNormal];
    ...
    [self.orderButton setImage:[QWERImage imageNamed:@"order_conference"] forState:UIControlStateNormal];
    ...
    line.backgroundColor = [QWERConfigManager globalManager].separateColor;
...
- (void)reloadData {
    ...
    [[QWERConfigManager globalManager].appServiceProvider getFavConferences:^(NSArray<RADCOConferenceInfo *> * _Nonnull conferences) {
        ws.favConferences = conferences;
        NSMutableArray *arrays = [[NSMutableArray alloc] init];
        for (RADCOConferenceInfo *info in conferences) {
...
- (void)setFavConferences:(NSArray<RADCOConferenceInfo *> *)favConferences {
...
- (void)onStartBtn:(id)sender {
    RADCOStartConferenceVC *vc = [[RADCOStartConferenceVC alloc] init];
    vc.createResult = ^(RADCOConferenceInfo * _Nonnull conferenceInfo) {
        RADCOConferenceInfoVC *vc = [[RADCOConferenceInfoVC alloc] init];
...
- (void)onOrderBtn:(id)sender {
    RADCOOrderConferenceVC *vc = [[RADCOOrderConferenceVC alloc] init];
    UINavigationController *nav = [[UINavigationController alloc] initWithRootViewController:vc];
    nav.modalPresentationStyle = UIModalPresentationFullScreen;
    [self presentViewController:nav animated:YES completion:nil];
}
...
- (void)onHistoryButton:(id)sender {
    RADCOConferenceHistoryListVC *histVC = [[RADCOConferenceHistoryListVC alloc] init];
    [self.navigationController pushViewController:histVC animated:YES];
}

- (nonnull UITableViewCell *)tableView:(nonnull UITableView *)tableView cellForRowAtIndexPath:(nonnull NSIndexPath *)indexPath {
    RADCOConferenceInfo *info = self.favConferences[indexPath.row];
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    if(editingStyle == UITableViewCellEditingStyleDelete) {
        RADCOConferenceInfo *info = self.favConferences[indexPath.row];
        __weak typeof(self)ws = self;
        [[QWERConfigManager globalManager].appServiceProvider unfavConference:info.conferenceId success:^{
            [ws reloadData];
        } error:^(int errorCode, NSString * _Nonnull message) {
