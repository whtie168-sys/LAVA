...
@interface RADCOStartConferenceVC () <UITableViewDelegate, UITableViewDataSource>
...
@implementation RADCOStartConferenceVC
#if WFCU_SUPPORT_VOIP
- (void)viewDidLoad {
    [super viewDidLoad];
...
- (void)createConference:(BOOL)andJoin {
    ...
    RADCOConferenceInfo *info = [[RADCOConferenceInfo alloc] init];
    ...
    [[QWERConfigManager globalManager].appServiceProvider createConference:info success:^(NSString * _Nonnull conferenceId) {
        info.conferenceId = conferenceId;
        if(andJoin) {
            [ws stopProgress:hud finishText:nil];
            RADCOConferenceVC *vc = [[RADCOConferenceVC alloc] initWithConferenceInfo:info muteAudio:!self.enableAudio muteVideo:!self.enableVideo];
...
- (void)setupDateKeyPan {
    ...
    UIView *container = [[UIView alloc] initWithFrame:CGRectMake(0, bounds.size.height - pickerHeight - [QWERUtilities wf_safeDistanceBottom] - 34, bounds.size.width, pickerHeight + [QWERUtilities wf_safeDistanceBottom] + 34)];
    ...
    if(indexPath.section == 0) {
        ...
        if(indexPath.row == 0) {
            ...
            switchCell.onSwitch = ^(BOOL value, int type, void (^handleBlock)(BOOL success)) {
                ...
                DSRACGeneralSwitchTVCell *switchModeCell = [tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:1 inSection:2]];
        ...
        DSRACGeneralSwitchTVCell *cell = [[DSRACGeneralSwitchTVCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:@"switch2"];
        ...
        DSRACGeneralSwitchTVCell *cell = [tableView dequeueReusableCellWithIdentifier:@"switch"];
        if (cell == nil) {
            cell = [[DSRACGeneralSwitchTVCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:@"switch"];
        }
        if(self.userPrivateConferenceId) {
            self.advanceConference = NO;
            cell.valueSwitch.enabled = NO;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if(indexPath.section == 0) {
        if(indexPath.row == 0) {
            RWADCGeneralModifyVC *vc = [[RWADCGeneralModifyVC alloc] init];
