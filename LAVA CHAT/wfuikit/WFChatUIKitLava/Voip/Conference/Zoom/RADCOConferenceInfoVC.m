...
@interface RADCOConferenceInfoVC () <UITableViewDelegate, UITableViewDataSource>
...
@implementation RADCOConferenceInfoVC
#if WFCU_SUPPORT_VOIP
- (void)viewDidLoad {
    [super viewDidLoad];
    ...
    [[QWERConfigManager globalManager].appServiceProvider queryConferenceInfo:self.conferenceId password:self.password success:^(RADCOConferenceInfo * _Nonnull conferenceInfo) {
    ...
    [[QWERConfigManager globalManager].appServiceProvider isFavConference:self.conferenceId success:^(BOOL isFav) {
...
- (void)setConferenceInfo:(RADCOConferenceInfo *)conferenceInfo {
...
- (void)onDestroyBtn:(id)sender {
    ...
    [[QWERConfigManager globalManager].appServiceProvider destroyConference:self.conferenceId success:^{
...
- (void)onDeleteBtn:(id)sender {
    ...
    [[QWERConfigManager globalManager].appServiceProvider unfavConference:self.conferenceId success:^{
...
- (void)onStartConference:(id)sender {
    ...
    RADCOConferenceVC *vc = [[RADCOConferenceVC alloc] initWithConferenceInfo:self.conferenceInfo muteAudio:!self.enableAudio muteVideo:!self.enableVideo];
...
- (void)onFav:(id)sender {
    ...
    [[QWERConfigManager globalManager].appServiceProvider favConference:self.conferenceInfo.conferenceId success:^{
...
//    UIPasteboard *pasteboard = [UIPasteboard generalPasteboard];
//    pasteboard.string = [[RADCOConferenceManager sharedInstance] linkFromConferenceId:self.conferenceInfo.conferenceId password:self.conferenceInfo.password];
//    [self.view makeToast:@"已拷贝到剪贴板！" duration:1 position:CSToastPositionCenter];
}
...
- (void)showConferenceQrCode {
    ...
    vc.conferenceUrl = [[RADCOConferenceManager sharedInstance] linkFromConferenceId:self.conferenceInfo.conferenceId password:self.conferenceInfo.password];
    ...
    if(indexPath.section == 0) {
        ...
        if(indexPath.row == 0) {
            ...
            if([[WFCCNetworkService sharedInstance].userId isEqualToString:self.conferenceInfo.owner]) {
                ...
                titleCell.detailTextLabel.text = (userInfo.friendAlias.length > 0 ? userInfo.friendAlias : userInfo.displayName);
            }
        ...
        __weak typeof(self)ws = self;
        DSRACGeneralSwitchTVCell *switchCell = [[DSRACGeneralSwitchTVCell alloc] init];
        if(indexPath.row == 0) {
