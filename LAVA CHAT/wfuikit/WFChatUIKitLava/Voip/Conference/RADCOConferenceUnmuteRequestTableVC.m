//
//  RADCOConferenceUnmuteRequestTableVC.m
//  WFChatUIKit
//
...
@interface RADCOConferenceUnmuteRequestTableVC () <UITableViewDataSource, UITableViewDelegate>
...
@implementation RADCOConferenceUnmuteRequestTableVC
...
- (void)viewDidLoad {
    ...
    self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, self.view.bounds.size.width, bounds.size.height - [QWERUtilities wf_safeDistanceBottom] - buttonHeight - 16)];
    ...
    if([RADCOConferenceManager sharedInstance].applyingUnmuteMembers.count) {
        self.acceptAllBtn = [self createBtn:CGRectMake(16, bounds.size.height - [QWERUtilities wf_safeDistanceBottom] - buttonHeight - 16, buttonWidth, buttonHeight) title:@"全部同意" action:@selector(onAcceptAllBtnPressed:)];
        self.rejectAllBtn = [self createBtn:CGRectMake(bounds.size.width - 16 - buttonWidth, bounds.size.height - [QWERUtilities wf_safeDistanceBottom] - buttonHeight - 16, buttonWidth, buttonHeight) title:@"全部拒绝" action:@selector(onRejectAllBtnPressed:)];
    }
}

- (void)onAcceptAllBtnPressed:(id)sender {
    [[RADCOConferenceManager sharedInstance] approveAllMemberUnmute:YES];
    [self.tableView reloadData];
}

- (void)onRejectAllBtnPressed:(id)sender {
    [[RADCOConferenceManager sharedInstance] approveAllMemberUnmute:NO];
    [self.tableView reloadData];
}
...
- (void)onAcceptBtn:(UIButton *)sender {
    ...
    NSString *userId = [RADCOConferenceManager sharedInstance].applyingUnmuteMembers[row];
    [[RADCOConferenceManager sharedInstance] approveMember:userId unmute:YES];
    [self.tableView reloadData];
}
...
- (void)onRejectBtn:(UIButton *)sender {
    int row = (int)sender.tag;
    NSString *userId = [RADCOConferenceManager sharedInstance].applyingUnmuteMembers[row];
    [[RADCOConferenceManager sharedInstance] approveMember:userId unmute:NO];
    [self.tableView reloadData];
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return [RADCOConferenceManager sharedInstance].applyingUnmuteMembers.count;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *userId = [RADCOConferenceManager sharedInstance].applyingUnmuteMembers[indexPath.row];
    WFCCUserInfo *userInfo = [[WFCCIMService sharedWFCIMService] getUserInfo:userId refresh:NO];
    cell.textLabel.text = userInfo.friendAlias.length?userInfo.friendAlias:userInfo.displayName;
    [cell.imageView sd_setImageWithURL:[NSURL URLWithString:userInfo.portrait] placeholderImage:[QWERImage imageNamed:@"PersonalChat"] options:SDWebImageScaleDownLargeImages
