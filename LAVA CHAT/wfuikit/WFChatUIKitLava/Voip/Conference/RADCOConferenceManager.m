...
#import "RADCOConferenceChangeModelContent.h"
...
#import "RADCOBroadcastDefine.h"

NSString *kMuteStateChanged = @"kMuteStateChanged";


@interface RADCOConferenceManager () <GCDAsyncSocketDelegate, WFAVExternalVideoSource>
...
static RADCOConferenceManager *sharedSingleton = nil;
@implementation RADCOConferenceManager
+ (RADCOConferenceManager *)sharedInstance {
    if (sharedSingleton == nil) {
        @synchronized (self) {
            if (sharedSingleton == nil) {
                sharedSingleton = [[RADCOConferenceManager alloc] init];
...
- (void)muteAudio:(BOOL)mute {
    if(mute) {
        if(![Chat86AVEngineKit sharedEngineKit].currentSession.isAudience && [Chat86AVEngineKit sharedEngineKit].currentSession.isVideoMuted && ![[Chat86AVEngineKit sharedEngineKit].currentSession isBroadcasting]) {
            if(![[Chat86AVEngineKit sharedEngineKit].currentSession switchAudience:YES]) {
                NSLog(@"switch to audience failure");
                return;
            }
        }
        [[Chat86AVEngineKit sharedEngineKit].currentSession muteAudio:mute];
    } else {
        if([Chat86AVEngineKit sharedEngineKit].currentSession.isAudience && ![[Chat86AVEngineKit sharedEngineKit].currentSession canSwitchAudience]) {
            NSLog(@"can not switch to participater");
            return;
        }
        
        if([Chat86AVEngineKit sharedEngineKit].currentSession.isAudience && self.currentConferenceInfo.maxParticipants > 0) {
            __block int participantCount = 0;
            [[Chat86AVEngineKit sharedEngineKit].currentSession.participants enumerateObjectsUsingBlock:^(WFAVParticipantProfile * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                if(!obj.audience) {
                    participantCount++;
                }
            }];
        ...
        if([Chat86AVEngineKit sharedEngineKit].currentSession.isAudience) {
            [[Chat86AVEngineKit sharedEngineKit].currentSession muteVideo:YES];
            [[Chat86AVEngineKit sharedEngineKit].currentSession switchAudience:NO];
        }
    }
...
- (void)muteVideo:(BOOL)mute {
    if(mute) {
        if(![Chat86AVEngineKit sharedEngineKit].currentSession.isAudience && [Chat86AVEngineKit sharedEngineKit].currentSession.isAudioMuted && ![[Chat86AVEngineKit sharedEngineKit].currentSession isBroadcasting]) {
            [[Chat86AVEngineKit sharedEngineKit].currentSession switchAudience:YES];
        }
        [[Chat86AVEngineKit sharedEngineKit].currentSession muteVideo:mute];
    } else {
        if([Chat86AVEngineKit sharedEngineKit].screenSharingReplaceMode && [self isBroadcasting]) {
            return;
        }
        if([Chat86AVEngineKit sharedEngineKit].currentSession.isAudience && self.currentConferenceInfo.maxParticipants > 0) {
            __block int participantCount = 0;
        ...
        if([Chat86AVEngineKit sharedEngineKit].currentSession.isAudience) {
            [[Chat86AVEngineKit sharedEngineKit].currentSession muteAudio:YES];
            [[Chat86AVEngineKit sharedEngineKit].currentSession switchAudience:NO];
        }
    }
    [self notifyMuteStateChanged];
...
- (void)enableAudioDisableVideo {
    [[Chat86AVEngineKit sharedEngineKit].currentSession muteVideo:YES];
    [[Chat86AVEngineKit sharedEngineKit].currentSession muteAudio:NO];
    if([Chat86AVEngineKit sharedEngineKit].currentSession.isAudience) {
        [[Chat86AVEngineKit sharedEngineKit].currentSession switchAudience:NO];
    }
    [self notifyMuteStateChanged];
...
- (void)reloadConferenceInfo {
    ...
    [[QWERConfigManager globalManager].appServiceProvider queryConferenceInfo:self.currentConferenceInfo.conferenceId password:self.currentConferenceInfo.password success:^(RADCOConferenceInfo * _Nonnull conferenceInfo) {
...
- (void)setCurrentConferenceInfo:(RADCOConferenceInfo *)currentConferenceInfo {
...
- (void)leaveConference:(BOOL)destroy {
    if([[Chat86AVEngineKit sharedEngineKit].currentSession isBroadcasting]) {
        [self cancelBroadcast];
    }
    
    [[Chat86AVEngineKit sharedEngineKit].currentSession leaveConference:NO];
    
    if(destroy) {
        [[QWERConfigManager globalManager].appServiceProvider  destroyConference:[Chat86AVEngineKit sharedEngineKit].currentSession.callId success:^{
...
- (UIButton *)alertViewCheckBtn {
    if(!_alertViewCheckBtn) {
        ...
        [_alertViewCheckBtn setImage:[QWERImage imageNamed:@"multi_unselected"] forState:UIControlStateNormal];
        [_alertViewCheckBtn setImage:[QWERImage imageNamed:@"multi_selected"] forState:UIControlStateSelected];
...
- (void)requestRecording:(BOOL)recording {
    __weak typeof(self)ws = self;
    [[QWERConfigManager globalManager].appServiceProvider recordConference:self.currentConferenceInfo.conferenceId record:recording success:^{
...
- (void)onReceiveMessages:(NSNotification *)notification {
    ...
    if([Chat86AVEngineKit sharedEngineKit].currentSession.state == kWFAVEngineStateConnected && [Chat86AVEngineKit sharedEngineKit].currentSession.isConference) {
        for (WFCCMessage *msg in messages) {
            if([msg.content isKindOfClass:[RADCOConferenceChangeModelContent class]]) {
                RADCOConferenceChangeModelContent *changeModelCnt = (RADCOConferenceChangeModelContent *)msg.content;
                if([changeModelCnt.conferenceId isEqualToString:[Chat86AVEngineKit sharedEngineKit].currentSession.callId]) {
            ...
            } else if([msg.content isKindOfClass:[RADCOConferenceCommandContent class]]) {
                RADCOConferenceCommandContent *command = (RADCOConferenceCommandContent *)msg.content;
                if([command.conferenceId isEqualToString:[Chat86AVEngineKit sharedEngineKit].currentSession.callId]) {
                    switch (command.type) {
...
- (void)request:(NSString *)userId changeModel:(BOOL)isAudience inConference:(NSString *)conferenceId {
    RADCOConferenceChangeModelContent *cnt = [[RADCOConferenceChangeModelContent alloc] init];
...
- (void)addHistory:(RADCOConferenceInfo *)info duration:(int)duration {
    ...
    NSMutableArray<RADCOConferenceHistory *> *conferenceHistorys = [[self getConferenceHistoryList] mutableCopy];
    for (RADCOConferenceHistory *his in conferenceHistorys) {
    ...
    [conferenceHistorys enumerateObjectsUsingBlock:^(RADCOConferenceHistory * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (NSArray<RADCOConferenceHistory *> *)getConferenceHistoryList {
    ...
    if([o isKindOfClass:NSArray.class]) {
        ...
        [arr enumerateObjectsUsingBlock:^(NSDictionary * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            [ret addObject:[RADCOConferenceHistory fromDictionary:obj]];
        }];
    }
    return ret;
}
...
- (void)sendCommandMessage:(RADCOConferenceCommandContent *)commandContent {
...
- (BOOL)requestMuteAll:(BOOL)allowMemberUnmute {
    ...
    [[QWERConfigManager globalManager].appServiceProvider updateConference:self.currentConferenceInfo success:^() {
...
- (BOOL)requestUnmuteAll:(BOOL)unmute {
    ...
    [[QWERConfigManager globalManager].appServiceProvider updateConference:self.currentConferenceInfo success:^(void) {
...
- (BOOL)requestFocus:(NSString *)focusedUserId {
    ...
    __weak typeof(self)ws = self;
    [[QWERConfigManager globalManager].appServiceProvider focusConference:self.currentConferenceInfo.conferenceId userId:focusedUserId success:^{
...
- (BOOL)requestCancelFocus {
    ...
    __weak typeof(self)ws = self;
    [[QWERConfigManager globalManager].appServiceProvider focusConference:self.currentConferenceInfo.conferenceId userId:nil success:^{
...
- (void)sendCommandMessage:(WFCUConferenceCommandType)type targetUserId:(NSString *)userId boolValue:(BOOL)boolValue {
    RADCOConferenceCommandContent *command = [RADCOConferenceCommandContent commandOfType:type conference:self.currentConferenceInfo.conferenceId];
...
- (BOOL)isBroadcasting {
    return [Chat86AVEngineKit sharedEngineKit].currentSession.isBroadcasting;
}

- (void)onBroadcastStarted {
    [[Chat86AVEngineKit sharedEngineKit].currentSession setBroadcastingWithVideoSource:self];
...
- (void)onBroadcastStoped {
    ...
    [[Chat86AVEngineKit sharedEngineKit].currentSession setBroadcastingWithVideoSource:nil];
