...
@interface REDSACreateGroupVC () <UIImagePickerControllerDelegate, UIActionSheetDelegate, UINavigationControllerDelegate>
...
@implementation REDSACreateGroupVC
...
- (void)viewDidLoad {
    ...
    self.trewqPortraitView = [[UIImageView alloc] initWithFrame:CGRectMake((bound.size.width - portraitWidth)/2, 100, portraitWidth, portraitWidth)];
    self.trewqPortraitView.image = [QWERImage imageNamed:@"groupIcon"];
  ...
  if (!_isModifyPortrait) {
    ...
    UIImageView *trewqPortraitView = [[UIImageView alloc] initWithFrame:self.combineHeadView.bounds];
    WFCCGroupInfo *groupInfo = [[WFCCIMService sharedWFCIMService] getGroupInfo:self.groupId refresh:NO];
    [trewqPortraitView sd_setImageWithURL:[NSURL URLWithString:[groupInfo.portrait stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]] placeholderImage:[QWERImage imageNamed:@"PersonalChat"]];
    [self.combineHeadView addSubview:trewqPortraitView];
  }
}
    ...
    CGFloat width = (PortraitWidth - padding) / numPerRow - padding;
        ...
        for (int i = 0; i < row; i++) {
            ...
            for (int j = 0; j < c; j++) {
                ...
                [imageView sd_setImageWithURL:[NSURL URLWithString:[user.portrait stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]] placeholderImage:[QWERImage imageNamed:@"PersonalChat"]];
...
didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if ([mediaType isEqual:@"public.image"]) {
        ...
        //获取截取区域的图像
        UIImage *captureImage = [QWERUtilities thumbnailWithImage:originImage maxSize:CGSizeMake(60, 60)];
        self.trewqPortraitView.image = captureImage;
...
- (UIImage *)getPortraitImage {
    if (self.trewqPortraitView.hidden == NO) {
        return self.trewqPortraitView.image;
    } else {
