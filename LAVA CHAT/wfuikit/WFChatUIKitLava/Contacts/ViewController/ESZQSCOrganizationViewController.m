//
//  ESZQSCOrganizationViewController.m
//  WFChat UIKit
//
...
@interface ESZQSCOrganizationViewController () <UITableViewDataSource, UISearchControllerDelegate, UITableViewDelegate, UITableViewDataSource, UISearchResultsUpdating, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout>
{
    BOOL _isChinese;
}
...
@property(nonatomic, strong)NSArray<ESZQSCEmployee *> *searchResults;
@end

@implementation ESZQSCOrganizationViewController
...
- (void)viewDidLoad {
    ...
    if (self.selectContact) {
        ...
        UIBarButtonItem *close = [[UIBarButtonItem alloc] initWithImage:[QWERImage imageNamed:@"close"] style:UIBarButtonItemStylePlain target:self action:@selector(onLeftBarBtn:)];
    ...
    [self.searchController.searchBar setPlaceholder:(_isChinese?@"Đang tìm kiếm…":@"Đang tìm kiếm…")];
...
- (void)onOrganizationExUpdated:(NSNotification *)notification {
    ...
    if(orgId == self.lastOrganizationId) {
        ESZQSCOrganizationEx * path = [[ESZQSCOrganizationCache sharedCache] getOrganizationEx:self.lastOrganizationId refresh:NO];
        ESZQSCOrganizationEx *p = [self.paths lastObject];
...
- (void)setOrganizationIds:(NSArray<NSNumber *> *)organizationIds {
    ...
    [organizationIds enumerateObjectsUsingBlock:^(NSNumber * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ESZQSCOrganizationEx *path = [[ESZQSCOrganizationEx alloc] init];
...
- (void)loadData {
    ESZQSCOrganization *roganization = [[ESZQSCOrganizationCache sharedCache] getOrganization:self.lastOrganizationId refresh:YES];
    ...
    [[ESZQSCOrganizationCache sharedCache] getOrganizationEx:self.lastOrganizationId refresh:YES success:^(NSInteger organizationId, ESZQSCOrganizationEx * _Nonnull ex) {
        [hud hideAnimated:NO];
...
- (NSString *)getOrganizationNameOfIndexPath:(NSIndexPath *)indexPath {
    ...
    if(indexPath.row > 0) {
        ESZQSCOrganizationEx *path = self.paths[indexPath.row - 1];
        if(path.organization.name.length) {
            name = path.organization.name;
        } else {
            ESZQSCOrganization *org = [[ESZQSCOrganizationCache sharedCache] getOrganization:path.organizationId refresh:NO];
            if(org.name.length) {
                name = org.name;
            } else {
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CGSize size = [QWERUtilities getTextDrawingSize:name font:[UIFont systemFontOfSize:16] constrainedSize:CGSizeMake(10000, 20)];
    ...
    if(!lastItem) {
        ...
        iv.image = [QWERImage imageNamed:@"back_normal"];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    ESZQSCOrganizationEx *path = [self.paths lastObject];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if(self.searchController.active) {
        ESZQSCEmployee *emp = self.searchResults[indexPath.row];
        FRSDAContactTVCell *contactCell = [self dequeueOrAllocContactCell:tableView];
        contactCell.asoucNameLabel.text = emp.name;
        
        [contactCell.trewqPortraitView sd_setImageWithURL:[NSURL URLWithString:[emp.portraitUrl stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]] placeholderImage: [QWERImage imageNamed:@"employee"] options:SDWebImageScaleDownLargeImages
        ...
        if(path.subOrganizations.count && indexPath.section == 0) {
            ESZQSCOrganization *org = path.subOrganizations[indexPath.row];
            FRSDAContactTVCell *contactCell = [self dequeueOrAllocOrganizationCell:tableView];
            contactCell.asoucNameLabel.text = [NSString stringWithFormat:@"%@(%d)", org.name, org.memberCount];
            contactCell.imageView.image = [QWERImage imageNamed:@"organization_icon"];
            cell = contactCell;
        } else {
            ESZQSCEmployee *emp = path.employees[indexPath.row];
            FRSDAContactTVCell *contactCell = [self dequeueOrAllocContactCell:tableView];
            contactCell.asoucNameLabel.text = emp.name;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if(self.searchController.active) {
        ...
        if(path.subOrganizations.count && indexPath.section == 0) {
            ESZQSCOrganization *org = path.subOrganizations[indexPath.row];
            ESZQSCOrganizationEx *newPath = [[ESZQSCOrganizationEx alloc] init];
...
-(void)updateSearchResultsForSearchController:(UISearchController *)searchController {
    if (searchController.active) {
        ...
        if(searchString.length) {
            [[QWERConfigManager globalManager].orgServiceProvider searchEmployee:self.lastOrganizationId keyword:searchString success:^(NSArray<ESZQSCEmployee *> * _Nonnull employees) {
