...
#import "GFDSAFriendRequestVC.h"
#import "UITabBar+badge.h"
#import "FRSDANewFriendTVCell.h"
#import "ATERWAddFriendVC.h"
#import "MBProgressHUD.h"
#import "AUETAFavChannelTableVC.h"
...
#import "ESZQSCOrganizationViewController.h"
#import "ESZQSCOrgRelationship.h"

@interface FRSDAContactListVC () <UITableViewDataSource, UISearchControllerDelegate, UITableViewDelegate, UITableViewDataSource, UISearchResultsUpdating>
...
@implementation FRSDAContactListVC
...
- (void)viewDidLoad {
    ...
    if (self.selectContact) {
      ...
      self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[QWERImage imageNamed:@"nav_add_friend"] style:UIBarButtonItemStyleDone target:self action:@selector(onRightBarBtn:)];
    }
    ...
    if (@available(iOS 13, *)) {
        ...
        self.searchController.searchBar.searchTextField.backgroundColor = [QWERConfigManager globalManager].naviBackgroudColor;
...
- (void)updateRightBarBtn {
    if(self.selectedContacts.count == 0) {
        ...
        if (self.multiSelect && self.maxSelectCount > 1) {
            self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:[NSString stringWithFormat:@"%@(%d/%d)", @"确定",  (int)self.selectedContacts.count, self.maxSelectCount] style:UIBarButtonItemStyleDone target:self action:@selector(onRightBarBtn:)];
        } else {
            self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:[NSString stringWithFormat:@"%@(%d)", @"确定",  (int)self.selectedContacts.count] style:UIBarButtonItemStyleDone target:self action:@selector(onRightBarBtn:)];
        }
    }
}
...
- (void)sortAndRefreshWithList:(NSArray *)friendList {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        self.resultDic = [FRSDAContactListVC sortedArrayWithPinYinDic:friendList];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if (self.searchController.active || self.selectContact) {
        ...
        if (section == 0) {
            return 3 + [ESZQSCOrganizationCache sharedCache].rootOrganizationIds.count + [ESZQSCOrganizationCache sharedCache].bottomOrganizationIds.count;
        } else {
            dataSource = self.allFriendSectionDic[self.allKeys[section - 1]];
            return dataSource.count;
        }
...
- (FRSDAContactTVCell *)dequeueOrAllocContactCell:(UITableView *)tableView {
    FRSDAContactTVCell *contactCell = [tableView dequeueReusableCellWithIdentifier:REUSEIDENTIFY];
    if (contactCell == nil) {
        contactCell = [[FRSDAContactTVCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:REUSEIDENTIFY];
        contactCell.separatorInset = UIEdgeInsetsMake(0, 68, 0, 0);
    }
    return contactCell;
}
#define FAVGROUP_REUSEIDENTIFY @"favGroupCell"
- (FRSDAContactTVCell *)dequeueOrAllocFavGroupCell:(UITableView *)tableView {
    FRSDAContactTVCell *contactCell = [tableView dequeueReusableCellWithIdentifier:FAVGROUP_REUSEIDENTIFY];
    if (contactCell == nil) {
        contactCell = [[FRSDAContactTVCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:FAVGROUP_REUSEIDENTIFY];
        contactCell.separatorInset = UIEdgeInsetsMake(0, 68, 0, 0);
    }
    return contactCell;
}
#define CHANNEL_REUSEIDENTIFY @"channelCell"
- (FRSDAContactTVCell *)dequeueOrAllocChannelCell:(UITableView *)tableView {
    FRSDAContactTVCell *contactCell = [tableView dequeueReusableCellWithIdentifier:CHANNEL_REUSEIDENTIFY];
    if (contactCell == nil) {
        contactCell = [[FRSDAContactTVCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CHANNEL_REUSEIDENTIFY];
        contactCell.separatorInset = UIEdgeInsetsMake(0, 68, 0, 0);
    }
    return contactCell;
}
#define NEWFRIEND_REUSEIDENTIFY @"newFriendCell"
- (FRSDANewFriendTVCell *)dequeueOrAllocNewFriendCell:(UITableView *)tableView {
    FRSDANewFriendTVCell *contactCell = [tableView dequeueReusableCellWithIdentifier:NEWFRIEND_REUSEIDENTIFY];
    if (contactCell == nil) {
        contactCell = [[FRSDANewFriendTVCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:NEWFRIEND_REUSEIDENTIFY];
        contactCell.separatorInset = UIEdgeInsetsMake(0, 68, 0, 0);
    }
    return contactCell;
}
#define SELECT_REUSEIDENTIFY @"resueSelectCell"
- (FRSDAContactSelectTVCell *)dequeueOrAllocSelectContactCell:(UITableView *)tableView {
    FRSDAContactSelectTVCell *selectCell = [tableView dequeueReusableCellWithIdentifier:SELECT_REUSEIDENTIFY];
    if (selectCell == nil) {
        selectCell = [[FRSDAContactSelectTVCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:SELECT_REUSEIDENTIFY];
...
- (FRSDAContactTVCell *)dequeueOrAllocOrganizationCell:(UITableView *)tableView {
    FRSDAContactTVCell *contactCell = [tableView dequeueReusableCellWithIdentifier:ORGANIZATION_REUSEIDENTIFY];
    if (contactCell == nil) {
        contactCell = [[FRSDAContactTVCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ORGANIZATION_REUSEIDENTIFY];
        contactCell.separatorInset = UIEdgeInsetsMake(0, 68, 0, 0);
    }
    return contactCell;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.searchController.active || self.selectContact) {
        ...
        if (indexPath.section == 0) {
            if (indexPath.row == 0) {
                FRSDANewFriendTVCell *contactCell = [self dequeueOrAllocNewFriendCell:tableView];
                
                contactCell.asoucNameLabel.text = WFCString(@"NewFriend");
                contactCell.trewqPortraitView.image = [QWERImage imageNamed:@"friend_request_icon"];
                [contactCell refresh];
                contactCell.separatorInset = UIEdgeInsetsMake(0, 60, 0, 0);
                
                contactCell.asoucNameLabel.textColor = [QWERConfigManager globalManager].textColor;
                return contactCell;
            } else if(indexPath.row == 1) {
                FRSDAContactTVCell *contactCell = [self dequeueOrAllocFavGroupCell:tableView];
                contactCell.separatorInset = UIEdgeInsetsMake(0, 60, 0, 0);
                contactCell.asoucNameLabel.text = WFCString(@"Group");
                contactCell.trewqPortraitView.image = [QWERImage imageNamed:@"groupIcon"];
                contactCell.asoucNameLabel.textColor = [QWERConfigManager globalManager].textColor;
                contactCell.asoucOnlineView.hidden = YES;
                return contactCell;
            } else if(indexPath.row == 2) {
                FRSDAContactTVCell *contactCell = [self dequeueOrAllocChannelCell:tableView];
                
                contactCell.asoucNameLabel.text = @"频道";
                contactCell.trewqPortraitView.image = [QWERImage imageNamed:@"contact_channel_icon"];
                contactCell.asoucNameLabel.textColor = [QWERConfigManager globalManager].textColor;
                contactCell.asoucOnlineView.hidden = YES;
                return contactCell;
            } else {
                NSInteger index = indexPath.row - 3;
                FRSDAContactTVCell *contactCell = [self dequeueOrAllocOrganizationCell:tableView];
                if(index < [ESZQSCOrganizationCache sharedCache].rootOrganizationIds.count) {
                    int orgId = [[ESZQSCOrganizationCache sharedCache].rootOrganizationIds[index] intValue];
                    ESZQSCOrganization *organization = [[ESZQSCOrganizationCache sharedCache] getOrganization:orgId refresh:NO];
                    contactCell.asoucNameLabel.text = organization.name;
                    contactCell.trewqPortraitView.image = [QWERImage imageNamed:@"contact_organization_icon"];
                    contactCell.asoucNameLabel.textColor = [QWERConfigManager globalManager].textColor;
                    contactCell.asoucOnlineView.hidden = YES;
                    return contactCell;
                } else {
                    index -= [ESZQSCOrganizationCache sharedCache].rootOrganizationIds.count;
                    int orgId = [[ESZQSCOrganizationCache sharedCache].bottomOrganizationIds[index] intValue];
                    ESZQSCOrganization *organization = [[ESZQSCOrganizationCache sharedCache] getOrganization:orgId refresh:NO];
                    contactCell.asoucNameLabel.text = organization.name;
                    contactCell.trewqPortraitView.image = [QWERImage imageNamed:@"contact_expended_icon"];
                    contactCell.asoucNameLabel.textColor = [QWERConfigManager globalManager].textColor;
                    contactCell.asoucOnlineView.hidden = YES;
                    return contactCell;
                }
    ...
    if (self.selectContact) {
        if (self.multiSelect && !self.withoutCheckBox) {
            FRSDAContactSelectTVCell *selectCell = [self dequeueOrAllocSelectContactCell:tableView];
            WFCCUserInfo *userInfo = dataSource[indexPath.row];
            [selectCell showAsoucFriendUid:userInfo.userId];
            ...
            selectCell.asoucNameLabel.textColor = [QWERConfigManager globalManager].textColor;
            cell = selectCell;
        } else {
            FRSDAContactTVCell *selectCell = [self dequeueOrAllocContactCell:tableView];
        ...
        if (indexPath.section == 0 && !self.searchController.active) {
            ...
            FRSDAContactTVCell *contactCell = [self dequeueOrAllocContactCell:tableView];
            WFCCUserInfo *userInfo = dataSource[indexPath.row];
    ...
    if (self.searchController.active || self.selectContact) {
        ...
        if (indexPath.section == 0) {
            if (indexPath.row == 0) {
                ...
                TREWQFavGroupTableVC *groupVC = [[TREWQFavGroupTableVC alloc] init];;
                groupVC.hidesBottomBarWhenPushed = YES;
                [self.navigationController pushViewController:groupVC animated:YES];
            } else if(indexPath.row == 2) {
                AUETAFavChannelTableVC *channelVC = [[AUETAFavChannelTableVC alloc] init];;
                ...
                if(index < [ESZQSCOrganizationCache sharedCache].rootOrganizationIds.count) {
                    int orgId = [[ESZQSCOrganizationCache sharedCache].rootOrganizationIds[index] intValue];
                    ESZQSCOrganizationViewController *orgVC = [[ESZQSCOrganizationViewController alloc] init];
                    ...
                    index -= [ESZQSCOrganizationCache sharedCache].rootOrganizationIds.count;
                    int orgId = [[ESZQSCOrganizationCache sharedCache].bottomOrganizationIds[index] intValue];
                    NSArray<ESZQSCOrgRelationship *> *rs = [[ESZQSCOrganizationCache sharedCache] getRelationship:[WFCCNetworkService sharedInstance].userId refresh:NO];
                    __block NSInteger index = orgId;
                    NSMutableArray *ids = [[NSMutableArray alloc] init];
                    while (index) {
                        [ids insertObject:@(index) atIndex:0];
                        __block BOOL has = NO;
                        [rs enumerateObjectsUsingBlock:^(ESZQSCOrgRelationship * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
+ (NSMutableDictionary *)sortedArrayWithPinYinDic:(NSArray *)userList {
    ...
    [infoDic enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
        ...
        [_tempOtherArr sortUsingComparator:^NSComparisonResult(id  _Nonnull obj1, id  _Nonnull obj2) {
            ...
            NSString *user1Pinyin = [FRSDAContactListVC hanZiToPinYinWithString:user1.displayName];
            NSString *user2Pinyin = [FRSDAContactListVC hanZiToPinYinWithString:user2.displayName];
