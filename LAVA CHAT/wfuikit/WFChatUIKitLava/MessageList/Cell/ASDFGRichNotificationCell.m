...
@interface ASDFGRichNotificationCell ()
...
@property(nonatomic, strong)UIImageView *extrewqPortraitView;
@end
//CELL左右的margin
...
@implementation ASDFGRichNotificationCell
+ (CGSize)sizeForCell:(QWERTMessageModel *)msgModel withViewWidth:(CGFloat)width {
    ...
    CGSize titleSize = [QWERUtilities getTextDrawingSize:content.title font:[UIFont systemFontOfSize:TITLE_FONT_SIZE] constrainedSize:CGSizeMake(containerWidth-CELL_PADDING-CELL_PADDING, 50)];
    
    
    CGSize descSize = [QWERUtilities getTextDrawingSize:content.desc font:[UIFont systemFontOfSize:FONT_SIZE] constrainedSize:CGSizeMake(containerWidth-CELL_PADDING-CELL_PADDING, 50)];
    
    CGFloat itemsHeight = 0;
    for (NSDictionary<NSString*, NSString*> *data in content.datas) {
        NSString *value = data[@"value"];
        CGSize itemSize = [QWERUtilities getTextDrawingSize:value font:[UIFont systemFontOfSize:FONT_SIZE] constrainedSize:CGSizeMake(containerWidth-CELL_PADDING-CELL_PADDING-VALUE_BOARD_PADDING_LEFT, 50)];
    ...
    CGSize remarkSize = [QWERUtilities getTextDrawingSize:content.remark font:[UIFont systemFontOfSize:FONT_SIZE] constrainedSize:CGSizeMake(containerWidth-CELL_PADDING-CELL_PADDING, 50)];
...
- (void)setModel:(QWERTMessageModel *)model {
    ...
    if(content.exName.length) {
        ...
        [self.extrewqPortraitView sd_setImageWithURL:[NSURL URLWithString:content.exPortrait] placeholderImage:[QWERImage imageNamed:@"default_app_icon"] options:SDWebImageScaleDownLargeImages
...
- (CGFloat)addValueLabel:(CGFloat)offset text:(NSString *)text color:(NSString *)colorStr {
    ...
    CGFloat containerWidth = [UIScreen mainScreen].bounds.size.width - CELL_MARGIN - CELL_MARGIN;
    CGSize itemSize = [QWERUtilities getTextDrawingSize:text font:[UIFont systemFontOfSize:FONT_SIZE] constrainedSize:CGSizeMake(containerWidth-CELL_PADDING-CELL_PADDING-VALUE_BOARD_PADDING_LEFT, 50)];
...
- (CGFloat)setLabel:(UILabel *)label widht:(CGFloat)width text:(NSString *)text offset:(CGFloat)offset fontSize:(int)fontSize {
    CGSize titleSize = [QWERUtilities getTextDrawingSize:text font:[UIFont systemFontOfSize:fontSize] constrainedSize:CGSizeMake(width, 50)];
    label.text = text;
...
- (UIView *)exView {
    if(!_exView) {
        ...
        _exFWView.image = [QWERImage imageNamed:@"forward_normal"];
        [_exView addSubview:_exFWView];
        
        _extrewqPortraitView = [[UIImageView alloc] initWithFrame:CGRectMake(CELL_PADDING, EX_LINE_WIDTH + (EX_FW_WIDTH - EX_FONT_SIZE)/2, EX_FONT_SIZE, EX_FONT_SIZE)];
        [_exView addSubview:_extrewqPortraitView];
...
- (void)prepareForReuse {
    ...
    [self.extrewqPortraitView sd_cancelCurrentImageLoad];
    self.extrewqPortraitView.image = nil;
}
@end
