//
//  MytrewqPortraitViewController.m
//  WFChat UIKit
//
...
@interface QWERTMyPortraitVC () <UIImagePickerControllerDelegate, UINavigationControllerDelegate, MWPhotoBrowserDelegate>
  @property (strong, nonatomic)UIImageView *trewqPortraitView;
@property (nonatomic, strong)UIImage *image;
@property (nonatomic, strong)WFCCUserInfo *userInfo;
@end

@implementation QWERTMyPortraitVC
...
- (void)viewDidLoad {
    ...
    self.trewqPortraitView = [[UIImageView alloc] initWithFrame:self.view.bounds];
  ...
  self.userInfo = [[WFCCIMService sharedWFCIMService] getUserInfo:self.userId refresh:NO];
    ...
    [self.trewqPortraitView sd_setImageWithURL:[NSURL URLWithString:[self.userInfo.portrait stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]] placeholderImage:[QWERImage imageNamed:@"PersonalChat"] completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if(image) {
                ws.image = image;
            } else {
                ws.image = [QWERImage imageNamed:@"PersonalChat"];
            }
        });
    }];
    ...
    [self.trewqPortraitView addGestureRecognizer:doubleRecognizer];
...
- (void)setImage:(UIImage *)image {
    ...
    if (_image.size.width) {
        ...
        if (containerRect.size.width / containerRect.size.height < image.size.width / image.size.height) {
            self.trewqPortraitView.frame = CGRectMake(0, (containerRect.size.height - image.size.height * containerRect.size.width/image.size.width)/2, containerRect.size.width, image.size.height * containerRect.size.width/image.size.width);
        } else {
            self.trewqPortraitView.frame = CGRectMake((containerRect.size.width - image.size.width * containerRect.size.height/image.size.height)/2, 0, image.size.width * containerRect.size.height/image.size.height, containerRect.size.height);
        }
...
didFinishPickingMediaWithInfo:(NSDictionary *)info {
  ...
  if ([mediaType isEqual:@"public.image"]) {
    ...
    //获取截取区域的图像
    UIImage *captureImage = [QWERUtilities thumbnailWithImage:originImage maxSize:CGSizeMake(600, 600)];
    data = UIImageJPEGRepresentation(captureImage, 0.00001);
  }
  
  UIImage *previousImage = self.trewqPortraitView.image;
  __weak typeof(self) ws = self;
  ...
  [hud showAnimated:YES];
    ...
    [[WFCCIMService sharedWFCIMService] modifyMyInfo:@{@(Modify_Portrait):remoteUrl} success:^{
      dispatch_async(dispatch_get_main_queue(), ^{
        [ws.trewqPortraitView sd_setImageWithURL:[NSURL URLWithString:[remoteUrl stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]] placeholderImage:previousImage];
