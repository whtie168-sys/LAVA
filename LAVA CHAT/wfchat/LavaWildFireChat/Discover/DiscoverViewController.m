...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([des isEqualToString:@"Dev"]) {
        RWADCBrowserVC *vc = [[RWADCBrowserVC alloc] init];
        vc.hidesBottomBarWhenPushed = YES;
...
//    }
    ...
    if ([des isEqualToString:@"Conference"]) {
        RADCOHomeVC *vc = [[RADCOHomeVC alloc] init];
        vc.hidesBottomBarWhenPushed = YES;
        [self.navigationController pushViewController:vc animated:YES];
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0 && self.hasMoments) {
            ...
            if (unread) {
                momentsCell.asoucBubbleView.hidden = NO;
                [momentsCell.asoucBubbleView setBubbleTipNumber:unread];
            } else {
                momentsCell.asoucBubbleView.hidden = YES;
            }
