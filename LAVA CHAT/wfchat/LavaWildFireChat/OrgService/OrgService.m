    ...
    [self post:@"/api/relationship/employee" data:@{@"employeeId":employeeId} isLogin:NO success:^(NSDictionary *dict) {
        if([dict[@"code"] intValue] == 0) {
            ...
            [arr enumerateObjectsUsingBlock:^(NSDictionary  *obj, NSUInteger idx, BOOL * _Nonnull stop) {
                ESZQSCOrgRelationship *rs = [self relationshipFromDict:obj];
                [result addObject:rs];
            }];
...
- (ESZQSCOrganization *)organizationFromDict:(NSDictionary *)obj {
    ESZQSCOrganization *org = [[ESZQSCOrganization alloc] init];
...
- (ESZQSCEmployee *)employeeFromDict:(NSDictionary *)obj {
    ESZQSCEmployee *emp = [[ESZQSCEmployee alloc] init];
...
- (ESZQSCOrgRelationship *)relationshipFromDict:(NSDictionary *)obj {
    ESZQSCOrgRelationship *rs = [[ESZQSCOrgRelationship alloc] init];
...
- (void)getRootOrganization:(void(^)(NSArray<ESZQSCOrganization *> *))successBlock
    ...
    [self post:@"/api/organization/root" data:nil isLogin:NO success:^(NSDictionary *dict) {
        if([dict[@"code"] intValue] == 0) {
            ...
            NSArray *arr = dict[@"result"];
            [arr enumerateObjectsUsingBlock:^(NSDictionary  *obj, NSUInteger idx, BOOL * _Nonnull stop) {
                ESZQSCOrganization *org = [self organizationFromDict:obj];
...
- (void)getOrganizationEx:(NSInteger)organizationId
                success:(void(^)(ESZQSCOrganizationEx *ex))successBlock
                  error:(void(^)(int error_code))errorBlock {
    [self post:@"/api/organization/query_ex" data:@{@"id":@(organizationId)} isLogin:NO success:^(NSDictionary *dict) {
        if([dict[@"code"] intValue] == 0) {
            ...
            ESZQSCOrganization *org = [self organizationFromDict:d[@"organization"]];
            ...
            if(d[@"employees"]) {
                ...
                [arr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    ESZQSCEmployee *emp = [self employeeFromDict:obj];
...
- (void)getEmployee:(NSString *)employeeId
    ...
    [self post:@"/api/employee/query" data:@{@"employeeId":employeeId} isLogin:NO success:^(NSDictionary *dict) {
        if([dict[@"code"] intValue] == 0) {
            ...
            ESZQSCEmployee *employee = [self employeeFromDict:emp];
...
- (void)getEmployeeEx:(NSString *)employeeId
    ...
    [self post:@"/api/employee/query_ex" data:@{@"employeeId":employeeId} isLogin:NO success:^(NSDictionary *dict) {
        if([dict[@"code"] intValue] == 0) {
            ...
            ESZQSCEmployee *employee = [self employeeFromDict:exDict[@"employee"]];
            ...
            ESZQSCEmployeeEx *empEx = [[ESZQSCEmployeeEx alloc] init];
...
- (void)searchEmployee:(NSInteger)organizationId
    ...
    [self post:@"/api/employee/search" data:@{@"keyword":keyword, @"organizationId":@(organizationId), @"count":@(50), @"page":@(0)} isLogin:NO success:^(NSDictionary *dict) {
        if([dict[@"code"] intValue] == 0) {
            ...
            NSMutableArray *result = [[NSMutableArray alloc] init];
            [arr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                ESZQSCEmployee *rs = [self employeeFromDict:obj];
                [result addObject:rs];
            }];
...
- (void)clearOrgServiceAuthInfos {
    ...
    [[ESZQSCOrganizationCache sharedCache] clearCaches];
}
