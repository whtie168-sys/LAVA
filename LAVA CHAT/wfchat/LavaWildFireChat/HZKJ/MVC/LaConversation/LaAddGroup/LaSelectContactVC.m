...
#import "LaAddGroupVC.h"

@interface LaSelectContactVC ()<UITableViewDelegate, UITableViewDataSource, UICollectionViewDelegate, UICollectionViewDataSource, UISearchControllerDelegate, UISearchResultsUpdating>
{
    BOOL _isChinese;
}
...
@implementation LaSelectContactVC
...
- (void)viewDidLoad {
    ...
    [_oxgcseoaiOkButton setTitle:UNString(@"%@(0)", LLLLLL(@"AlertButton")) forState:UIControlStateNormal];
    
    NSArray *results = [WFCCIMService.sharedWFCIMService getUserInfos:[WFCCIMService.sharedWFCIMService getMyFriendList:YES] inGroup:@""];
    ...
    [_iconCollectionView registerNib:[UINib nibWithNibName:@"LaContactIconCVCell" bundle:nil] forCellWithReuseIdentifier:@"LaContactIconCVCell"];
    ...
    [_oxgcseoaiTableView registerNib:[UINib nibWithNibName:@"LaContactInfoTVCell" bundle:[NSBundle mainBundle]] forCellReuseIdentifier:@"LaContactInfoTVCell"];
...
- (IBAction)oxgcseoaiOk:(UIButton *)sender {
    if (_iconArray.count <= 0) {
        [SVProgressHUD showErrorWithStatus:(_isChinese ? @"请选择好友" : @"Xin vui lòng chọn bạn bè")];
        [SVProgressHUD dismissWithDelay:1.0];
        return;
    }
    if (_iconArray.count < 1) {
        [SVProgressHUD showErrorWithStatus:(_isChinese?@"群聊的成员不能小于1个":@"Nhóm trò chuyện không thể có ít hơn 1 thành viên")];
        [SVProgressHUD dismissWithDelay:1.0];
        return;
    }
    LaAddGroupVC *vc = LaAddGroupVC.new;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    LaContactInfoTVCell *cell = [tableView dequeueReusableCellWithIdentifier:@"LaContactInfoTVCell" forIndexPath:indexPath];
    ...
    cell.oxgcseoaiSelectButton.tag = indexPath.section * 1000 + indexPath.row;
...
- (void)oxgcseoaiSelect:(UIButton *)sender {
    ...
    [_oxgcseoaiOkButton setTitle:[NSString stringWithFormat:@"%@(%ld)",LLLLLL(@"AlertButton"), _iconArray.count] forState:UIControlStateNormal];
}
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    LaContactIconCVCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"LaContactIconCVCell" forIndexPath:indexPath];
    [cell.oxgcseoaiIconView sd_setImageWithURL:URL(_iconArray[indexPath.row].portrait) placeholderImage:[QWERImage imageNamed:@"PersonalChat"] options:SDWebImageScaleDownLargeImages
