...
@interface LaAddGroupVC ()<UIImagePickerControllerDelegate, UINavigationControllerDelegate, UICollectionViewDelegate, UICollectionViewDataSource>
{
    UIImage *_iconImg;
    NSString *review;
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = (_isChinese ? @"新建群聊" : @"Trò chuyện nhóm mới");
    ...
    [_oxgcseoaiCollectionView registerNib:[UINib nibWithNibName:@"LaAddGroupCVCell" bundle:nil] forCellWithReuseIdentifier:@"LaAddGroupCVCell"];
}
...
//        return;
//    }
    UIAlertController * alertController = [UIAlertController alertControllerWithTitle:(_isChinese?@"您确定要创建群聊吗？":@"Bạn có chắc bạn muốn tạo ra một nhóm chat?") message:nil preferredStyle:UIAlertControllerStyleAlert];
...
- (void)sendHUD:(MBProgressHUD *)hud AddGroupReview:(NSString *)groupId {
    ...
    [AppService.sharedAppService requestUrl:@"/group/invite" params:@{@"groupId":groupId, @"inviteUsers":inviteUsers, @"source":@(2)} success:^(NSDictionary * _Nonnull dict) {
        UIAlertController * alertController = [UIAlertController alertControllerWithTitle:(self->_isChinese?@"请求已发送":@"Yêu cầu đã được gửi") message:(self->_isChinese?UNString(@"%@等开启了入群需审核，对方同意后才会进入群聊", userStr):UNString(@"%@ mở vào nhóm cần phải được phê duyệt, sau khi đồng ý để nhập vào nhóm trò chuyện", userStr)) preferredStyle:UIAlertControllerStyleAlert];
        [hud hideAnimated:YES];
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    LaAddGroupCVCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"LaAddGroupCVCell" forIndexPath:indexPath];
