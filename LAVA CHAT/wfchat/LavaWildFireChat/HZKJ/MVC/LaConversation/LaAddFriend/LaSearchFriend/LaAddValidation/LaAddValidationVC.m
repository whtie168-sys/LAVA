...
@interface LaAddValidationVC ()<UITextViewDelegate>
{
    BOOL _isChinese;
}
...
@implementation LaAddValidationVC

- (void)viewDidLoad {
    [super viewDidLoad];
    _isChinese = [CommonHelper.main isChinese];
    self.navigationItem.title = _isChinese?@"添加验证":@"Thêm xác nhận";
    
    _descTV.delegate = self;
    
    _descTV.text = [NSString stringWithFormat:@"%@ %@",(_isChinese?@"我是":@"Tôi là"), _name];
    _numLabel.text = UNString(@"%ld", _descTV.text.length);
    
    _bgView.layer.cornerRadius = 12.0;
    _postButton.layer.cornerRadius = 25.0;
    
    [_postButton setTitle:(_isChinese ? @"完成" : @"Hoàn tất") forState:UIControlStateNormal];
}

- (IBAction)sendApply:(UIButton *)sender {
    [self.view endEditing:YES];
    if (_descTV.text.length <= 0) {
        [SVProgressHUD showErrorWithStatus:_isChinese?@"请输入内容...":@"Xin vui lòng nhập nội dung..."];
    ...
    WS(weakself)
    if ([UserExtraInfo mj_objectWithKeyValues:self.userInfo.extra].disableAutoAddFriend == 1) { // 加我为朋友时是否需要验证
        [WFCCIMService.sharedWFCIMService sendFriendRequest:_userInfo.userId reason:_descTV.text extra:@"" success:^{
            dispatch_async(dispatch_get_main_queue(), ^{
                [hud hideAnimated:YES];
                
                [weakself.view makeToast:(self->_isChinese?@"好友申请已发送":@"Yêu cầu đã được gửi") duration:1.0 position:CSToastPositionCenter];
        ...
        [AppService.sharedAppService requestUrl:@"/user/friend/add" params:@{@"userId":_userInfo.userId} success:^(NSDictionary * _Nonnull dict) {
            ...
            [weakself.view makeToast:self->_isChinese?@"添加成功":@"Thêm thành công" duration:1.0 position:CSToastPositionCenter];
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
