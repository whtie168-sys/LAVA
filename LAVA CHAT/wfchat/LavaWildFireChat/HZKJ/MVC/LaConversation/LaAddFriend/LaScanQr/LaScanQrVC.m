...
//    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:[self itemImage:@"view_close" action:@selector(view_close)]];
//    
...
//    self.navigationItem.titleView = titleLabel;
    ...
    self.style.colorAngle = RGBA(0x17B1FE);
...
- (void)drawTitle {
    if (!_topTitle) {
        ...
        _topTitle.frame = CGRectMake(25.0, NavigationHeight + 52.0, WIDTH-50.0, 60);
        ...
        _topTitle.text = (_isChinese?@"请将镜头对准地址二维码进行扫描":@"Vui lòng hướng ống kính vào địa chỉ mã QR để quét");
...
- (void)drawBottomItems {
    ...
    self.bottomItemsView = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.view.bounds)-132 - [QWERUtilities wf_safeDistanceBottom],
...
- (void)popAlertMsgWithScanResult:(NSString*)strResult {
    if (!strResult) {
        strResult = _isChinese?@"识别失败":@"Nhận ra thất bại";
    }
    
    __weak __typeof(self) weakSelf = self;
    [LBXAlertAction showAlertWithTitle:(_isChinese?@"扫码内容":@"Quét mã nội dung") msg:strResult buttonsStatement:@[LLLLLL(@"iGotIt")] chooseBlock:^(NSInteger buttonIdx) {
...
- (void)openPhoto {
    ...
    [LBXPermission authorizeWithType:LBXPermissionType_Photos completion:^(BOOL granted, BOOL firstTime) {
        if (granted) {
            ...
            [LBXPermissionSetting showAlertToDislayPrivacySettingWithTitle:LLLLLL(@"Tips") msg:(self->_isChinese?@"没有相册权限，是否前往设置":@"Không có quyền tập ảnh, cho dù đi đến thiết lập.") cancel:LLLLLL(@"Cancel") setting:LLLLLL(@"Settings")];
        }
    }];
}
