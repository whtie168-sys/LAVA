...
#import "LaAnnouncementTopView.h"
#import "LaTopMessageView.h"
#import "AllTopMessagePopupView.h"
#import "LaForwardVC.h"
#import "LaTopMessageShowViewController.h"


@interface LaMessageVC ()<UITextFieldDelegate, UICollectionViewDelegateFlowLayout, UICollectionViewDataSource, UICollectionViewDelegate, UINavigationControllerDelegate, ASDFGMessageCellDelegate, AVAudioPlayerDelegate, WDCARChatInputBarDelegate, UIGestureRecognizerDelegate, UITableViewDelegate, UITableViewDataSource, QWERTMultiCallOngoingExpendedCellDelegate, MWPhotoBrowserDelegate>
{
    NSString *_saveOnlineText;
    BOOL _isShowInputState; // 当前账号是否展示输入状态
    
    BOOL _isManager; // 是否是管理员-包含群主 0227新增
    
    BOOL _isChinese;
    
    BOOL _isUpdateGroupAnnouncement; // 是否更新了群公告内容
}
@property (nonatomic, strong)NSMutableArray<QWERTMessageModel *> *modelList;
...
@property (nonatomic, strong)ASDFGMessageCellBase *cell4Menu;
...
@property (nonatomic, strong) TREWQGroupAnnouncement *groupAnnouncement; // 0116
// 群公告新发布时 顶部的弹窗提示
@property (nonatomic, strong) LaAnnouncementTopView *announcementTopView;


@property (nonatomic, strong) LaTopMessageView *topMessageView; // 顶部的消息置顶或者公告
...
- (UIView *)userView {
    if (!_userView) {
        ...
        UILabel *asoucNameLabel = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(iconView.frame) + 10.0, 0.0, WIDTH/2.0, 25.3)];
        asoucNameLabel.textColor = UIColor.blackColor;
        asoucNameLabel.textAlignment = NSTextAlignmentLeft;
        asoucNameLabel.font = PINGFANG_M(18.0)
        asoucNameLabel.backgroundColor = UIColor.clearColor;
        [_userView addSubview:asoucNameLabel];
        _asoucNameLabel = asoucNameLabel;
        
        UILabel *onlineLabel = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMinX(asoucNameLabel.frame), CGRectGetMaxY(asoucNameLabel.frame), WIDTH-100.0, 16.0)];
...
- (void)topUserInfo {
    if (self.conversation.type == Single_Type) {
        [_iconView sd_setImageWithURL:URL(self.targetUser.portrait) placeholderImage: [QWERImage imageNamed:@"PersonalChat"] options:SDWebImageScaleDownLargeImages
                              context:@{SDWebImageContextImageForceDecodePolicy : @(SDImageForceDecodePolicyNever), SDWebImageContextStoreCacheType : @(SDImageCacheTypeDisk)}];
        if (self.targetUser.friendAlias.length) {
            _asoucNameLabel.text = self.targetUser.friendAlias;
        } else if (self.targetUser.groupAlias.length) {
            _asoucNameLabel.text = self.targetUser.groupAlias;
        } else if(self.targetUser.displayName.length > 0) {
            _asoucNameLabel.text = self.targetUser.displayName;
        } else {
            _asoucNameLabel.text = [NSString stringWithFormat:@"Người dùng<%@>", self.targetUser.userId];
        }
        if ([self.conversation.target isEqualToString:@"customer_service"]) {
            _iconView.image = IMAGENAME(@"customerService");
            _asoucNameLabel.text = LLLLLL(@"AppCustomerService");
        }else if ([self.conversation.target isEqualToString:@"FireRobot"]) {
            if (self.targetUser.portrait.length <= 0) {
                _iconView.image = IMAGENAME(@"LAVA Messenger");
            }
            _asoucNameLabel.text = @"LAVA Quản lý";
        }else if ([self.conversation.target isEqualToString:@"wfc_file_transfer"]) {
            if (_isChinese) {
                _asoucNameLabel.text = @"文件传输助手";
            }else {
                _asoucNameLabel.text = @"Trợ lý truyền tải tập tin";
            }
        }
        if ([[WFCCIMService sharedWFCIMService] isEnableUserOnlineState]) {
            ...
            if (extraInfo.disableShowLastLoginTime == 0) { // 0 所有人    1 仅通讯录联系人    2 不显示在线时间
                ...
                if ([CommonHelper.main isAddressBookContact:_targetUser.mobile]) {
                    ...
                    _asoucNameLabel.frame = CGRectMake(CGRectGetMaxX(_iconView.frame) + 10.0, 0.0, WIDTH/2.0, 44.0);
            ...
            if (_isShowInputState == 1) {
                _asoucNameLabel.frame = CGRectMake(CGRectGetMaxX(_iconView.frame) + 10.0, 0.0, WIDTH/2.0, 25.0);
                _onlineLabel.hidden = NO;
                _saveOnlineText = (_isChinese?@"近期上过线":@"Gần đây trên dòng");
        ...
        [_iconView sd_setImageWithURL:URL(self.targetGroup.portrait) placeholderImage: IMAGENAME(@"groupIcon")];
        if (self.targetGroup.name) {
            //            _asoucNameLabel.text = self.targetGroup.name;
            _asoucNameLabel.text = [NSString stringWithFormat:@"%@ (%ld)",_targetGroup.name, _targetGroup.memberCount];
        }else {
            _asoucNameLabel.text = [NSString stringWithFormat:@"Nhóm<%@>", self.targetGroup.target];
        }
...
- (void)onlineState {
    //    WFCCUserOnlineState *state = [[WFCCIMService sharedWFCIMService] getUserOnlineState:self.targetUser.userId];
    WFCCUserOnlineState *state = [[WFCCIMService sharedWFCIMService] getUserOnlineState:self.conversation.target];
    BOOL online = NO;
...
- (void)onRightBarBtn:(UIBarButtonItem *)sender {
    ...
    if (self.conversation.type == Group_Type) {
        LaGroupSetupVC *vc = LaGroupSetupVC.new;
        vc.conversation = self.conversation;
        vc.groupAnnouncement = _groupAnnouncement;
        [self.navigationController pushViewController:vc animated:YES];
    }else {
        LaSingleSetupVC *gvc = LaSingleSetupVC.new;
...
- (void)setupNavigationItem {
    if (self.multiSelecting) {
        self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[QWERImage imageNamed:@"search"] style:UIBarButtonItemStyleDone target:self action:@selector(onSearchBarBtn:)];
        ...
        if (self.conversation.type == Single_Type) {
            //            self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[QWERImage imageNamed:@"nav_chat_single"] style:UIBarButtonItemStyleDone target:self action:@selector(onRightBarBtn:)];
            ...
            //            self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[QWERImage imageNamed:@"nav_chat_group"] style:UIBarButtonItemStyleDone target:self action:@selector(onRightBarItem:)]; // 1211  15:15 打开的注视  用于测试要删掉
            
            self.navigationItem.titleView = self.userView;
        } else if(self.conversation.type == Channel_Type) {
            self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[QWERImage imageNamed:@"nav_chat_channel"] style:UIBarButtonItemStyleDone target:self action:@selector(onRightBarBtn:)];
...
- (void)videoVoice:(UIBarButtonItem *)item {
    ...
    RADCOVideoVC *videoVC = [[RADCOVideoVC alloc] initWithTargets:@[userId] conversation:_conversation audioOnly:(item.tag == 1 ? YES : NO)];
...
//        NSLog(@"toJsonObj===%@",userInfo.toJsonObj);
    ...
    if (self.conversation.type == Group_Type) {
        dispatch_async(dispatch_get_main_queue(), ^{
            NSArray<WFCCGroupMember *> *groupMembers = [[WFCCIMService sharedWFCIMService] getGroupMembers:self.conversation.target forceUpdate:YES];
...
- (void)onGroupInfoUpdated:(NSNotification *)notification {
    ...
    for (WFCCGroupInfo *groupInfo in groupInfoList) {
        if ([self.conversation.target isEqualToString:groupInfo.target]) {
            self.targetGroup = groupInfo;
            self.groupExtraInfo = [GroupExtraInfo mj_objectWithKeyValues:groupInfo.extra];
            _isManager = ([self isGroupOwner] || [self isGroupManager:WFCCNetworkService.sharedInstance.userId]); // 0227
//            NSLog(@"targetGroup=====%@",self.targetGroup.toJsonObj);
            break;
        }
    }
}
...
//The VC maybe pushed from search VC, so no need go back to search VC, need remove all the VC between current VC to ASECWConversationTableVC
- (void)removeControllerStackIfNeed {
    ...
    if (self.highlightMessageId > 0) {
        ...
        for (int i = (int)controllers.count - 2; i >=0; i--) {
            ...
            if ([controller isKindOfClass:[ASECWConversationTableVC class]]) {
                foundParent = YES;
                break;
            } else {
...
- (void)loadRemoteHistoryMessages:(void (^ __nullable)(BOOL more))completion {
    ...
    for (QWERTMessageModel *model in self.modelList) {
...
- (void)sendChatroomWelcomeMessage {
    if(!self.silentJoinChatroom) {
        ...
        WFCCUserInfo *userInfo = [[WFCCIMService sharedWFCIMService] getUserInfo:[WFCCNetworkService sharedInstance].userId refresh:NO];
        if (_isChinese) {
            tip.tip = UNString(@"欢迎 %@ 加入聊天室", (userInfo.friendAlias.length > 0 ? userInfo.friendAlias : userInfo.displayName));
        }else {
            tip.tip = UNString(@"Chào mừng %@ đến phòng chat", (userInfo.friendAlias.length > 0 ? userInfo.friendAlias : userInfo.displayName));
        }
...
- (void)sendChatroomLeaveMessage {
    ...
    if(!self.silentJoinChatroom) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            WFCCUserInfo *userInfo = [[WFCCIMService sharedWFCIMService] getUserInfo:[WFCCNetworkService sharedInstance].userId refresh:NO];
            if (self->_isChinese) {
                tip.tip = UNString(@"%@ 离开了聊天室", (userInfo.friendAlias.length > 0 ? userInfo.friendAlias : userInfo.displayName));
            }else {
                tip.tip = UNString(@"%@ đã rời nhóm", (userInfo.friendAlias.length > 0 ? userInfo.friendAlias : userInfo.displayName));
            }
...
- (void)onSearchBarBtn:(id)sender {
    if (self.multiSelecting) {
        ...
        LaConversationSearchVC *mvc = [[LaConversationSearchVC alloc] init];
            ...
            if([onlineState.clientStates count]) {
                ...
                if(hasOnline) {
                    ...
                    if(onlineState.customState.state == 0) {
                        if(pcState == 0) {
                            self.title = [NSString stringWithFormat:@"%@(%@)", self.title, (_isChinese?@"电脑在线":@"Máy tính trực tuyến")];
                        } else if(padState == 0) {
                            self.title = [NSString stringWithFormat:@"%@(%@)", self.title, _isChinese?@"平板在线":@"Máy tính bảng trực tuyến"];
                        } else if(webState == 0) {
                            self.title = [NSString stringWithFormat:@"%@(%@)", self.title, _isChinese?@"网页在线":@"Trang trực tuyến"];
                        } else if(wxState == 0) {
                            self.title = [NSString stringWithFormat:@"%@(%@)", self.title, _isChinese?@"小程序在线":@"Tiểu dụng Trực tuyến"];
                        } else if(mobileState == 0) {
                            self.title = [NSString stringWithFormat:@"%@(%@)", self.title, _isChinese?@"手机在线":@"Điện thoại di động trực tuyến"];
                        }
                    } else if(onlineState.customState.state == 1) {
                        self.title = [NSString stringWithFormat:@"%@(%@)", self.title, _isChinese?@"忙碌":@"Bận rộn"];
                    } else if(onlineState.customState.state == 2 || onlineState.customState.state == 3) {
                        self.title = [NSString stringWithFormat:@"%@(%@)", self.title, _isChinese?@"离开":@"Offline"];
                    } else {
                        //其它情况需要客户自己开发。。。
                    }
    ...
    if (self.modelList) {
        for (QWERTMessageModel *model in self.modelList) {
            if (showAlias && model.message.direction == MessageDirection_Receive) {
                model.showasoucNameLabel = YES;
            } else {
                model.showasoucNameLabel = NO;
            }
        }
    }
}
...
- (void)onForwardMultiSelectedMessage:(id)sender {
    ...
    UIAlertAction *oneByOneAction = [UIAlertAction actionWithTitle:LLLLLL(@"ForwardingOneByOne") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        LaForwardVC *controller = [[LaForwardVC alloc] init];
    ...
    UIAlertAction *AllInOneAction = [UIAlertAction actionWithTitle:LLLLLL(@"MergeForward") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        if (self.conversation.type == Single_Type) {
            ...
            if (self->_isChinese) {
                ...
                compositeContent.title = [NSString stringWithFormat:@"Lịch sử trò chuyện của %@ và %@", title, myself.displayName];
            }
        } else if (self.conversation.type == Group_Type) {
            compositeContent.title = self->_isChinese?@"群的聊天记录":@"Lịch sử trò chuyện nhóm";
        } else if (self.conversation.type == Channel_Type) {
            compositeContent.title = self->_isChinese?@"频道的聊天记录":@"Lịch sử trò chuyện của kênh";
        } else if(self.conversation.type == SecretChat_Type) {
            compositeContent.title = self->_isChinese?@"密聊记录":@"Ghi âm trò chuyện bí mật";
        } else {
            compositeContent.title = self->_isChinese?@"聊天记录":@"Lịch sử trò chuyện";
        }
...
- (void)initializedSubViews {
    ...
    frame.origin.y += [QWERUtilities wf_navigationFullHeight];
    frame.size.height -= ([QWERUtilities wf_safeDistanceBottom] + [QWERUtilities wf_navigationFullHeight]);
    ...
    [self registerCell:[ASDFGTextCell class] forContent:[WFCCTextMessageContent class]];
    [self registerCell:[ASDFGTextCell class] forContent:[WFCCPTextMessageContent class]];
    [self registerCell:[ASDFGImageCell class] forContent:[WFCCImageMessageContent class]];
    [self registerCell:[ASDFGVoiceCell class] forContent:[WFCCSoundMessageContent class]];
    [self registerCell:[ASDFGVoiceCell class] forContent:[WFCCPTTSoundMessageContent class]];
    [self registerCell:[ASDFGVideoCell class] forContent:[WFCCVideoMessageContent class]]; // 视频类型
    [self registerCell:[ASDFGLocationCell class] forContent:[WFCCLocationMessageContent class]];
    [self registerCell:[ASDFGFileCell class] forContent:[WFCCFileMessageContent class]];
    [self registerCell:[ASDFGStickerCell class] forContent:[WFCCStickerMessageContent class]];
    
    [self registerCell:[ASDFGInformationCell class] forContent:[WFCCCreateGroupNotificationContent class]];
    [self registerCell:[ASDFGInformationCell class] forContent:[WFCCAddGroupeMemberNotificationContent class]];
    [self registerCell:[ASDFGInformationCell class] forContent:[WFCCKickoffGroupMemberNotificationContent class]];
    [self registerCell:[ASDFGInformationCell class] forContent:[WFCCQuitGroupNotificationContent class]];
    [self registerCell:[ASDFGInformationCell class] forContent:[WFCCKickoffGroupMemberVisibleNotificationContent class]];
    [self registerCell:[ASDFGInformationCell class] forContent:[WFCCQuitGroupVisibleNotificationContent class]];
    [self registerCell:[ASDFGInformationCell class] forContent:[WFCCDismissGroupNotificationContent class]];
    [self registerCell:[ASDFGInformationCell class] forContent:[WFCCTransferGroupOwnerNotificationContent class]];
    [self registerCell:[ASDFGInformationCell class] forContent:[WFCCModifyGroupAliasNotificationContent class]];
    [self registerCell:[ASDFGInformationCell class] forContent:[WFCCChangeGroupNameNotificationContent class]];
    [self registerCell:[ASDFGInformationCell class] forContent:[WFCCChangeGroupPortraitNotificationContent class]];
    [self registerCell:[ASDFGInformationCell class] forContent:[WFCCFriendAddedMessageContent class]];
    [self registerCell:[ASDFGInformationCell class] forContent:[WFCCFriendGreetingMessageContent class]];
    
    [self registerCell:[ASDFGCallSummaryCell class] forContent:[WFCCCallStartMessageContent class]];
    [self registerCell:[ASDFGInformationCell class] forContent:[WFCCTipNotificationContent class]];
    [self registerCell:[ASDFGInformationCell class] forContent:[WFCCUnknownMessageContent class]];
    [self registerCell:[ASDFGRecallCell class] forContent:[WFCCRecallMessageContent class]];
    [self registerCell:[ASDFGConferenceInviteCell class] forContent:[WFCCConferenceInviteMessageContent class]];
    [self registerCell:[ASDFGCardCell class] forContent:[WFCCCardMessageContent class]];
    [self registerCell:[ASDFGCompositeCell class] forContent:[WFCCCompositeMessageContent class]];
    [self registerCell:[ASDFGLinkCell class] forContent:[WFCCLinkMessageContent class]];
    [self registerCell:[ASDFGRichNotificationCell class] forContent:[WFCCRichNotificationMessageContent class]];
    [self registerCell:[ASDFGArticlesCell class] forContent:[WFCCArticlesMessageContent class]];
    
    // 自定义群公告消息 1130
    [self registerCell:[LaAnnouncementCell class] forContent:[LaAnnouncementMessageContent class]];
...
- (void)showTyping {
    ...
    if(self.typingDict.count == 1) {
        ...
        if(typingType == Typing_VOICE) {
            title = _isChinese?@"对方正在录音...":@"Đang ghi âm...";
        } else if(typingType == Typing_CAMERA) {
            title = _isChinese?@"对方正在拍照...":@"Đang chụp ảnh...";
        } else if(typingType == Typing_LOCATION) {
            title = _isChinese?@"对方正在选取位置...":@"Đang chọn vị trí...";
        } else if(typingType == Typing_FILE) {
            title = _isChinese?@"对方正在选取文件...":@"Đang chọn tập tin...";
        } else {
            title = _isChinese?@"对方正在输入...":@"Nhập...";
        }
        if (_isShowInputState == 1) {
        ...
        if (_isShowInputState == 1) {
            _onlineLabel.text = [NSString stringWithFormat:@"%ld%@",self.typingDict.count, (_isChinese?@"人正在输入":@" người đang nhập")];
        }
...
- (void)onReceiveCallOngoingNotifications:(NSArray<WFCCMessage *> *)messages {
    ...
    if(![QWERConfigManager globalManager].enableMultiCallAutoJoin) {
        [self.ongoingCallDict removeAllObjects];
    }
    
    if(self.ongoingCallDict.count) {
        self.ongoingCallTableView.frame = CGRectMake(0, [QWERUtilities wf_navigationFullHeight], self.view.bounds.size.width, MIN(200, self.ongoingCallDict.count * 28 + 28));
        ...
        self.ongoingCallTableView.frame = CGRectMake(0, [QWERUtilities wf_navigationFullHeight], self.view.bounds.size.width, 0);
        if(self.checkOngoingCallTimer) {
            [self.checkOngoingCallTimer invalidate];
            self.checkOngoingCallTimer = nil;
        }
...
- (void)onRecallMessages:(NSNotification *)notification {
    ...
    if (self.conversation.type != Chatroom_Type) {
        ...
        if (msg != nil) {
            for (int i = 0; i < self.modelList.count; i++) {
                QWERTMessageModel *model = [self.modelList objectAtIndex:i];
...
- (void)sendMessage:(WFCCMessageContent *)content {
    ...
    [[WFCCIMService sharedWFCIMService] send:self.conversation content:content toUsers:tousers expireDuration:0 success:^(long long messageUid, long long timestamp) {
        ...
        //        for (QWERTMessageModel *msg in _modelList) {
...
- (void)onMessageUpdated:(NSNotification *)notification {
    ...
    if(isUpdated) {
        [self.modelList sortUsingComparator:^NSComparisonResult(QWERTMessageModel *obj1, QWERTMessageModel *obj2) {
...
- (void)onMessageDelivered:(NSNotification *)notification {
    ...
    if (refresh) {
        ...
        for (int i = 0; i < self.modelList.count; i++) {
            QWERTMessageModel *model  = self.modelList[i];
...
- (void)onSecretMessageBurned:(NSNotification *)notification {
    ...
    if(self.conversation.type == SecretChat_Type) {
        ...
        [self.modelList enumerateObjectsUsingBlock:^(QWERTMessageModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)showMentionedLabel {
    ...
    [self.mentionedButton setTitle:[NSString stringWithFormat:@"%lu %@", self.mentionedMsgs.count, (_isChinese?@"条消息":@"tin nhắn")] forState:UIControlStateNormal];
}
...
- (void)showUnreadLabel {
    CGRect bount = self.view.bounds;
    self.asoucUnreadButton = [[UIButton alloc] initWithFrame:CGRectMake(bount.size.width+15, 200, 0, 30)];
    [self.asoucUnreadButton setTitle:[NSString stringWithFormat:@"%d %@",self.unreadMessageCount, (_isChinese?@"条新消息":@"tin nhắn mới")] forState:UIControlStateNormal];
    self.asoucUnreadButton.titleLabel.font = [UIFont systemFontOfSize:12];
    [self.asoucUnreadButton setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
    self.asoucUnreadButton.backgroundColor = [UIColor whiteColor];
    self.asoucUnreadButton.layer.cornerRadius = 15;
    self.asoucUnreadButton.layer.borderColor = [UIColor blackColor].CGColor;
    [self.asoucUnreadButton addTarget:self action:@selector(onUnreadBtn:) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:self.asoucUnreadButton];
    [UIView animateWithDuration:0.6 animations:^{
        self.asoucUnreadButton.frame = CGRectMake(bount.size.width - 85, 200, 100, 30);
    }];
}
...
- (void)dismissUnreadLabel {
    CGRect bount = self.view.bounds;
    [UIView animateWithDuration:0.5 animations:^{
        self.asoucUnreadButton.frame = CGRectMake(bount.size.width+15, 200, 0, 30);
    } completion:^(BOOL finished) {
        [self.asoucUnreadButton removeFromSuperview];
        self.asoucUnreadButton = nil;
    }];
}
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    QWERTMessageModel *model = self.modelList[indexPath.row];
 ...
 */
- (BOOL)filteringData:(QWERTMessageModel *)model { // 返回NO 不添加该条数据
    if (self.conversation.type == Single_Type) {
        if (_isChinese) {
            ...
            if ([_conversation.target isEqualToString:@"FireRobot"] || [_conversation.target isEqualToString:@"wfc_file_transfer"]) {
                if ([model.message.content isKindOfClass:NSClassFromString(@"WFCCTextMessageContent")]) {
                    ...
                    if ([content.text containsString:@"我是官方客服"]) {
                        content.text = @"Xin chào, tôi là dịch vụ khách hàng chính thức! Anh có thể nói chuyện với tôi.";
                    }
                    if ([content.text containsString:@"我是文件传输助手"]) {
                        content.text = @"Xin chào, tôi là trợ lý chuyển tập tin";
                    }
                }
            }
        }
...
//        NSLog(@"contentClass===%@",NSStringFromClass(model.message.content.class));
    ...
    if ([model.message.content isKindOfClass:NSClassFromString(@"WFCCGroupSetManagerNotificationContent")]) { // 设置/取消群管理员的通知消息
        ...
        WFCCGroupSetManagerNotificationContent *content = (WFCCGroupSetManagerNotificationContent *)model.message.content;
        for (NSString *userid in content.memberIds) {
        ...
        [self deleteMessage:model.message.messageId];
        return NO;
    ...
    for (int i = 0; i < messages.count; i++) {
        ...
        if (duplcated) {
            if ([message.content isKindOfClass:NSClassFromString(@"WFCCRecallMessageContent")]) { // 0415新增
                ...
                QWERTMessageModel *model = [QWERTMessageModel modelOf:message showName:message.direction == MessageDirection_Receive && self.showAlias showTime:showTime];
        ...
        if (newMessage) {
            ...
            QWERTMessageModel *model = [QWERTMessageModel modelOf:message showName:message.direction == MessageDirection_Receive&&self.showAlias showTime:YES];
            if ([self filteringData:model] == NO) { // 0226新增 用于筛选通知消息的显示
                continue;
            }
    ...
    if (modifiedAliasUsers.count) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            for (NSIndexPath *item in visibleItems) {
                QWERTMessageModel *model = [self.modelList objectAtIndex:item.row];
...
- (QWERTMessageModel *)modelOfMessage:(long)messageId {
    if (messageId == 0) {
        return nil;
    }
    for (QWERTMessageModel *model in self.modelList) {
...
-(void)prepardToPlay:(QWERTMessageModel *)model {
    ...
    if (self.playingMessageId == model.message.messageId) {
        ...
        if (soundContent.localPath.length == 0 || ![QWERUtilities isFileExist:soundContent.localPath]) {
            __weak typeof(self) weakSelf = self;
            
            BOOL isDownloading = [[QWERMediaMessageDownloader sharedDownloader] tryDownload:model.message success:^(long long messageUid, NSString *localPath) {
...
-(void)startPlay:(QWERTMessageModel *)model {
    ...
    if ([model.message.content isKindOfClass:[WFCCSoundMessageContent class]]) {
        ...
        if(model.message.conversation.type == SecretChat_Type) {
            ...
            if (![@"mp3" isEqualToString:[snc.localPath pathExtension]]) {
                NSString *cacheDir = [[QWERConfigManager globalManager] cachePathOf:model.message.conversation mediaType:0];
        ...
        if (!url) {
            [self.view makeToast:_isChinese?@"无法播放":@"Không thể chơi"];
            return;
        }
...
- (void)didTapMessageCell:(ASDFGMessageCellBase *)cell withModel:(QWERTMessageModel *)model {
    if ([model.message.content isKindOfClass:[LaAnnouncementMessageContent class]]) { // 1130新增。公告点击事件
        
        TREWQGroupAnnouncement *ann = [[TREWQGroupAnnouncement alloc] init];
        ann.groupId = self.conversation.target;
        ann.author = model.message.fromUser;
        ann.text = [(LaAnnouncementMessageContent *)model.message.content text];
        ...
        if(self.conversation.type == SecretChat_Type) {
                ...
                if([model.message.content isKindOfClass:[WFCCImageMessageContent class]]) {
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        if(image) {
                            [[WFCCIMService sharedWFCIMService] setMediaMessagePlayed:model.message.messageId];
                            [hud hideAnimated:YES];
                            QWERImagePreviewViewController *previewVC = [[QWERImagePreviewViewController alloc] init];
            ...
            if (self.conversation.type == Chatroom_Type) {
                ...
                for (QWERTMessageModel *msgModle in self.modelList) {
...
#if WFCU_SUPPORT_VOIP
        __weak typeof(self)ws = self;
        __block MBProgressHUD *hud = [self startProgress:LLLLLL(@"Loading")];
        if ([Chat86AVEngineKit sharedEngineKit].supportConference) {
            ...
            [[QWERConfigManager globalManager].appServiceProvider queryConferenceInfo:invite.callId password:invite.password success:^(RADCOConferenceInfo * _Nonnull conferenceInfo) {
                ...
                if (errorCode == 16) {
                    [ws stopProgress:hud finishText:self->_isChinese?@"会议已结束！":@"Cuộc họp kết thúc!"];
                } else {
                    [ws stopProgress:hud finishText:LLLLLL(@"NetworkError")];
                }
            }];
        } else {
            [ws stopProgress:hud finishText:_isChinese?@"不支持会议":@"Không hỗ trợ họp"];
        }
#endif
        ...
        if (card.type == CardType_User) {
            ...
            //            if (!userInfo.deleted) {
            //                RWADCProfileTableVC *vc = [[RWADCProfileTableVC alloc] init];
            ...
            AUETAChannelProfileVC *pvc = [[AUETAChannelProfileVC alloc] init];
        ...
        QWERCompositeMessageVC *vc = [[QWERCompositeMessageVC alloc] init];
...
- (void)didDoubleTapMessageCell:(ASDFGMessageCellBase *)cell withModel:(QWERTMessageModel *)model {
    if ([model.message.content isKindOfClass:[WFCCTextMessageContent class]]) {
        ...
        UITextView *textView = [[UITextView alloc] initWithFrame:CGRectMake(0, [QWERUtilities wf_navigationFullHeight], [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height - [QWERUtilities wf_navigationFullHeight] - [QWERUtilities wf_safeDistanceBottom])];
        ...
        if (self.targetGroup.privateChat) {
            if (![self.targetGroup.owner isEqualToString:model.message.fromUser] && ![self.targetGroup.owner isEqualToString:[WFCCNetworkService sharedInstance].userId]) {
                ...
                if (gm.type != Member_Type_Manager) {
                    ...
                    if (gm.type != Member_Type_Manager) {
                        [self.view makeToast:(_isChinese?@"管理员关闭了群组私聊权限":@"Quản trị viên tắt quyền trò chuyện nhóm") duration:1 position:CSToastPositionCenter];
                        return;
                    }
                }
            }
...
- (void)didLongPressMessageCell:(ASDFGMessageCellBase *)cell withModel:(QWERTMessageModel *)model {
    if ([cell isKindOfClass:[ASDFGMessageCellBase class]]) {
        //        if (!self.isFirstResponder) {
        //            [self becomeFirstResponder];
        //        }
        
        [self displayMenu:(ASDFGMessageCellBase *)cell];
    }
}
...
- (void)didLongPressMessagePortrait:(ASDFGMessageCellBase *)cell withModel:(QWERTMessageModel *)model {
    if (self.conversation.type == Group_Type) {
        ...
        if ([channelInfo.owner isEqualToString:[WFCCNetworkService sharedInstance].userId]) {
            ...
            UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:LLLLLL(@"Cancel") style:UIAlertActionStyleCancel handler:nil];
            UIAlertAction *okAction = [UIAlertAction actionWithTitle:(_isChinese?@"与订阅者私聊":@"Trò chuyện riêng với người đăng ký") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                if (model.message.direction == MessageDirection_Receive) {
                    WFCCChannelInfo *channelInfo = [[WFCCIMService sharedWFCIMService] getChannelInfo:self.conversation.target refresh:NO];
                    if ([channelInfo.owner isEqualToString:[WFCCNetworkService sharedInstance].userId]) {
...
- (void)didSelectUrl:(ASDFGMessageCellBase *)cell withModel:(QWERTMessageModel *)model withUrl:(NSString *)urlString {
    if ([model.message.content.class isEqual:NSClassFromString(@"LaAnnouncementMessageContent")]) { // 群公告点击链接
...
- (void)didTapReceiptView:(ASDFGMessageCellBase *)cell withModel:(QWERTMessageModel *)model {
    ASECWReceiptVC *receipt = [[ASECWReceiptVC alloc] init];
...
- (void)showQuote:(WFCCQuoteInfo *)quoteInfo ofMessage:(WFCCMessage *)msg {
    if ([msg.content isKindOfClass:[WFCCTextMessageContent class]]) {
    ...
    } else if ([msg.content isKindOfClass:[LaAnnouncementMessageContent class]]) { // 1130新增 引用群公告引用
        //        WS(weakself)
        //        [[QWERConfigManager globalManager].appServiceProvider getGroupAnnouncement:self.conversation.target success:^(TREWQGroupAnnouncement *announcement) {
        //            dispatch_async(dispatch_get_main_queue(), ^{
        //            });
        //        } error:^(int error_code) {
        //        }];
        ...
        ann.text = [(LaAnnouncementMessageContent *)msg.content text];
        ...
        // 1120 空白页面
        //        QWERQuoteVC *vc = [[QWERQuoteVC alloc] init];
...
- (BOOL)isGroupOwner {
    ...
    return [self.targetGroup.owner isEqualToString:[WFCCNetworkService sharedInstance].userId];
}
...
- (void)imageDidCapture:(UIImage *)capturedImage fullImage:(BOOL)fullImage{
    ...
    NSString *cacheDir = [[QWERConfigManager globalManager] cachePathOf:self.conversation mediaType:Media_Type_IMAGE];
...
-(void)gifDidCapture:(NSData *)gifData {
    //save gif
    UInt64 recordTime = [[NSDate date] timeIntervalSince1970]*1000;
    NSString *cacheDir = [[QWERConfigManager globalManager] cachePathOf:self.conversation mediaType:Media_Type_STICKER];
...
- (void)recordDidEnd:(NSString *)dataUri duration:(long)duration error:(NSError *)error {
    NSString *cacheDir = [[QWERConfigManager globalManager] cachePathOf:self.conversation mediaType:Media_Type_VOICE];
...
- (void)didSelectFiles:(NSArray *)files {
    if(![[WFCCIMService sharedWFCIMService] isSupportBigFilesUpload]) {
        for (NSString *file in files) {
            ...
            if(content.size >= 100 * 1024 * 1024) {
                UIAlertController *alertController = [UIAlertController alertControllerWithTitle:LLLLLL(@"Warning") message:(_isChinese?@"文件内容超大，无法发送！":@"Nội dung tập tin quá lớn, không thể gửi đi!") preferredStyle:UIAlertControllerStyleAlert];
    ...
    for (NSString *file in files) {
        ...
        if(isDir) {
            ...
            UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"" message:[NSString stringWithFormat:@"%@: %@",(_isChinese?@"无法发送文件夹":@"Không thể gửi thư mục"), file.lastPathComponent] preferredStyle:UIAlertControllerStyleAlert];
...
- (void)saveStickerRemoteUrl:(WFCCStickerMessageContent *)stickerContent {
    if (stickerContent.localPath.length && [QWERUtilities isFileExist:stickerContent.localPath] && stickerContent.remoteUrl.length) {
        if(self.conversation.type == SecretChat_Type) {
...
- (void)didTouchVideoBtn:(BOOL)isAudioOnly {
    if(self.conversation.type == Single_Type || self.conversation.type == SecretChat_Type) {
        RADCOVideoVC *videoVC = [[RADCOVideoVC alloc] initWithTargets:@[self.conversation.target] conversation:self.conversation audioOnly:isAudioOnly];
        [[Chat86AVEngineKit sharedEngineKit] presentViewController:videoVC];
    } else {
        //      FRSDAContactListVC *pvc = [[FRSDAContactListVC alloc] init];
        //      pvc.selectContact = YES;
        //      pvc.multiSelect = [Chat86AVEngineKit sharedEngineKit].supportMultiCall;
        //        if (pvc.multiSelect) {
        //            pvc.maxSelectCount = isAudioOnly ? [Chat86AVEngineKit sharedEngineKit].maxAudioCallCount : [Chat86AVEngineKit sharedEngineKit].maxVideoCallCount;
        //            pvc.maxSelectCount -= 1;
        //        }
        
        FRSDASeletedUserVC *vc = [[FRSDASeletedUserVC alloc] init];
        ...
        vc.maxSelectCount = isAudioOnly ? [Chat86AVEngineKit sharedEngineKit].maxAudioCallCount : [Chat86AVEngineKit sharedEngineKit].maxVideoCallCount;
        ...
        vc.selectResult = ^(NSArray<NSString *> * _Nonnull contacts) {
            ...
            if (ws.conversation.type == Group_Type && [Chat86AVEngineKit sharedEngineKit].supportMultiCall) {
                videoVC = [[RADCOMultiVideoVC alloc] initWithTargets:contacts conversation:ws.conversation audioOnly:isAudioOnly];
            } else {
                videoVC = [[RADCOVideoVC alloc] initWithTargets:contacts conversation:ws.conversation audioOnly:isAudioOnly];
            }
            [[Chat86AVEngineKit sharedEngineKit] presentViewController:videoVC];
        };
...
#pragma mark - menu
- (void)displayMenu:(ASDFGMessageCellBase *)baseCell {
    ...
    CGRect menuPos;
    if ([baseCell isKindOfClass:[ASDFGMessageCell class]]) {
        ASDFGMessageCell *msgCell = (ASDFGMessageCell *)baseCell;
        menuPos = msgCell.asoucBubbleView.frame;
    } else {
    ...
    if ([msg.content isKindOfClass:[WFCCTextMessageContent class]]/* || [msg.content isKindOfClass:LaAnnouncementMessageContent.class]*/) {
    ...
    if(self.conversation.type != SecretChat_Type) {
        if ([baseCell isKindOfClass:[ASDFGMessageCell class]] || [baseCell isKindOfClass:[ASDFGArticlesCell class]]) {
    ...
    if (_conversation.type == Group_Type) {
        if ([msg.content isKindOfClass:[WFCCTextMessageContent class]] ||
            [msg.content isKindOfClass:[WFCCFileMessageContent class]] ||
            [msg.content isKindOfClass:[LaAnnouncementMessageContent class]] ||
            [msg.content isKindOfClass:[WFCCImageMessageContent class]] ||
            [msg.content isKindOfClass:[WFCCVideoMessageContent class]]) {
            if (_isManager) { // 群聊 && 消息类型(文本、文件、公告) && 群主或管理员   才能置顶操作
...
//                    }];
                ...
                UIAlertAction *actionRemoteDelete = [UIAlertAction actionWithTitle:(_isChinese?@"删除服务器消息":@"Xóa tin nhắn máy chủ") style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
                    __block MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:weakSelf.view animated:YES];
                    hud.label.text = LLLLLL(@"Deleting");
                    [hud showAnimated:YES];
                    [[WFCCIMService sharedWFCIMService] deleteRemoteMessage:message.messageUid success:^{
...
-(void)performCancel:(UIMenuController *)sender {
    if (self.cell4Menu) {
        if(![[WFCCIMService sharedWFCIMService] cancelSendingMessage:self.cell4Menu.model.message.messageId]) {
            [self.view makeToast:(_isChinese?@"取消失败":@"Hủy bỏ thất bại") duration:1 position:CSToastPositionCenter];
        }
    }
}
...
-(void)performRecall:(UIMenuItem *)sender {
    if (self.cell4Menu.model.message) {
        ...
        ASDFGMessageCellBase *cell = self.cell4Menu;
...
- (void)performComplain:(UIMenuItem *)sender {
    ...
    if (_isChinese) {
        ...
        message = @"Nếu bạn phát hiện nội dung vi phạm pháp luật hoặc quyền lợi của mình, vui lòng gửi ảnh chụp màn hình cho chúng tôi. Chúng tôi sẽ xử lý trong 24 giờ, bao gồm xóa nội dung, cảnh báo tác giả, khóa tài khoản. Để báo cáo, vui lòng vào mục 'Tôi -> Về thông tin  -> Ý kiến'. Xin cảm ơn!";
    }
...
- (void)performFavorite:(UIMenuItem *)sender {
    if (self.cell4Menu.model.message) {
        QWERFavoriteItem *item = [QWERFavoriteItem itemFromMessage:self.cell4Menu.model.message];
        if (!item) {
            [self.view makeToast:(_isChinese?@"暂不支持":@"Không được hỗ trợ") duration:1 position:CSToastPositionCenter];
            return;
        }
        ...
        __weak typeof(self)ws = self;
        [[QWERConfigManager globalManager].appServiceProvider addFavoriteItem:item success:^{
            NSLog(@"added");
            [ws.view makeToast:(self->_isChinese?@"已收藏":@"Bộ sưu tập") duration:1 position:CSToastPositionCenter];
        } error:^(int error_code) {
            NSLog(@"add failure");
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *callHint = (_isChinese?@"通话正在进行中...":@"Cuộc gọi đang tiến hành...");
    if(userName.length) {
        if (_isChinese) {
            callHint = [NSString stringWithFormat:@"%@ 发起的通话正在进行中...", userName];
        }else {
            callHint = [NSString stringWithFormat:@"Cuộc gọi từ %@ đang tiến hành...", userName];
        }
    }
    
    if(!isFocused) {
        QWERTMultiCallOngoingCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell"];
        if(!cell) {
            cell = [[QWERTMultiCallOngoingCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"cell"];
        }
        cell.callHintLabel.text = callHint;
        return cell;
    } else {
        QWERTMultiCallOngoingExpendedCell *cell = [tableView dequeueReusableCellWithIdentifier:@"expended_cell"];
        if(!cell) {
            cell = [[QWERTMultiCallOngoingExpendedCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"expended_cell"];
            cell.delegate = self;
        }
        cell.callHintLabel.text = callHint;
        return cell;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.focusedOngoingCellIndex == indexPath.row) {
        ...
        if(([ongoing.callId isEqualToString:[Chat86AVEngineKit sharedEngineKit].currentSession.callId] && [Chat86AVEngineKit sharedEngineKit].currentSession.state != kWFAVEngineStateIdle) || [ongoing.targetIds containsObject:[WFCCNetworkService sharedInstance].userId] || [message.fromUser isEqualToString:[WFCCNetworkService sharedInstance].userId]) {
            self.focusedOngoingCellIndex = -1;
        } else {
...
#pragma mark - QWERTMultiCallOngoingExpendedCellDelegate
...
- (void)single_get_option {
    ...
    WS(weakself)
    [AppService.sharedAppService requestUrl:@"/single/get_option" params:@{@"current":WFCCNetworkService.sharedInstance.userId, @"target":_conversation.target} success:^(NSDictionary * _Nonnull dict) {
...
- (void)dealloc {
    ...
    for (NSInteger i = 0; i<self.modelList.count; i++) {
        ASDFGMessageCellBase *cell = [self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForRow:i inSection:0]];
        if (![cell isKindOfClass:ASDFGMessageCell.class]) {
            continue;
        }
        QWERTMessageModel *model = self.modelList[i];
        if (model.message.direction == MessageDirection_Send) { // 如果是发送方 则需要判断该条消息是否已读
            if (((ASDFGMessageCell *)cell).asoucUnreadButton.selected == NO || isRead == NO) { // 该条消息没有人已读
                continue;
            }
        }else {
...
//- (LaAnnouncementTopView *)announcementTopView {
//    if (!_announcementTopView) {
//        _announcementTopView = [[LaAnnouncementTopView alloc] init];
//    }
...
//- (void)showTopAnnouncementView {
//    CGSize size = [QWERUtilities getTextDrawingSize:_groupAnnouncement.text font:[UIFont pingFangSCWithWeight:FontWeightStyleRegular size:13.0] constrainedSize:CGSizeMake(WIDTH-70.0, 8000)];
//    CGFloat height = MIN(56.0, size.height) + 103.0;
//    self.announcementTopView.frame = CGRectMake(20.0, -(103.0+size.height), WIDTH-40.0, height);
//    self.announcementTopView.contentLabel.text = _groupAnnouncement.text;
//
//    [self.view addSubview:self.announcementTopView];
//    WS(weakself)
//    [UIView animateWithDuration:0.5 animations:^{
//        weakself.announcementTopView.frame = CGRectMake(20.0, NavigationHeight+20.0, WIDTH-40.0, height);
//    } completion:^(BOOL finished) {
//    }];
//
//    [self.announcementTopView setPopAnnouncementViewBlock:^(NSInteger type) {
//        if (type == 0) { // type 0 知道了   1 点击整个view
//            [UIView animateWithDuration:0.5 animations:^{
//                weakself.announcementTopView.frame = CGRectMake(20.0, -(103.0+size.height), WIDTH-40.0, height);
//            } completion:^(BOOL finished) {
//                [weakself.announcementTopView removeFromSuperview];
//            }];
//        }else {
//            LaGroupAnnouncementVC *vc = [[LaGroupAnnouncementVC alloc] init];
//            vc.isCanPost = NO;
//            vc.announcement = weakself.groupAnnouncement;
//            if ([weakself.targetGroup.owner isEqualToString:weakself.groupAnnouncement.author]) {
//                vc.type = Member_Type_Owner;
//            }
//            if ([weakself isGroupManager:weakself.groupAnnouncement.author]) {
//                vc.type = Member_Type_Manager;
//            }
...
- (void)topMessageList {
    ...
    WS(weakself)
    [AppService.sharedAppService requestUrl:@"/group/message/top/list" params:@{@"groupId":_conversation.target} success:^(NSDictionary * _Nonnull dict) {
        weakself.topMessages = [MessageTopList mj_objectArrayWithKeyValuesArray:(NSArray *)dict[@"result"]];
...
- (LaTopMessageView *)topMessageView {
    if (!_topMessageView) {
        _topMessageView = [[LaTopMessageView alloc] init];
        ...
        [_topMessageView.coaeoxRemoveBtn addTarget:self action:@selector(coaeoxRemove:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)clickTopMessage {
    ...
    if (self.topMessages.count == 1) { // 有且只有1个置顶消息、并且是更新群公告消息
        if (self.topMessageView.coaeoxRemoveBtn.selected) {
            self.topMessageView.coaeoxRemoveBtn.selected = NO;
            self.topMessageView.coaeoxRemoveBtn.backgroundColor = UIColor.clearColor;
            return;
        }
...
//            [NSUserDefaults.standardUserDefaults setValue:UNString(@"%ld", _groupAnnouncement.timestamp) forKey:kAnnouncementTimestamp];
    ...
    AllTopMessagePopupView *popupView = [[AllTopMessagePopupView alloc] init];
    WS(weakself)
...
- (void)dealData:(MessageTopList *)topList {
    ...
    if (topList.content.type == 1001) { // 1001  公告消息
        ...
        //图片
        LaTopMessageShowViewController *vc = [[LaTopMessageShowViewController alloc] init];
        vc.topList = topList;
        [self.navigationController pushViewController:vc animated:YES];
