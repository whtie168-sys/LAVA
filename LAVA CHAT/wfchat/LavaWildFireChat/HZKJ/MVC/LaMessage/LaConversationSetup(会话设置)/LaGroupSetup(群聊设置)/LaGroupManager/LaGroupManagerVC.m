...
@implementation LaGroupManagerVC
...
- (void)viewDidLoad {
    ...
    if (_isChinese) {
        ...
        _managerL.text = @"Quản trị viên";
        _needReviewL.text = @"Giấy phép tham gia nhóm";
        _addFirdentL.text = @"Cấm thêm bạn lẫn nhau";
        _allMuteL.text = @"Cấm chat toàn bộ thành viên";
        ...
        _disbandL.text = @"Chuyển nhóm";
        _memberL.text = @"Giấy phép thành viên";
...
- (void)onReceiveMessages:(NSNotification *)notification {
    _groupInfo = [[WFCCIMService sharedWFCIMService] getGroupInfo:_groupInfo.target refresh:YES];
//    [self initUiData];
}

// 管理员
- (IBAction)administrator:(UIButton *)sender {
    LaAdministratorVC *mtvc = [[LaAdministratorVC alloc] init];
...
- (void)transferGroup:(NSString *)newOwner {
    ...
    if (_isChinese) {
        ...
        message = UNString(@"\nSau khi nhóm trò chuyện được chuyển giao, bạn sẽ tự động trở thành thành viên bình thường và mất mọi quyền sở hữu nhóm. %@ sẽ trở thành chủ nhóm mới.", name);
    }
...
- (void)requestStateSW:(UISwitch *)sw params:(NSDictionary *)dic {
    ...
    [AppService.sharedAppService requestUrl:@"/group/set_option" params:params success:^(NSDictionary * _Nonnull dict) {
