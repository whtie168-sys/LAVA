...
@property (nonatomic, strong) NSNumber *controlOther;
@property (nonatomic, strong) NSNumber *modifyGroupInfo;
@property (nonatomic, strong) NSNumber *pushNotice;
@property (nonatomic, strong) NSNumber *renewRequest;
...
- (void)viewDidLoad {
    ...
    self.permissions = @[LLLLLL(@"ModifyGroupData"), LLLLLL(@"GroupAccessManagerVercode"), LLLLLL(@"GroupAccessManagerPublishMsg"), LLLLLL(@"GroupAccessManagerSetOther")];
...
- (void)get_option {
    ...
    [AppService.sharedAppService requestUrl:@"/group/member/get_option" params:params success:^(NSDictionary * _Nonnull dict) {
        [hud hideAnimated:YES];

        weakself.controlOther = dict[@"result"][@"controlOther"];
        weakself.modifyGroupInfo = dict[@"result"][@"modifyGroupInfo"];
        weakself.pushNotice = dict[@"result"][@"pushNotice"];
        weakself.renewRequest = dict[@"result"][@"renewRequest"];
...
- (void)set_option {
    ...
    [params setObject:[NSString stringWithFormat:@"%d",[_controlOther intValue]] forKey:@"controlOther"];
    [params setObject:[NSString stringWithFormat:@"%d",[_modifyGroupInfo intValue]] forKey:@"modifyGroupInfo"];
    [params setObject:[NSString stringWithFormat:@"%d",[_pushNotice intValue]] forKey:@"pushNotice"];
    [params setObject:[NSString stringWithFormat:@"%d",[_renewRequest intValue]] forKey:@"renewRequest"];

    [AppService.sharedAppService requestUrl:@"/group/member/set_option" params:params success:^(NSDictionary * _Nonnull dict) {
        [hud hideAnimated:YES];
...
- (void)managerDelete {
    ...
    [params setObject:_groupInfo.target forKey:@"groupId"];
    [params setObject:_userId forKey:@"memberId"];
    [AppService.sharedAppService requestUrl:@"/group/manager/delete" params:params success:^(NSDictionary * _Nonnull dict) {
        [hud hideAnimated:YES];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    WFCCUserInfo *userInfo = [[WFCCIMService sharedWFCIMService] getUserInfo:self.userId inGroup:self.groupInfo.target refresh:NO];
    [avatar sd_setImageWithURL:URL(userInfo.portrait) placeholderImage: nil  options:SDWebImageScaleDownLargeImages
    ...
    if (userInfo.friendAlias.length) {
        nameLabel.text = userInfo.friendAlias;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([item isEqualToString:LLLLLL(@"ModifyGroupData")]) {
        ...
        [cell setBlock:^(BOOL ison) {
            self.modifyGroupInfo = [NSNumber numberWithBool:ison];
            [self set_option];
        }];
        ...
        [cell setBlock:^(BOOL ison) {
            self.renewRequest = [NSNumber numberWithBool:ison];
            [self set_option];
        }];
        ...
        [cell setBlock:^(BOOL ison) {
            self.pushNotice = [NSNumber numberWithBool:ison];
            [self set_option];
        }];
        ...
        [cell setBlock:^(BOOL ison) {
            self.controlOther = [NSNumber numberWithBool:ison];
            [self set_option];
        }];
    }
    return cell;
