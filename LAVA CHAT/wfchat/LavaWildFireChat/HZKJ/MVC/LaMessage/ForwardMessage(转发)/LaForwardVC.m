...
#import "LaForwardContactsTVCell.h"
...
@interface LaForwardVC ()<UITableViewDataSource, UISearchControllerDelegate, UITableViewDelegate, UITableViewDataSource, UISearchResultsUpdating>
{
    BOOL _isChinese;
}
...
- (void)viewDidLoad {
    ...
    [self.tableView registerNib:[UINib nibWithNibName:@"LaForwardContactsTVCell" bundle:[NSBundle mainBundle]] forCellReuseIdentifier:@"LaForwardContactsTVCell"];
...
- (void)sortAndRefreshWithList:(NSArray *)friendList {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        self.resultDic = [LaForwardVC sortedArrayWithPinYinDic:friendList];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if (self.searchController.active || self.selectContact) {
        ...
        if (section == 0) {
            return 2 + [ESZQSCOrganizationCache sharedCache].rootOrganizationIds.count + [ESZQSCOrganizationCache sharedCache].bottomOrganizationIds.count;
        } else {
            dataSource = self.allFriendSectionDic[self.allKeys[section - 1]];
            return dataSource.count;
        }
    ...
    if (self.searchController.active || self.selectContact) {
        ...
        if (indexPath.section == 0) {
            if (indexPath.row == 0) {
                ...
                contactCell.asoucNameLabel.text = LLLLLL(@"SelectFriend");
                contactCell.trewqPortraitView.image = IMAGENAME(@"xaicosgoeNew1");
                contactCell.trewqPortraitView.contentMode = UIViewContentModeCenter;
                [contactCell refresh];
                contactCell.separatorInset = UIEdgeInsetsMake(0, 80.0, 0, 0);
                ...
                contactCell.separatorInset = UIEdgeInsetsMake(0, 80.0, 0, 0);
                contactCell.asoucNameLabel.text = LLLLLL(@"SelectGroupChat");
                contactCell.trewqPortraitView.image = IMAGENAME(@"xaicosgoeGroup1");
                contactCell.trewqPortraitView.contentMode = UIViewContentModeCenter;
                contactCell.asoucNameLabel.textColor = [QWERConfigManager globalManager].textColor;
        ...
        if (indexPath.section == 0 && !self.searchController.active) {
            if (indexPath.row == 0) {
              ...
              [contactCell refresh];
                ...
                contactCell.asoucNameLabel.text = LLLLLL(@"NewFriend");
              ...
              cell = contactCell;
                ...
                FRSDAContactTVCell *contactCell = [self dequeueOrAllocFavGroupCell:tableView];
                contactCell.isHiddenLine = NO;
                contactCell.asoucNameLabel.text = LLLLLL(@"GroupChat");
...
//            cell = contactCell;
            ...
            LaForwardContactsTVCell *cell = [tableView dequeueReusableCellWithIdentifier:@"LaForwardContactsTVCell" forIndexPath:indexPath];
            WFCCUserInfo *userInfo = dataSource[indexPath.row];
...
//                addFriendVC.hidesBottomBarWhenPushed = YES;
//                [self.navigationController pushViewController:addFriendVC animated:YES];
                SelectLaContactVC *vc = SelectLaContactVC.new;
                vc.message = self.message;
                vc.messages= self.messages;
                vc.hidesBottomBarWhenPushed = YES;
                [self.navigationController pushViewController:vc animated:YES];
            } else if(indexPath.row == 1) { // 我的群组
                SelectLaGroupVC *groupVC = [[SelectLaGroupVC alloc] init];;
...
+ (NSMutableDictionary *)sortedArrayWithPinYinDic:(NSArray *)userList {
    ...
    [infoDic enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
        ...
        [_tempOtherArr sortUsingComparator:^NSComparisonResult(id  _Nonnull obj1, id  _Nonnull obj2) {
            ...
            NSString *user1Pinyin = [LaForwardVC hanZiToPinYinWithString:user1.displayName];
            NSString *user2Pinyin = [LaForwardVC hanZiToPinYinWithString:user2.displayName];
