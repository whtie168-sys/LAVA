...
@interface LaContactsVC (TagAddMemberSorting)
...
- (void)configureWithGroup:(WFCCGroupInfo *)groupInfo
    ...
    if (groupInfo.portrait.length > 0) {
        [_groupIconView sd_setImageWithURL:[NSURL URLWithString:groupInfo.portrait]
                          placeholderImage:[QWERImage imageNamed:@"groupIcon"]];
    } else {
        _groupIconView.image = [QWERImage imageNamed:@"groupIcon"];
    }
}
...
- (void)rebuildSectionsWithKeyword:(NSString *)keyword {
    ...
    NSDictionary *result = [LaContactsVC sortedArrayWithPinYinDic:self.visibleFriends] ?: @{};
...
- (void)updateAddButtonState {
    ...
    self.addButton.backgroundColor = enabled ? [UIColor colorWithRed:0x17 / 255.0 green:0x9F / 255.0 blue:0xFE / 255.0 alpha:1.0] : [UIColor colorWithWhite:0.88 alpha:1.0];
...
- (NSString *)displayNameForUser:(WFCCUserInfo *)userInfo {
    return userInfo.displayName.length > 0 ? userInfo.displayName :
        (userInfo.friendAlias.length > 0 ? userInfo.friendAlias :
         (userInfo.displayName.length > 0 ? userInfo.displayName : userInfo.userId));
}
...
- (void)updateImportTagButtonState {
    ...
    self.importTagButton.backgroundColor = enabled ? [UIColor colorWithRed:0x17 / 255.0 green:0x9F / 255.0 blue:0xFE / 255.0 alpha:1.0] : [UIColor colorWithWhite:0.88 alpha:1.0];
...
- (void)updateImportGroupButtonState {
    ...
    self.importGroupButton.backgroundColor = enabled ? [UIColor colorWithRed:0x17 / 255.0 green:0x9F / 255.0 blue:0xFE / 255.0 alpha:1.0] : [UIColor colorWithWhite:0.88 alpha:1.0];
...
- (void)updateGroupMemberBottomState {
    ...
    self.groupMemberAddButton.backgroundColor = enabled ? [UIColor colorWithRed:0x17 / 255.0 green:0x9F / 255.0 blue:0xFE / 255.0 alpha:1.0] : [UIColor colorWithWhite:0.88 alpha:1.0];
...
- (void)refreshSelectedMembersPreview {
    ...
    for (WFCCUserInfo *userInfo in selectedUsers) {
        ...
        if (userInfo.portrait.length > 0) {
            ...
            avatarView.image = [QWERImage imageNamed:@"PersonalChat"];
        }
