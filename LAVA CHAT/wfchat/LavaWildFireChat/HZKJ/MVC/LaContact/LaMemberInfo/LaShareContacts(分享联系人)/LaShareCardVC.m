...
@interface LaShareCardVC ()<UITableViewDataSource, UITableViewDelegate, UISearchControllerDelegate, UISearchResultsUpdating>
{
    BOOL _isChinese;
}
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = _isChinese?@"发送给":@"Gửi đến";
    ...
    [_tableView registerNib:[UINib nibWithNibName:@"LaShareIconTVCell" bundle:NSBundle.mainBundle] forCellReuseIdentifier:@"LaShareIconTVCell"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (_searchController.active || indexPath.section == 1) {
        LaShareIconTVCell *cell = [tableView dequeueReusableCellWithIdentifier:@"LaShareIconTVCell" forIndexPath:indexPath];
        ...
        cell.textLabel.text = (indexPath.row == 0 ? (_isChinese?@"选择朋友":@"Chọn bạn bè") : (_isChinese?@"选择群聊":@"Chọn nhóm"));
...
- (void)send:(UIButton *)sender {
    ...
    [[WFCCIMService sharedWFCIMService] send:info.conversation content:card success:^(long long messageUid, long long timestamp) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [sender setTitle:(self->_isChinese?@"已发送":@"Đã được gửi") forState:UIControlStateNormal];
...
// view上设置背景色无效。 请使用方法 willDisplayHeaderView
    ...
    view.oxaicsgoeTitleLabel.textColor = RGBA(0x222222);
    view.oxaicsgoeTitleLabel.font = PINGFANG_R(14.0);
    if (section == 0 || _dataList.count == 0) {
        view.oxaicsgoeTitleLabel.text = @"";
    }else {
        view.oxaicsgoeTitleLabel.text = (_isChinese?@"最近":@"Gần đây");
    }
    return view;
}
...
@implementation LaShareIconTVCell
...
- (void)setInfo:(WFCCConversationInfo *)info {
    ...
    if (info.conversation.type == Single_Type) {
        ...
        [_imgView sd_setImageWithURL:URL(userInfo.portrait) placeholderImage: [QWERImage imageNamed:@"PersonalChat"] options:SDWebImageScaleDownLargeImages
        ...
        if (userInfo.friendAlias.length) {
            ...
            _asoucNameLabel.text = [NSString stringWithFormat:@"Người dùng<%@>",  _info.conversation.target];
        }
        ...
        [_imgView sd_setImageWithURL:URL(groupInfo.portrait) placeholderImage:[QWERImage imageNamed:@"groupIcon"] options:SDWebImageScaleDownLargeImages
                             context:@{SDWebImageContextImageForceDecodePolicy : @(SDImageForceDecodePolicyNever), SDWebImageContextStoreCacheType : @(SDImageCacheTypeDisk)}];
        if (groupInfo.displayName.length > 0) {
            _asoucNameLabel.text = groupInfo.displayName;
        }else {
            _asoucNameLabel.text = LLLLLL(@"GroupChat");
        }
    }
}
