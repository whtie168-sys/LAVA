...
@interface LaGroupVC ()<UITableViewDataSource, UITableViewDelegate, UISearchControllerDelegate, UISearchResultsUpdating>
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_type == 1) { // 分享联系人到群聊
        ...
        WFCCUserInfo *targetUserinfo = [[WFCCIMService sharedWFCIMService] getUserInfo:_target refresh:NO];
...
//            @param fromUser 分享用户。
        ...
        if ([CommonHelper.main isChinese]) {
            [popView showCommand:[NSString stringWithFormat:@"将%@发送给%@",(targetUserinfo.friendAlias.length > 0 ? targetUserinfo.friendAlias : targetUserinfo.displayName), groupInfo.displayName] imgA:targetUserinfo.portrait imB:groupInfo.portrait];
        }else {
//            [popView showCommand:[NSString stringWithFormat:@"%@ gửi %@",(targetUserinfo.friendAlias.length > 0 ? targetUserinfo.friendAlias : targetUserinfo.displayName), groupInfo.displayName] imgA:targetUserinfo.portrait imB:groupInfo.portrait];
            
            [popView showCommand:[NSString stringWithFormat:@"Của %@ đến %@",(targetUserinfo.friendAlias.length > 0 ? targetUserinfo.friendAlias : targetUserinfo.displayName), groupInfo.displayName] imgA:targetUserinfo.portrait imB:groupInfo.portrait];
...
- (void)setGroupInfo:(WFCCGroupInfo *)groupInfo {
    ...
    if (groupInfo.displayName.length == 0) {
        _asoucNameLabel.text = [NSString stringWithFormat:@"%@(%d)",LLLLLL(@"GroupChat") ,(int)groupInfo.memberCount];
    } else {
        _asoucNameLabel.text = [NSString stringWithFormat:@"%@(%d)", groupInfo.displayName, (int)groupInfo.memberCount];
    }

//    if (groupInfo.portrait.length) {
        [_portraitImgView sd_setImageWithURL:URL(groupInfo.portrait) placeholderImage:[QWERImage imageNamed:@"groupIcon"]];
//    } else {
//        __weak typeof(self)ws = self;
...
//                [ws.portraitImgView sd_setImageWithURL:[NSURL fileURLWithPath:path] placeholderImage:[QWERImage imageNamed:@"groupIcon"]];
//            }
//        }];
//        
...
//            return [QWERImage imageNamed:@"groupIcon"];
//        }];
//        if (path) {
//            [_portraitImgView sd_setImageWithURL:[NSURL fileURLWithPath:path] placeholderImage:[QWERImage imageNamed:@"groupIcon"]];
//        }
//    }
}
