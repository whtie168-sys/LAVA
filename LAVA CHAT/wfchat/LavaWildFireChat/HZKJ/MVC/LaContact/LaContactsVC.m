...
@interface LaContactsVC ()<UITableViewDataSource, UISearchControllerDelegate, UITableViewDelegate, UITableViewDataSource, UISearchResultsUpdating>
{
    BOOL _isChinese;
}
...
- (void)sortAndRefreshWithList:(NSArray *)friendList {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        self.resultDic = [LaContactsVC sortedArrayWithPinYinDic:friendList];
    ...
    if (self.searchController.active || self.selectContact) {
        ...
        if (indexPath.section == 0) {
            if (indexPath.row == 0) {
                ...
                FRSDAContactTVCell *contactCell = [self dequeueOrAllocFavGroupCell:tableView];
                contactCell.separatorInset = UIEdgeInsetsMake(0, 80.0, 0, 0);
                contactCell.asoucNameLabel.text = LLLLLL(@"Biaoqian");
...
//                addFriendVC.hidesBottomBarWhenPushed = YES;
//                [self.navigationController pushViewController:addFriendVC animated:YES];
                LaNewFriendVC *vc = LaNewFriendVC.new;
                vc.hidesBottomBarWhenPushed = YES;
                [self.navigationController pushViewController:vc animated:YES];
...
//                [self.navigationController pushViewController:groupVC animated:YES];
                LaGroupVC *groupVC = [[LaGroupVC alloc] init];;
                groupVC.hidesBottomBarWhenPushed = YES;
                [self.navigationController pushViewController:groupVC animated:YES];
        ...
        if (self.multiSelect) {
            ...
            if ([self.selectedContacts containsObject:userInfo.userId]) {
                ...
                if (self.maxSelectCount > 0 && self.selectedContacts.count >= self.maxSelectCount) {
                    [self.view makeToast:@"Không vượt quy định"];
                    return;
                }
...
+ (NSMutableDictionary *)sortedArrayWithPinYinDic:(NSArray *)userList {
    ...
    [infoDic enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
        ...
        [_tempOtherArr sortUsingComparator:^NSComparisonResult(id  _Nonnull obj1, id  _Nonnull obj2) {
            ...
            NSString *user1Pinyin = [LaContactsVC hanZiToPinYinWithString:user1.displayName];
            NSString *user2Pinyin = [LaContactsVC hanZiToPinYinWithString:user2.displayName];
