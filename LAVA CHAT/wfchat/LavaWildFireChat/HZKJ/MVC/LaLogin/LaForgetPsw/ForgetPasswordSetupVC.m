...
@interface ForgetPasswordSetupVC ()<UITextFieldDelegate>
...
@implementation ForgetPasswordSetupVC
...
- (void)viewDidLoad {
    ...
    if ([CommonHelper.main isChinese]) {
        ...
        _newsPswTF.placeholder = @"Mật khẩu mới";
        _newsPswATF.placeholder = @"Xác nhận";
...
- (IBAction)ok:(UIButton *)sender {
    ...
    if (_type == 0) { // 通过手机号找回密码
        NSDictionary *params = @{@"mobile":_account, @"resetCode":_code, @"newPassword":_newsPswTF.text, @"area":_area};
        WS(weakself)
        ...
        WS(weakself)
        NSDictionary *params = @{@"email":_account, @"resetCode":_code, @"newPassword":_newsPswTF.text};
        [AppService.sharedAppService requestUrlNoLogin:@"/reset_forgot_password_email_pwd" params:params success:^(NSDictionary * _Nonnull dict) {
            [hud hideAnimated:YES];
