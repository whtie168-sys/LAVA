...
- (void)configureWithArticle:(WFCCCommunity *)article
    ...
    self.titleLabel.text = article.title.length > 0 ? article.title : article.summary;
    ...
    [self.avatarView sd_setImageWithURL:URL(portrait) placeholderImage:[QWERImage imageNamed:@"PersonalChat"] options:SDWebImageScaleDownLargeImages];
}
