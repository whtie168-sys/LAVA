...
- (void)buildViews {
    ...
    UIImage *publishImage = [UIImage imageNamed:@"community_public"] ?: [QWERImage imageNamed:@"community_public"];
...
- (UIButton *)authorButtonWithAuthor:(WFCCCommunityUser *)author itemWidth:(CGFloat)itemWidth {
    ...
    [avatarView sd_setImageWithURL:URL(author.portrait) placeholderImage:[QWERImage imageNamed:@"PersonalChat"] options:SDWebImageScaleDownLargeImages];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *date = [self shortDateFromString:article.createTime.length > 0 ? article.createTime : article.updateTime];
...
- (NSString *)shortDateFromString:(NSString *)dateString {
    ...
    NSString *normalized = [[value stringByReplacingOccurrencesOfString:@"/" withString:@"-"] stringByReplacingOccurrencesOfString:@"." withString:@"-"];
    if (normalized.length >= 10) {
        return [normalized substringToIndex:10];
    }
    return normalized;
}
