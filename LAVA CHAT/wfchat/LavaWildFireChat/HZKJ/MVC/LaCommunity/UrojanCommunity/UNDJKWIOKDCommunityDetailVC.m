...
- (void)buildViews {
    ...
    self.avatarView.image = [QWERImage imageNamed:@"PersonalChat"];
...
- (void)renderArticle:(WFCCCommunity *)article {
    ...
    self.dateLabel.text = [self shortDateFromString:article.createTime.length > 0 ? article.createTime : article.updateTime];
...
- (void)updateAuthorAvatar:(WFCCCommunity *)article {
    ...
    if (portrait.length == 0) {
        ...
        if (authorUid.length > 0) {
            WFCCUserInfo *userInfo = [[WFCCIMService sharedWFCIMService] getUserInfo:authorUid refresh:NO];
            portrait = userInfo.portrait ?: @"";
        }
    }
    [self.avatarView sd_setImageWithURL:URL(portrait)
                       placeholderImage:[QWERImage imageNamed:@"PersonalChat"]
