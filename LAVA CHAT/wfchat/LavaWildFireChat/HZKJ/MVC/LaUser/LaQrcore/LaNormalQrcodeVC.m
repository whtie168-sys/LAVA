...
@implementation LaNormalQrcodeVC
...
- (void)viewDidLoad {
    ...
    if (_isChinese) {
        ...
        [_saoButton setTitle:@"Quét mã" forState:UIControlStateNormal];
        [_saveButton setTitle:@"Lưu hình ảnh" forState:UIControlStateNormal];
    }
    
    _groupasoucNameLabel.text = @"";
    if (_qrType == QRType_User) {
        self.navigationItem.title = _isChinese ? @"我的二维码" : @"Mã QR của tôi";
        _qrStr = [NSString stringWithFormat:@"wildfirechat://user/%@", self.target];
        _descLabel.text = _isChinese ? @"打开App扫码加我为好友" : @"Mở APP quét mã để thêm bạn";
        ...
        self.navigationItem.title = _isChinese ? @"群组二维码" : @"Mã QR nhóm";
        ...
        _descLabel.text = _isChinese ? @"打开App扫码入群" : @"Mở APP quét mã để thêm bạn";
        _groupasoucNameLabel.text = self.groupInfo.name;
        ...
        self.navigationItem.title = _isChinese ? @"会议二维码" : @"Mã qr của cuộc họp";
        _descLabel.text = _isChinese ? @"打开App扫码进入会议室" : @"Mở mã quét App và vào phòng họp";
...
//    if (groupInfo.portrait.length) {
        [_iconView sd_setImageWithURL:URL(groupInfo.portrait) placeholderImage:IMAGENAME(@"groupIcon") options:SDWebImageScaleDownLargeImages
                              context:@{SDWebImageContextImageForceDecodePolicy : @(SDImageForceDecodePolicyNever), SDWebImageContextStoreCacheType : @(SDImageCacheTypeDisk)}];
        _groupasoucNameLabel.text = _groupInfo.name;
//    } else {
...
- (IBAction)save:(UIButton *)sender {
    if (_target.length <= 0) {
        [SVProgressHUD showErrorWithStatus:(_isChinese ? @"请等待数据加载" : @"Vui lòng chờ dữ liệu được tải lên")];
