...
@interface LaTextModifyVC ()<UITextFieldDelegate,UITextViewDelegate>
{
    BOOL _isChinese;
}
...
- (void)viewDidLoad {
    ...
    if (_modifyType == Modify_DisplayName) {
        self.navigationItem.title = _isChinese ? @"修改昵称" : @"Sửa đổi biệt danh";
        _textTF.placeholder = _isChinese ? @"请输入昵称" : @"Hãy nhập biệt hiệu";
        [self showTip];
    }else if (_modifyType == 100) {
        self.navigationItem.title = _isChinese ? @"修改账号" : @"Sửa đổi tài khoản";
        _textTF.placeholder = _isChinese ? @"请输入账号" : @"Xin vui lòng nhập vào tài khoản";
    }else if (_modifyType == Modify_Email) {
        self.navigationItem.title = _isChinese ? @"修改邮箱" : @"Thay đổi hộp thư";
        _textTF.placeholder = _isChinese ? @"请输入邮箱" : @"Xin vui lòng nhập vào hộp thư";
    }else if (_modifyType == Modify_Mobile) {
        self.navigationItem.title = _isChinese ? @"修改手机号" : @"Sửa đổi số điện thoại di động";
        _textTF.placeholder = _isChinese ? @"请输入手机号" : @"Vui lòng nhập số điện thoại";
        ...
        self.navigationItem.title = _isChinese ? @"修改我在群中的昵称" : @"Thay đổi biệt danh của tôi trong nhóm";
        _textTF.placeholder = _isChinese ? @"请输入群昵称" : @"Hãy nhập biệt hiệu nhóm";
    }else if (_modifyType == 102) {
        self.navigationItem.title = _isChinese ? @"群聊名称" : @"Nhóm tên";
        _textTF.placeholder = _isChinese ? @"请输入群聊名称" : @"Hãy nhập tên nhóm";
    }else if (_modifyType == Modify_FriendAlias) {
        self.navigationItem.title = _isChinese ? @"备注名" : @"Lưu ý tên";
        _textTF.placeholder = _isChinese ? @"请输入备注名" : @"Hãy nhập tên ghi chú";
    }
    ...
    if (_modifyType == Modify_Sign) {
        ...
        placeHolderLabel.text = _isChinese ? @"填写个性签名..." : @"Ngày sinh...";
...
- (IBAction)save:(UIButton *)sender {
        ...
        if (_modifyType == 101 || _modifyType == Modify_FriendAlias) { // 该值可以为空
            ...
            if (_modifyType == 100) { // 修改账户名
                if (_textTF.text.length <= 2) {
                    [SVProgressHUD showErrorWithStatus:_isChinese ? @"请输入3位及以上字符" : @"Hãy nhập ba ký tự hoặc nhiều hơn"];
                    [SVProgressHUD dismissWithDelay:1.0];
                    return;
                }
            }
    ...
    WS(weakself)
    if (_modifyType == 100) { // 修改账户名
        [[QWERConfigManager globalManager].appServiceProvider changeName:_textTF.text success:^{
...
//            [hud hideAnimated:YES afterDelay:1.f];
        ...
        [[WFCCIMService sharedWFCIMService] modifyGroupAlias:_groupId alias:_textTF.text notifyLines:@[@(0)] notifyContent:nil success:^{
            ...
            [hud hideAnimated:YES];
            [weakself.view makeToast:(self->_isChinese ? @"群昵称修改失败" : @"Biệt hiệu nhóm sửa đổi thất bại") duration:1.0 position:CSToastPositionCenter];
        }];
        ...
        [AppService.sharedAppService requestUrl:@"/user/set_option" params:@{@"sign":_contentTV.text} success:^(NSDictionary * _Nonnull dict) {
            [hud hideAnimated:YES];
            ...
            hud = [MBProgressHUD showHUDAddedTo:weakself.view animated:YES];
            hud.mode = MBProgressHUDModeText;
            hud.label.text = (errCode == ERROR_CODE_NOT_MODIFIED ? (self->_isChinese ? @"未修改成功！" : @"Không sửa đổi thành công") : (self->_isChinese ? @"更新失败！" : @"Thất bại!"));
            hud.offset = CGPointMake(0.f, MBProgressMaxOffset);
            [hud hideAnimated:YES afterDelay:1.f];
        }];
    }else if (_modifyType == Modify_FriendAlias) {
        ...
        [WFCCIMService.sharedWFCIMService setFriend:self.userId alias:_textTF.text success:^{
            ...
            hud = [MBProgressHUD showHUDAddedTo:weakself.view animated:YES];
            hud.mode = MBProgressHUDModeText;
            hud.label.text = (error_code == ERROR_CODE_NOT_MODIFIED ? (self->_isChinese ? @"未修改成功！" : @"Không sửa đổi thành công") : (self->_isChinese ? @"更新失败！" : @"Thất bại!"));
            hud.offset = CGPointMake(0.f, MBProgressMaxOffset);
            [hud hideAnimated:YES afterDelay:1.f];
        }];
