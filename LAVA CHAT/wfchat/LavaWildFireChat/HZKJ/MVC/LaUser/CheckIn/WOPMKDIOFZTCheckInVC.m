...
- (void)configWithTask:(WFCCSignTask *)task selected:(BOOL)selected {
    ...
    self.nameLabel.text = taskName.length ? taskName : (creatorName.length ? creatorName : LLLLLL(@"CheckInDefaultUserName"));
...
- (void)setupBackgroundView {
    UIImage *backgroundImage = [UIImage imageNamed:@"me_checkIn_bg"];
    if (!backgroundImage) {
        return;
    }
...
- (void)setupUserView {
    ...
    self.userNameLabel.text = LLLLLL(@"CheckInDefaultUserName");
...
- (void)setupCheckInCard {
    ...
    [self.checkButton setTitle:LLLLLL(@"CheckInButtonNow") forState:UIControlStateNormal];
    ...
    self.walletHintLabel.text = LLLLLL(@"CheckInWalletHint");
...
- (void)setupRuleView {
    ...
    self.ruleTitleLabel.text = LLLLLL(@"CheckInRuleTitle");
...
- (void)loadSignTasks {
    ...
    [[AppService sharedAppService] signTasks:^(WFCCSignTasks * _Nonnull tasks) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [hud hideAnimated:YES];
            strongSelf.loading = NO;
            [strongSelf showTextHUD:(message.length ? message : LLLLLL(@"CheckInLoadFailed"))];
            [strongSelf refreshContent];
        });
    }];
}
...
- (void)refreshContent {
    ...
    NSString *taskName = WOPMKDIOFZTSafeText(task.taskName);
    NSString *creatorName = WOPMKDIOFZTSafeText(task.creatorDisplayName);
    NSString *userName = taskName.length ? taskName : (creatorName.length ? creatorName : LLLLLL(@"CheckInNoTaskUser"));
    self.userNameLabel.text = userName;
    NSString *rewardDesc = WOPMKDIOFZTSafeText(task.rewardDesc);
    self.ruleLabel.text = rewardDesc.length ? rewardDesc : LLLLLL(@"CheckInNoRule");
    ...
    [self.checkButton setTitle:(hasTask ? (todaySigned ? LLLLLL(@"CheckInTodaySigned") : LLLLLL(@"CheckInButtonNow")) : LLLLLL(@"CheckInNoActivity")) forState:UIControlStateNormal];
}
...
- (void)checkButtonAction {
    ...
    [[AppService sharedAppService] signSubmit:@{@"taskId" : self.selectedTask.taskId} success:^(WFCCSign * _Nonnull sign) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [hud hideAnimated:YES];
            strongSelf.loading = NO;
            [strongSelf showTextHUD:(message.length ? message : LLLLLL(@"CheckInFailed"))];
            [strongSelf loadSignTasks];
        });
    }];
}
...
- (void)showSuccessPanelWithPoints:(NSInteger)points {
    ...
    titleLabel.text = LLLLLL(@"CheckInSuccess");
    ...
    hintLabel.text = LLLLLL(@"CheckInSuccessHint");
    ...
    [confirmButton setTitle:LLLLLL(@"Confirm") forState:UIControlStateNormal];
...
- (void)setupSelectorPanel {
    ...
    UILabel *titleLabel = [[UILabel alloc] init];
    titleLabel.translatesAutoresizingMaskIntoConstraints = NO;
    titleLabel.text = LLLLLL(@"CheckInSelectUser");
    titleLabel.textAlignment = NSTextAlignmentCenter;
    ...
    [confirmButton setTitleColor:[UIColor colorWithRed:0x17 / 255.0 green:0x9F / 255.0 blue:0xFE / 255.0 alpha:1.0] forState:UIControlStateNormal];
...
- (NSAttributedString *)continuousText:(NSInteger)days {
    NSString *text = [NSString stringWithFormat:LLLLLL(@"CheckInContinuousDaysFormat"), (long)days];
    ...
    if (range.location != NSNotFound) {
        [attr addAttributes:@{NSForegroundColorAttributeName : [UIColor colorWithRed:0x17 / 255.0 green:0x9F / 255.0 blue:0xFE / 255.0 alpha:1.0]} range:range];
    }
    return attr;
}
