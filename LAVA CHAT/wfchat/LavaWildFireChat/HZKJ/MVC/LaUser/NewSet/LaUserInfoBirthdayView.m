...
@interface LaUserInfoBirthdayView()<UIPickerViewDelegate, UIPickerViewDataSource>
...
@implementation LaUserInfoBirthdayView
...
- (void)setupDateData {
    ...
    for (NSInteger year = 1900; year <= 2100; year++) {
        [yearsArray addObject:[NSString stringWithFormat:@"%ld", year]];
    }
...
- (void)updateDaysForYear:(NSInteger)year month:(NSInteger)month {
    ...
    for (NSInteger day = 1; day <= daysInMonth; day++) {
        [daysArray addObject:[NSString stringWithFormat:@"%ld", day]];
    }
    self.days = daysArray;
}
...
- (void)showDatePicker {
    if (!self.pickerContainerView) {
        ...
        [confirmButton setTitle:LLLLLL(@"AlertDone") forState:UIControlStateNormal];
        ...
        [NSLayoutConstraint activateConstraints:@[
            ...
            [confirmButton.widthAnchor constraintEqualToConstant:80],
        ...
        self.selectedDay = 18;
        [self.datePicker selectRow:[self.years indexOfObject:[NSString stringWithFormat:@"2025"]] inComponent:0 animated:NO];
