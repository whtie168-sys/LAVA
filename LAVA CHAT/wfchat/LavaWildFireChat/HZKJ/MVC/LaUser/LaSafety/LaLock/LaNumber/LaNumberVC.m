...
- (void)viewDidLoad {
    ...
    [_resetButton setTitle:(_isChinese ? @"重新设置" : @"Cài đặt lại") forState:UIControlStateNormal];
    [_forgetButton setTitle:(_isChinese ? @"忘记密码" : @"Quên mật khẩu") forState:UIControlStateNormal];
...
- (void)naviTitle {
    if (self.type == 0) {
        self.navigationItem.title = _isChinese ? @"设置数字密码" : @"Thiết lập mật khẩu kỹ thuật số";
        self.titleLabel.text = _isChinese ? @"输入数字密码" : @"Nhập mật khẩu số";
    }else if (self.type == 1) {
        self.navigationItem.title = _isChinese ? @"验证数字密码" : @"Xác nhận mật khẩu số";
        self.titleLabel.text = _isChinese ? @"验证数字密码" : @"Xác nhận mật khẩu số";
    }else if (self.type == 2) {
        self.navigationItem.title = _isChinese ? @"验证数字密码" : @"Xác nhận mật khẩu số";
        self.titleLabel.text = _isChinese ? @"验证数字密码" : @"Xác nhận mật khẩu số";
    }else if (self.type == 3) {
        ...
        self.titleLabel.text = _isChinese ? @"验证数字密码" : @"Xác nhận mật khẩu số";
        UIButton *rightBtn = [self itemTitle:(_isChinese ? @"切换账号" : @"Đổi tài khoản") action:@selector(switchAccount)];
    ...
    if (self.type == 0) {
        [self doubleSetup:sender.tag success:^(NSString *number) {
            [AppService.sharedAppService requestUrl:@"/device_lock/update_device_number" params:@{@"newNumber":number} success:^(NSDictionary * _Nonnull dict) {
                [LockStatusManager.main getLockStatusData:^(BOOL lockStatus) {
                    [weakself reset:nil];
        ...
        if (_isReset == NO) {
            ...
            [self doubleSetup:sender.tag success:^(NSString *number) {
                [AppService.sharedAppService requestUrl:@"/device_lock/update_device_number" params:@{@"oldNumber":LockStatusManager.main.lockStatus.number, @"newNumber":number} success:^(NSDictionary * _Nonnull dict) {
                    [weakself reset:nil];
                    if (weakself.pswBlock) {
                        weakself.pswBlock(number);
                    }
                    [LockStatusManager.main reWriteLockInfo:number ForKey:@"number"];
                    [self.navigationController popViewControllerAnimated:YES];
                } error:^(int errCode, NSString * _Nonnull message) {
                    [weakself reset:nil];
                    [self.view makeToast:message duration:1.0 position:CSToastPositionCenter];
        ...
        [self doubleSetup:sender.tag success:^(NSString *number) {
            [AppService.sharedAppService requestUrl:@"/device_lock/reset_device_number" params:@{@"code":weakself.code, @"newNumber":number} success:^(NSDictionary * _Nonnull dict) {
                ...
                for (UIViewController *vc in self.navigationController.viewControllers) {
                    if ([vc isKindOfClass:LaLockVC.class]) {
...
- (void)singleVerification:(NSInteger)num success:(void(^)(void))successBlock {
    ...
    if (_firstDatas.count >= 4) { // 可以跟服务器的值进行对比、如果是正确的、就直接返回
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            if (![LockStatusManager.main.lockStatus.number isEqualToString:first]) {
                [weakself.firstDatas removeAllObjects];
                weakself.titleLabel.text = (self->_isChinese ? @"密码错误，请重新输入..." : @"Mật khẩu sai, xin vui lòng nhập lại …");
    ...
    if (_group == 0) {
        ...
        if (_firstDatas.count >= 4) {
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                ...
                weakself.titleLabel.text = self->_isChinese ? @"再次输入" : @"Một lần nữa nhập";
            });
        }else {
            self.currentIndex = _firstDatas.count - 1;
        }
        ...
        if (_secondDatas.count >= 4) {
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                ...
                if (![first isEqualToString:second]) {
                    ...
                    weakself.titleLabel.text = (self->_isChinese ? @"与首次输入不一致，请重新输入" : @"Mật khẩu không khớp");
                    weakself.titleLabel.textColor = UIColor.systemRedColor;
                    [weakself shakeLabel:weakself.titleLabel];
                    return;
                }
...
- (IBAction)forget:(UIButton *)sender { // 忘记数字密码
    LaClearChatVC *vc = LaClearChatVC.new;
    vc.type = self.type;
    WS(weakself)
