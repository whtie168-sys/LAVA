...
- (void)updateADFLanguage {
    ...
    if (_isChinese) {
        ...
        _descL.text = @"Sau khi kích hoạt, LAVA vượt quá thời lượng đã đặt, khi mở lại cần cài đặt mật khẩu hình vẽ*.  Để bảo mật dữ liệu,  nếu như bạn quên mật khẩu cảu khóa bảo mật, cần phải xóa dữ liệu trò chuyện mới có thể tiếp tục sử dụng.";
        _automaticLockingL.text = @"Tự động khóa";
        _modityL.text = @"Sửa đổi mật khẩu kỹ thuật số";
        _forgotL.text = @"Quên mật khẩu số";
    }
}

- (void)initData {
    LockStatus *lockStatus = LockStatusManager.main.lockStatus;
...
- (IBAction)lock:(UISwitch *)sender {
    WS(weakself)
    if (sender.on) {
        NSString *title = @"开启安全锁后，如果您在启动LAVA时忘记了密码，需要清除账户聊天记录才能继续使用，请牢记您的安全锁密码";
        if (_isChinese) {
        }else {
            title = @"Sau khi kích hoạt khóa bảo mật, nếu như trong quá trình mở LAVA quên mật khẩu cần phải xóa hết dữ liệu trò chuyện mới có thể tiếp tục sử dụng. Vui lòng ghi nhớ khóa bảo mật của bạn.";
        }
        ...
        UIAlertAction *okAct = [UIAlertAction actionWithTitle:(_isChinese ? @"确定开启" : @"Xác nhận kích hoạt") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
...
- (IBAction)forget:(UIButton *)sender {
    LaGetCodeVC *vc = LaGetCodeVC.new;
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (void)requestLockTime:(NSInteger)row {
    ...
    [AppService.sharedAppService requestUrl:@"/device_lock/set_status" params:@{@"status":@(_lockSW.on ? 1 : 0), @"waitTime":number} success:^(NSDictionary * _Nonnull dict) {
        ...
        [LockStatusManager.main reWriteLockInfo:number ForKey:@"waitTime"];
    } error:^(int errCode, NSString * _Nonnull message) {
        weakself.timeLabel.text = [NSString stringWithFormat:@"%@ %@",LLLLLL(@"Backstage"), [weakself tranfrom:LockStatusManager.main.lockStatus.waitTime]];
...
- (NSString *)tranfrom:(NSInteger)min {
    NSString *value = @"";
    if (min < 60) {
        value = [NSString stringWithFormat:@"%ld%@",min, (_isChinese?@"分钟":@" Phút")];
    }else if (min < 24*60) {
        value = [NSString stringWithFormat:@"%ld%@",min/60, (_isChinese?@"小时":@" Giờ")];
    }
    return value;
}
