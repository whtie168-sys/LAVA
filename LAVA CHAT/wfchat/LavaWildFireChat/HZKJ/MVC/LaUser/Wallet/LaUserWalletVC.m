...
@interface LaUserWalletVC () <UITableViewDelegate, UITableViewDataSource>
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = LLLLLL(@"MyWallet");
...
- (void)setupHeaderCard {
    ...
    titleLabel.text = LLLLLL(@"WalletCoinWallet");
    ...
    balanceTitleLabel.text = LLLLLL(@"WalletBalance");
...
- (void)setupFilterView {
    ...
    detailLabel.text = LLLLLL(@"WalletDetails");
    ...
    self.allButton = [self createFilterButton:LLLLLL(@"WalletAll") selected:YES];
    self.incomeButton = [self createFilterButton:LLLLLL(@"WalletIncome") selected:NO];
    self.expenseButton = [self createFilterButton:LLLLLL(@"WalletExpense") selected:NO];
    self.packetButton = [self createFilterButton:LLLLLL(@"WalletRedPacket") selected:NO];
...
- (void)setupTableView {
    ...
    self.emptyLabel.text = LLLLLL(@"WalletNoDetails");
...
- (void)loadWalletData {
    ...
    [[AppService sharedAppService] signTasks:^(WFCCSignTasks * _Nonnull result) {
        ...
        errorMessage = message.length ? message : LLLLLL(@"WalletBalanceLoadFailed");
        dispatch_group_leave(group);
    }];
    ...
    [[AppService sharedAppService] pointsHistory:@{@"pageNo": @1, @"pageSize": @100} success:^(WFCCPointsHistory * _Nonnull result) {
        ...
        errorMessage = message.length ? message : LLLLLL(@"WalletHistoryLoadFailed");
        dispatch_group_leave(group);
    }];
...
- (void)rebuildSections {
    ...
    for (WFCCPointsHistoryRecord *record in filteredRecords) {
        ...
        if (!month.length) {
            month = LLLLLL(@"WalletUnknownMonth");
        }
        if (!recordMap[month]) {
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    incomeLabel.text = [NSString stringWithFormat:LLLLLL(@"WalletIncomeFormat"), sectionModel.income];
    ...
    expenseLabel.text = [NSString stringWithFormat:LLLLLL(@"WalletExpenseFormat"), sectionModel.expense];
...
- (void)updateFilterButton:(UIButton *)button selected:(BOOL)selected {
    UIColor *selectedColor = [UIColor colorWithRed:0x17 / 255.0 green:0x9F / 255.0 blue:0xFE / 255.0 alpha:1.0];
    ...
    [button setTitleColor:selected ? selectedColor : [UIColor colorWithWhite:0.16 alpha:1.0] forState:UIControlStateNormal];
}
