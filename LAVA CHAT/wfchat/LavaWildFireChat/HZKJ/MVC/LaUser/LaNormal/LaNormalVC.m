...
- (IBAction)dataClear:(UIButton *)sender {
    UIAlertController *actionSheet = [UIAlertController alertControllerWithTitle:LLLLLL(@"ClearCache") message:([CommonHelper.main isChinese] ? @"该操作会将缓存数据全部清除，且无法恢复，是否继续？" : @"Thao tác này sẽ xóa sạch dữ liệu cache và không thể khôi phục, bạn có muốn tiếp tục?") preferredStyle:UIAlertControllerStyleAlert];
