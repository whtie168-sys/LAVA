...
- (void)sendArea:(NSString *)area phone:(NSString *)phone  button:(UIButton *)btn {
    if (phone.length <= 0) {
        [SVProgressHUD showErrorWithStatus:([self isChinese]?@"请输入手机号":@"Vui lòng nhập số điện thoại")];
        [SVProgressHUD dismissWithDelay:1.0];
        return ;
    }
    if (phone.length < 6) {
        [SVProgressHUD showErrorWithStatus:([self isChinese]?@"手机号码格式有误":@"Số điện thoại bị lỗi")];
        [SVProgressHUD dismissWithDelay:1.0];
        return ;
    }
    WS(weakself)
...
- (void)sendEmailCode:(NSString *)email button:(UIButton *)btn {
    if (email.length <= 0) {
        [SVProgressHUD showErrorWithStatus:([self isChinese]?@"请输入邮箱地址":@"Xin vui lòng nhập vào hộp thư")];
        [SVProgressHUD dismissWithDelay:1.0];
        return ;
    }
    WS(weakself)
    btn.userInteractionEnabled = NO;
    [AppService.sharedAppService requestUrlNoLogin:@"/send_email_code" params:@{@"email":email} success:^(NSDictionary * _Nonnull dict) {
        btn.userInteractionEnabled = NO;
        [SVProgressHUD showSuccessWithStatus:LLLLLL(@"SentSuccessfully")];
        [SVProgressHUD dismissWithDelay:1.0];
        [weakself handleTimer:btn];
...
- (void)sendEmailCodeForForget:(NSString *)email button:(UIButton *)btn {
    ...
    WS(weakself)
    btn.userInteractionEnabled = NO;
    [AppService.sharedAppService requestUrlNoLogin:@"/send_forgot_password_email_code" params:@{@"email":email} success:^(NSDictionary * _Nonnull dict) {
        btn.userInteractionEnabled = NO;
        [SVProgressHUD showSuccessWithStatus:LLLLLL(@"SentSuccessfully")];
        [SVProgressHUD dismissWithDelay:1.0];
        [weakself handleTimer:btn];
...
- (void)sendForgetCode:(NSString *)phone area:(NSString *)area button:(UIButton *)btn {
    WS(weakself)
    btn.userInteractionEnabled = NO;
    [AppService.sharedAppService requestUrlNoLogin:@"/send_forgot_password_code" params:@{@"mobile":phone, @"area":area} success:^(NSDictionary * _Nonnull dict) {
        btn.userInteractionEnabled = NO;
        [SVProgressHUD showSuccessWithStatus:LLLLLL(@"SentSuccessfully")];
        [SVProgressHUD dismissWithDelay:1.0];
        [weakself handleTimer:btn];
...
- (void)send_reset_device_code_button:(UIButton *)btn {
    WS(weakself)
    btn.userInteractionEnabled = NO;
...
- (void)updateAppSuccess:(void(^)(BOOL isUpdate))success {
    ...
    [AppService.sharedAppService requestUrlNoLogin:@"/global_config" params:@{} success:^(NSDictionary * _Nonnull dict) {
        ...
        UpdatedVersionPopView *popView = [[UpdatedVersionPopView alloc] init];
...
//        strSeenTime = [NSString stringWithFormat:@"%d天前", days];
        ...
        if(hours) {
            strSeenTime = [NSString stringWithFormat:@"%d%@",hours, ([self isChinese]?@"小时前":@" giờ trước")];
        } else {
            int mins = (int)(duration/60);
            if(mins) {
                strSeenTime = [NSString stringWithFormat:@"%d%@",mins, ([self isChinese]?@"分前":@" phút trước")];
            } else {
...
- (void)showImagePikerWithimageBlock:(ImagePikerBlock)imageBlock {
    ...
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:(isChinese?@"请选择...":@"Vui lòng chọn...") message:nil preferredStyle:UIAlertControllerStyleActionSheet];
    UIAlertAction *photographicAction = [UIAlertAction actionWithTitle:(isChinese?@"拍照":@"Chụp ảnh") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [weakself openSystemPhotoAlbum:UIImagePickerControllerSourceTypeCamera];
    }];
    UIAlertAction *photoAction = [UIAlertAction actionWithTitle:(isChinese?@"从手机相册中选择":@"Chọn từ ảnh trong điện thoại") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
...
- (void)openSystemPhotoAlbum:(UIImagePickerControllerSourceType)sourceType {
    ...
    if (sourceType == UIImagePickerControllerSourceTypeCamera && ![UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
        UIAlertController* actionSheet = [UIAlertController alertControllerWithTitle:LLLLLL(@"Tips") message:([self isChinese]?@"该设备不支持拍照！":@"Thiết bị này không hỗ trợ chụp ảnh!") preferredStyle:UIAlertControllerStyleAlert];
    ...
    if(authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied) {
        UIAlertController* actionSheet = [UIAlertController alertControllerWithTitle:LLLLLL(@"Tips") message:([self isChinese]?@"请在iPhone的”设置-隐私-相机“选项中允许零阅使用相机权限":@"Vui lòng cho phép sử dụng quyền máy ảnh không duyệt trong Cài đặt - Quyền riêng tư - Tùy chọn máy ảnh của iPhone") preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction *cancelAct = [UIAlertAction actionWithTitle:LLLLLL(@"AlertButton") style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
        }];
        [actionSheet addAction:cancelAct];
        [vc presentViewController:actionSheet animated:YES completion:nil];
