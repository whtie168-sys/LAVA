...
- (NSString *)timeIntervalDateFormat:(NSString *)format {
   NSDate *detailDate = [NSDate dateWithTimeIntervalSince1970:self.integerValue / 1000];
...
- (NSString *)timeFromNow:(NSInteger)timestamp {
    NSDate *lastTime = [NSDate dateWithTimeIntervalSince1970:timestamp / 1000];
    ...
    if (minutes <= 10) {
        ...
        return [NSString stringWithFormat:@"%@ 在线",[df stringFromDate:lastDate]];
    }
}
