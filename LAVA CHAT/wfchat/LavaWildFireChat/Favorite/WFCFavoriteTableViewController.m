...
@property(nonatomic, strong)NSMutableArray<QWERFavoriteItem *> *items;
@property(nonatomic, strong)NSMutableArray<QWERFavoriteItem *> *imageVideoItems;
...
@property(nonatomic, strong)QWERFavoriteItem *playingSoundItem;
@end
...
- (void)playSound:(QWERFavoriteItem *)item {
...
- (void)showItem:(QWERFavoriteItem *)item {
    switch (item.favType) {
...
- (void)onTapCell:(UITapGestureRecognizer *)gestureRecognizer {
    if (gestureRecognizer.state == UIGestureRecognizerStateRecognized) {
        ...
        if (indexPath) {
            ...
            QWERFavoriteItem *item = self.selectedCell.favoriteItem;
            [self showItem:item];
        }
    }
}
...
- (NSMutableArray<QWERFavoriteItem *> *)imageVideoItems {
    NSMutableArray *is = [[NSMutableArray alloc] init];
    [self.items enumerateObjectsUsingBlock:^(QWERFavoriteItem * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
-(void)performDelete:(UIMenuController *)sender {
    QWERFavoriteItem *favItem = self.selectedCell.favoriteItem;
...
- (void)loadMoreData {
    ...
    [[AppService sharedAppService] getFavoriteItems:self.items.count ? [self.items lastObject].favId:0 count:100 success:^(NSArray<QWERFavoriteItem *> * _Nonnull items, BOOL hasMore) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    QWERFavoriteItem *favItem = self.items[indexPath.section];
...
- (id <MWPhoto>)photoBrowser:(MWPhotoBrowser *)photoBrowser photoAtIndex:(NSUInteger)index {
    QWERFavoriteItem *item = self.imageVideoItems[index];
