...
- (void)setFavoriteItem:(QWERFavoriteItem *)favoriteItem {
    ...
    self.asouDurationLabel.text = [NSString stringWithFormat:@"%d 秒", duration];
    self.asouDurationLabel.frame = CGRectMake(8, image.size.height - 30, image.size.width-16, 20);
}
...
- (UIImageView *)thumbView {
    if (!_thumbView) {
        ...
        [self.asoucContentArea addSubview:_thumbView];
    }
    return _thumbView;
}
...
- (UILabel *)asouDurationLabel {
    if (!_asouDurationLabel) {
        ...
        _asouDurationLabel.textAlignment = NSTextAlignmentRight;
        _asouDurationLabel.font = [UIFont systemFontOfSize:14];
        [self.thumbView addSubview:_asouDurationLabel];
    }
