...
- (void)setFavoriteItem:(QWERFavoriteItem *)favoriteItem {
    ...
    self.asoucContentArea.frame = CGRectMake(16, 16, [UIScreen mainScreen].bounds.size.width - 16, contentHeight);
    self.buttomLabel.frame = CGRectMake(16, contentHeight+24, self.bounds.size.width-32, 16);
    self.buttomLabel.text = [favoriteItem.origin stringByAppendingFormat:@"  %@", [QWERUtilities formatTimeLabel:favoriteItem.timestamp]];
}
...
- (UIView *)asoucContentArea {
    if (!_asoucContentArea) {
        ...
        [self.contentView addSubview:_asoucContentArea];
    }
