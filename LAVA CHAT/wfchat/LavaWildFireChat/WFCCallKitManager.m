...
- (void)didReceiveCall:(WFAVCallSession *)session {
    if(self.callUUIDDict[session.callId]) {
        ...
        [self reportIncomingCallWithTitle:(userInfo.friendAlias.length > 0 ? userInfo.friendAlias : userInfo.displayName) Sid:session.initiator audioOnly:session.audioOnly callId:session.callId uuid:currentCall];
    }
    
}
- (void)didCallEnded:(WFAVCallEndReason)reason duration:(int)callDuration {
    if([Chat86AVEngineKit sharedEngineKit].currentSession.callUUID) {
        CXEndCallAction *endAction = [[CXEndCallAction alloc] initWithCallUUID:[Chat86AVEngineKit sharedEngineKit].currentSession.callUUID];
...
- (void)didReceiveIncomingPushWithPayload:(PKPushPayload *)payload
    ...
    if(wfc) {
        ...
        [self reportIncomingCallWithTitle:(userInfo.friendAlias.length > 0 ? userInfo.friendAlias : userInfo.displayName) Sid:sender audioOnly:audioOnly callId:callId];
    }
}
...
- (void)provider:(CXProvider *)provider performEndCallAction:(CXEndCallAction *)action {
    ...
    if([action.callUUID isEqual:[Chat86AVEngineKit sharedEngineKit].currentSession.callUUID]) {
        if ([Chat86AVEngineKit sharedEngineKit].currentSession.state != kWFAVEngineStateIdle) {
            [[Chat86AVEngineKit sharedEngineKit].currentSession endCall];
        }
    }
    [action fulfill];
}
-(void)provider:(CXProvider *)provider didActivateAudioSession:(AVAudioSession *)audioSession {
    if ([Chat86AVEngineKit sharedEngineKit].currentSession.state == kWFAVEngineStateIncomming) {
        [[Chat86AVEngineKit sharedEngineKit].currentSession answerCall:false callExtra:nil];
        
        UIViewController *videoVC;
        if ([Chat86AVEngineKit sharedEngineKit].currentSession.conversation.type == Group_Type && [Chat86AVEngineKit sharedEngineKit].supportMultiCall) {
            videoVC = [[RADCOMultiVideoVC alloc] initWithSession:[Chat86AVEngineKit sharedEngineKit].currentSession];
        } else {
            videoVC = [[RADCOVideoVC alloc] initWithSession:[Chat86AVEngineKit sharedEngineKit].currentSession];
        }
