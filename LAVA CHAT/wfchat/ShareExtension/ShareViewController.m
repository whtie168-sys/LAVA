...
- (void)viewDidLoad {
    ...
    if (![[ShareAppService sharedAppService] isLogin]) {
        ...
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"没有登录" message:@"请先登录LAVA" preferredStyle:UIAlertControllerStyleAlert];
        
        
        UIAlertAction *cancel = [UIAlertAction actionWithTitle:LLLLLL(@"Cancel") style:UIAlertActionStyleCancel handler:^(UIAlertAction *action) {
...
- (void)showImageLimit {
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"不支持发送多张图片" message:@"每次只能发送一张。如果您需要一次发送多张，请打开LAVA选择图片发送。" preferredStyle:UIAlertControllerStyleAlert];
...
- (void)showSuccess {
    ...
    __weak typeof(self)ws = self;
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"已发送" message:@"您可以在LAVA中查看" preferredStyle:UIAlertControllerStyleAlert];
...
- (void)showFailure {
    __weak typeof(self)ws = self;
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:LLLLLL(@"NetworkError") message:@"糟糕！网络出问题了！" preferredStyle:UIAlertControllerStyleAlert];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 0) {
        cell.asofaTextLabel.text = @"发给朋友";
    } else if(indexPath.row == 1) {
        cell.asofaTextLabel.text = @"发给自己";
    } else {
        cell.asofaTextLabel.text = @"分享到朋友圈";
    }
